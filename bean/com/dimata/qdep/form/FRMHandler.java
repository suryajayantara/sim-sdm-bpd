...
//	        System.out.println("--st2 : "+st2);
     ...
     * @throws parName == null exception
     */
    public static String[] getParamsStringValuesStatic(HttpServletRequest request, String parName) {
        String sVal[] = request.getParameterValues(parName);
        try {
            if (sVal == null) {
                return null;
            }
    ...
    public String[] getParamsStringValuesStaticFrm(String parName) {
        String sVal[] = req.getParameterValues(parName);
        try {
            if (sVal == null) {
                return null;
    ...
    private String getParamDate(String baseName) {
        try {
            ...
            //if(yr<1 || mn<1 || dy<1){
            //update by priska 2016-04-27
            if ((yr < 1 || mn < 1 || dy < 1) && hh < 1 && mm < 1) {
                return "";
            } //update by satrya 2012-11-27
            else if ((yr < 1 || mn < 1 || dy < 1) && hh > 1) {
                Date dtX = new Date();

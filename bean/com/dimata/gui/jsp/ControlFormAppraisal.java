...
import com.dimata.harisma.entity.masterdata.KPI_Employee_Target;
...
import com.dimata.harisma.entity.masterdata.PstKPI_Employee_Target;
...
public class ControlFormAppraisal {
	...
	private static String createItemFromEmployeeKpi(AppraisalMain appMain, AssessmentFormSection assSection, AssessmentFormItem assessmentFormItem, Appraisal appraisal, Vector vRating) {
		
		String whereEmployeeID =  "TAR."+PstKPI_Employee_Target.fieldNames[PstKPI_Employee_Target.FLD_EMPLOYEE_ID] + " = " + appMain.getEmployeeId() 
							+ " AND TAR." + PstKPI_Employee_Target.fieldNames[PstKPI_Employee_Target.FLD_STARTDATE]+" BETWEEN '"+appMain.getDataPeriodFrom()+"' AND '"+appMain.getDataPeriodTo()+"'";
		Vector listKpiTarget = PstKPI_Employee_Target.listJoinGrup(0, 0, whereEmployeeID, "GR.KPI_GROUP_ID, KPI.PARENT_ID") ; 
		String strItem = "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\"><tbody><tr><td width=\"100%\">";
		...
		if (listKpiTarget.size()>0){
			...
			for (int i=0; i < listKpiTarget.size(); i++){
				Vector vectTemp = (Vector) listKpiTarget.get(i);
				KPI_Employee_Target target = (KPI_Employee_Target) vectTemp.get(0);
				KPI_Group group = (KPI_Group) vectTemp.get(1);
				if (oidGroup != group.getKpi_group_id()){
					strItem += "<table width='100%' cellspacing='0' cellpadding='0' border='0'>";
					strItem += "<tbody><tr><td><div align='justify'><p>"+group.getGroup_title()+"</p></div></td></tr></tbody>";
					strItem += "</table>";
					oidGroup = group.getKpi_group_id();
				}
				
				KPI_List kpi_list = new KPI_List();
				try {
					kpi_list = PstKPI_List.fetchExc(target.getKpiListId());
				} catch (Exception exc){}
				...
				if (kpi_list.getParentId()>0){
					...
					if (oidParent != kpi_list.getParentId()){
						...
						double score = PstKPI_Employee_Achiev.getTotalParentScore(kpiParent.getOID(), appMain.getDataPeriodFrom(), appMain.getDataPeriodTo());
						double point = 0;
						if (appraisal.getPoint()>0){
							point = appraisal.getPoint();
						} else {
							for (int x = 0; x < vRating.size(); x++) {
								Evaluation evCurr = (Evaluation) vRating.get(x);
								Evaluation evPrevious = new Evaluation();
								if (x - 1 >= 0 /*i+1<vRating.size()*/) {
									evPrevious = (Evaluation) vRating.get(x - 1);
								}
								double maxValCurr = evCurr.getMaxPercentage();
								double maxValPrev = evPrevious.getMaxPercentage();
								
								if (score > (maxValPrev + 1 )){
									point = evCurr.getMaxPoint();
								} 
							}
						}
						
						String whereKPI = PstAppraisal.fieldNames[PstAppraisal.FLD_KPI_ID]+"="+kpiParent.getOID()+
								" AND "+ PstAppraisal.fieldNames[PstAppraisal.FLD_APP_MAIN_ID]+"="+appMain.getOID()+
								" AND "+ PstAppraisal.fieldNames[PstAppraisal.FLD_ASS_FORM_ITEM_ID]+"="+assessmentFormItem.getOID();
						Vector listAppMain = PstAppraisal.list(0, 0, whereKPI, "");
						Appraisal appraisalKpi = new Appraisal();
						if (listAppMain.size()>0){
							appraisalKpi = (Appraisal) listAppMain.get(0);
						}
						
						number++;
						strItem += "<table class=\"pageCover\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
						strItem += "<tbody><tr>";
						strItem += "<td width=\"5%\" valign=\"top\" height=\"100%\" align=\"left\">"
								+ "<input type='hidden' name='FRM_FIELD_KPI"+assessmentFormItem.getOID()+"' value='"+kpiParent.getOID()+"'>"
								+ "<input type='hidden' name='" + FrmAppraisal.fieldNames[FrmAppraisal.FRM_FIELD_APPRAISAL_ID] + assessmentFormItem.getOID()+"_"+kpiParent.getOID() + "' value='"+appraisalKpi.getOID()+"'>"
								+ "<input type='hidden' name='" + FrmAppraisal.fieldNames[FrmAppraisal.FRM_FIELD_ASS_FORM_ITEM_ID] + assessmentFormItem.getOID()+"_"+kpiParent.getOID() + "' value='" + ((assessmentFormItem.getOID() > 0) ? assessmentFormItem.getOID() : 0) + "'>"
								+ "<b>"+number+".</b>"
								+ "</td>";
						strItem += "<td width=\"35%\" valign=\"top\" height=\"100%\" align=\"left\">"+kpiParent.getKpi_title()+"</td>";
						strItem += "<td class='pageCover' width=\"30\" height=\"100%\" align=\"left\">Weight :"
								+ "<input type=\"text\" size=\"5\" name=\"FRM_FIELD_WEIGHT_POINT"+assessmentFormItem.getOID()+"_"+kpiParent.getOID()+"\" value=\""+appraisalKpi.getWeight()+"\">"+
								"</td>";
						strItem += "<td width=\"20%\" class='pageCover' valign=\"top\" height=\"100%\" align=\"left\">"+kpiParent.getDescription()+"</td>";
						strItem += "<td width=\"30\" height=\"100%\">Score : "
								+ "<input type=\"text\" size=\"5\" name=\"FRM_FIELD_REALIZATION"+assessmentFormItem.getOID()+"_"+kpiParent.getOID()+"\" value=\""+score+"\">"+
								"</td>";
						strItem += "<td width=\"70\" height=\"100%\">Evidence : "
								+ "<input type=\"text\" size=\"50\" name=\"FRM_FIELD_WEIGHT_EVIDENCE"+assessmentFormItem.getOID()+"_"+kpiParent.getOID()+"\" value=\"\">"+
								"</td>";
						strItem += "<td width=\"30\" height=\"100%\">Point : "
								+ "<input type=\"text\" readonly=\"\" size=\"5\" name=\"FRM_FIELD_POINT"+assessmentFormItem.getOID()+"_"+kpiParent.getOID()+"\" value=\""+point+"\">"+
								"</td>";
						strItem += "<td width=\"30\" height=\"100%\">Point x Weight : "
								+ "<input type=\"text\" readonly=\"\" size=\"5\" name=\"FRM_FIELD_RATING"+assessmentFormItem.getOID()+"_"+kpiParent.getOID()+"\" value=\""+(point * appraisalKpi.getWeight() )+"\">"+
								"</td>";
						oidParent = kpi_list.getParentId();
					}
				} else {
					double score = PstKPI_Employee_Achiev.getTotalScore(target.getKpiListId(), appMain.getDataPeriodFrom(), appMain.getDataPeriodTo());
					double point = 0;
					if (appraisal.getPoint()>0){
						point = appraisal.getPoint();
					} else {
					...
					String whereKPI = PstAppraisal.fieldNames[PstAppraisal.FLD_KPI_ID]+"="+kpi_list.getOID()+
							" AND "+ PstAppraisal.fieldNames[PstAppraisal.FLD_APP_MAIN_ID]+"="+appMain.getOID()+
							" AND "+ PstAppraisal.fieldNames[PstAppraisal.FLD_ASS_FORM_ITEM_ID]+"="+assessmentFormItem.getOID();
					Vector listAppMain = PstAppraisal.list(0, 0, whereKPI, "");
					Appraisal appraisalKpi = new Appraisal();
					...
					strItem += "<tbody><tr>";
					strItem += "<td width=\"5%\" valign=\"top\" height=\"100%\" align=\"left\">"
							+ "<input type='hidden' name='FRM_FIELD_KPI"+assessmentFormItem.getOID()+"' value='"+kpi_list.getOID()+"'>"
							+ "<input type='hidden' name='" + FrmAppraisal.fieldNames[FrmAppraisal.FRM_FIELD_APPRAISAL_ID] + assessmentFormItem.getOID()+"_"+kpi_list.getOID() + "' value='"+appraisalKpi.getOID()+"'>"
								+ "<input type='hidden' name='" + FrmAppraisal.fieldNames[FrmAppraisal.FRM_FIELD_ASS_FORM_ITEM_ID] + assessmentFormItem.getOID()+"_"+kpi_list.getOID() + "' value='" + ((assessmentFormItem.getOID() > 0) ? assessmentFormItem.getOID() : 0) + "'>"
							+ "<b>"+number+".</b>"
							+ "</td>";
					strItem += "<td width=\"35%\" valign=\"top\" height=\"100%\" align=\"left\">"+kpi_list.getKpi_title()+"</td>";
					strItem += "<td class='pageCover' width=\"30\" height=\"100%\" align=\"left\">Weight :"
							+ "<input type=\"text\" size=\"5\" name=\"FRM_FIELD_WEIGHT_POINT"+assessmentFormItem.getOID()+"_"+kpi_list.getOID()+"\" value=\""+appraisalKpi.getWeight()+"\">"+
							"</td>";
					strItem += "<td width=\"20%\" class='pageCover' valign=\"top\" height=\"100%\" align=\"left\">"+kpi_list.getDescription()+"</td>";
					strItem += "<td width=\"30\" height=\"100%\">Score : "
								+ "<input type=\"text\" size=\"5\" name=\"FRM_FIELD_REALIZATION"+assessmentFormItem.getOID()+"_"+kpi_list.getOID()+"\" value=\""+score+"\">"+
								"</td>";
					strItem += "<td width=\"70\" height=\"100%\">Evidence : "
							+ "<input type=\"text\" size=\"50\" name=\"FRM_FIELD_WEIGHT_EVIDENCE"+assessmentFormItem.getOID()+"_"+kpi_list.getOID()+"\" value=\"\">"+
							"</td>";
					strItem += "<td width=\"30\" height=\"100%\">Point : "
							+ "<input type=\"text\" readonly=\"\" size=\"5\" name=\"FRM_FIELD_POINT"+assessmentFormItem.getOID()+"_"+kpi_list.getOID()+"\" value=\""+point+"\">"+
							"</td>";
					strItem += "<td width=\"30\" height=\"100%\">Point x Weight : "
							+ "<input type=\"text\" readonly=\"\" size=\"5\" name=\"FRM_FIELD_RATING"+assessmentFormItem.getOID()+"_"+kpi_list.getOID()+"\" value=\""+(point * appraisalKpi.getWeight() )+"\">"+
							"</td>";
				}
			}
		}
    ...
    private static String createItemTypeKpiEmployeePosition(AppraisalMain appMain, AssessmentFormSection assSection, AssessmentFormItem assessmentFormItem, Appraisal appraisal) {
        ...
        if(appMain.getOID() == assessmentFormItem.getAppMainId()){
            alreadyHasItem = true;
        ...
        if(assessmentFormItem.getAppMainId() < 0 && !alreadyHasItem){
            if(assessmentFormItem.getNumber() > 0){
                strItem += "<table width='100%' border='0' cellspacing='0' cellpadding='0' class='pageCover'>";
                strItem += "<tr>";
                strItem += "  <td width='90%' height='100%'>";
    ...
    public static String createPage(long mainOid, int page, long appraisalMainOid) {
        ...
        if (vSection.size() > 0) {
            for (int k = 0; k < vSection.size(); k++) {
				...
				String sWhere = assSection.getPointEvaluationId() == 0 ? "" : PstEvaluation.fieldNames[PstEvaluation.FLD_EVAL_TYPE] + "=" + assSection.getPointEvaluationId();
				Vector vRating = PstEvaluation.list(0, 0, sWhere, PstEvaluation.fieldNames[PstEvaluation.FLD_MAX_POINT] + "," + PstEvaluation.fieldNames[PstEvaluation.FLD_MAX_PERCENTAGE] + " DESC");
                    ...
                    if (assItem.getPage() == page) {
                        ...
                        switch (assItem.getType()) {
                            ...
                            case PstAssessmentFormItem.ITEM_TYPE_KPI_EMPLOYEE_POSITION:
                                strPage += createItemTypeKpiEmployeePosition(appMain, assSection, assItem, appraisal);
                                break;
                        }
                        strPage += "</td>";
                        strPage += "</tr>";

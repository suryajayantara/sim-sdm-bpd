...
public class CtrlLeaveApplication extends Control implements I_Language {
    ...
    public static int RSLT_HAVE_PRESENCE_TIME = 5;
    public static String[][] resultText = {
        {"Berhasil", "Tidak dapat diproses", "NoPerkiraan sudah ada", "Data tidak lengkap","Membuat / Update Form Cuti Berhasil","ada presence time"},
        {"Succes", "Can not process", "Estimation code exist", "Data incomplete","Create / Update Form Cuti Success","have presence time"}};
    private int start;
        ...
        switch (cmd) {
            ...
            case Command.SAVE:
                ...
                if (autoUpdateStatus ==1){
                    leaveApplication.setDocStatus(PstLeaveApplication.FLD_STATUS_APPlICATION_TO_BE_APPROVE);
                }
            ...
            case Command.EDIT:
                ...
                if (oidLeaveApplication != 0) {
                    try {
                        ...
                        Vector listOfDetail = PstLeaveApplicationDetail.listDetailByApplicationMain(oidLeaveApplication);
                        leaveApplication.setListOfDetail(listOfDetail);
                        if (!status_excecution){
                            return RSLT_HAVE_PRESENCE_TIME;
                        }
                    } catch (DBException dbexc) {
                        excCode = dbexc.getErrorCode();
                        msgString = getSystemMessage(excCode);

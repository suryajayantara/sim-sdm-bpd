...
//    }
    ...
    public void requestEntityMultipleObjectDivisionV2(long kpiListId, int tahun,long companyId, Vector VListCompanyTarget ) { //melakukan 
        ///pemanggilan terhadap Employee
        try {
            this.requestParam();
            ...
            double total = 0;
                            ...
                            for (int m = 1; m <=12; m++){
                                for (int x = 0; x < VDivision.size(); x++) {
                                    Division division = (Division)VDivision.get(x);
                                    double value = this.getParamDouble(fieldNames[FRM_FIELD_PERCENT_DIVISION]+division.getOID()+"_"+m);
                                    double achievement = this.getParamDouble(fieldNames[FRM_FIELD_ACHIEVEMENT_DIVISION]+division.getOID()+"_"+m);
                                    String month = "";
                                    if (m < 10){
                                        month = "0"+m;
                                    } else {

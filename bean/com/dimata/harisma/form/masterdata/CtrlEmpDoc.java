    ...
    public int action(int cmd, long oidEmpDoc) {
        ...
        switch (cmd) {
          ...
          case Command.SAVE :
				...
				boolean checkSk = PstEmpDoc.checkNoSk(empDoc.getDoc_number());
				if (checkSk){
					msgString = FRMMessage.getMessage(FRMMessage.MSG_DOC_NUM_IN_EXIST);
				}
				...
				if (empDoc.getLeaveId()>0){
					try {
						LeaveApplication leaveApplication = PstLeaveApplication.fetchExc(empDoc.getLeaveId());
			...
			case Command.EDIT :
				...
				if (oidEmpDoc != 0) {
					try {
						empDoc = PstEmpDoc.fetchExc(oidEmpDoc);
                                                PstEmpDoc.updateEmpDoc(oidEmpDoc,PstLeaveApplication.FLD_STATUS_APPlICATION_CANCELED);
                                                PstEmpDoc.cancelUpdateSchedule(oidEmpDoc);

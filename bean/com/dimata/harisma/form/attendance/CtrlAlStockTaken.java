...
public class CtrlAlStockTaken extends Control implements I_Language {
    ...
    private String msgPentingString = "";
    ...
    public String getMessagePenting() {
        return msgPentingString;
    }
...
//                }
                ...
                if (leaveApp.getTypeLeaveCategory() == 1){
                    //anda mengambil cuti hamil
                    //float stockAl = SessLeaveApplication.getEligibleAnnualLeave(null, alStockTaken) ;
                    msgPentingString = msgPentingString + " : anda mengambil cuti hamil ";
                }
                if (leaveApp.getTypeLeaveCategory() == 2){
                     if (alStockTaken.getTakenQty() > 3){
                          msgPentingString = msgPentingString + " : taken qty can't more than 3" ;
                          //return RSLT_FORM_INCOMPLETE;
                     }   
                     Period periodeCutiPenting = new Period();
                     try{
                          periodeCutiPenting = PstPeriod.getPeriodBySelectedDate(alStockTaken.getTakenDate());
                     }catch(Exception e){}
                          boolean nilai = PstAlStockTaken.existAlStockTakenByCutiPenting(alStockTaken.getEmployeeId(), periodeCutiPenting.getStartDate(), periodeCutiPenting.getEndDate());
                     if (nilai){
                          msgPentingString = msgPentingString + " : cuti penting of this periode can't be taken " ;
                          //return RSLT_FORM_INCOMPLETE;
                     }
                }

...
public class FrmOvertimeDetail extends FRMHandler implements /*I_Language,*/I_FRMInterface, I_FRMType{
    ...
    public static final int FRM_FIELD_NORMAL_OVERTIME=19;
    public static final int FRM_FIELD_LATE_APPROVAL=20;

     public static final int FRM_FIELD_UPLOAD_FILE=21;
    ...
    public static final String[] fieldNames={
        ...
        "FRM_FIELD_NORMAL_OVERTIME",
        "FRM_FIELD_LATE_APPROVAL",
        "FRM_FIELD_UPLOAD_FILE"
    };
		...
		try{
			...
			overtimeDetail.setEmployeeId(getLong(FRM_FIELD_EMPLOYEE_ID));}
                       ...
                       if(getString(FRM_FIELD_NAME)!=null && getString(FRM_FIELD_NAME).length()>0){
                        ...
                        overtimeDetail.setDateFrom(java.sql.Date.valueOf(getString(FRM_FIELD_DATE_FROM)));
                        overtimeDetail.setDateTo(java.sql.Date.valueOf(getString(FRM_FIELD_DATE_TO)));
                        ...
                        overtimeDetail.setNormalOvertime(getInt(FRM_FIELD_NORMAL_OVERTIME));
                        overtimeDetail.setLateApproval(getInt(FRM_FIELD_LATE_APPROVAL));
                        overtimeDetail.setUploadFile(getString(FRM_FIELD_UPLOAD_FILE));
		}catch(Exception e){
		...
		try{
			...
			overtimeDetail.setEmployeeId(getLong(FRM_FIELD_EMPLOYEE_ID));}
                       ...
                       if(getString(FRM_FIELD_NAME)!=null && getString(FRM_FIELD_NAME).length()>0){
                        ...
                        String startDate = getString(FRM_FIELD_DATE_FROM);
                        Date dtNow = new Date();
                        if (!(startDate.equals(""))){
                            dtNow = df.parse(startDate);
                        }
                        
                        String endDate = getString(FRM_FIELD_DATE_TO);
                        Date dtEnd = new Date();
                        if (!(endDate.equals(""))){
                            dtEnd = df.parse(endDate);
                        }

                        
                        overtimeDetail.setDateFrom(dtNow);
                        overtimeDetail.setDateTo(dtEnd);
                         if(getInt(FRM_FIELD_STATUS)!=0){
                        overtimeDetail.setStatus(getInt(FRM_FIELD_STATUS));}

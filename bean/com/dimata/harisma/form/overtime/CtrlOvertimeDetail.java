        ...
        switch (cmd) {
            ...
            case Command.SAVE:
               ...
               // Hashtable hashCekOverlap = new Hashtable();
                ...
                try {
                    String strBackDate = PstSystemProperty.getValueByName("OVERTIME_CAN_INPUT_BACKDATE");
                    boolean canInputBackdate = Boolean.valueOf(strBackDate);
                    if (!canInputBackdate){
                        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
                        if (dateFormat.parse(dateFormat.format(overtimeDetail.getDateFrom())).before(dateFormat.parse(dateFormat.format(new Date())))){
                            int iErrCode = 8 ;
                            msgString =   " Tidak dapat input form lembur untuk tanggal yang sudah lewat ";
                            return RSLT_FORM_INCOMPLETE;
                        }
                    }
                } catch (Exception exc){}
                 ...
                 try {
                        Employee emp = PstEmployee.fetchExc(overtimeDetail.getEmployeeId());
                        ...
                        if (isCheck){
                            String limitStart = PstSystemProperty.getValueByName("OVERTIME_MIN_START_TIME");
                            String limitStartOther = PstSystemProperty.getValueByName("OVERTIME_OTHER_MIN_START_TIME");
                            long ovtOtherPosId = 0;
                            try {
                                ovtOtherPosId = Long.valueOf(PstSystemProperty.getValueByName("OVERTIME_OTHER_POSITION_ID"));
                            } catch (Exception exc){}
                            Vector listPH = PstPublicHolidays.list(0, 0, "'"+Formater.formatDate(overtimeDetail.getDateFrom(), "yyyy-MM-dd")+"' BETWEEN  holiday_date AND holiday_date_to", "holiday_status");
                            boolean isPH = false;
                            if (listPH.size()>0){
                            ...
                            boolean ovtEndOfYear = PstOvertime.endOfYearOvertime(overtimeDetail.getDateFrom());
                            if (!isPH && !ovtEndOfYear){
                                ...
                                objCal.setTime(overtimeDetail.getDateFrom());

                                String day = stDays[objCal.get(Calendar.DAY_OF_WEEK)-1];
                                if(day != stDays[0] && day != stDays[6]){
                                    SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm") ;
                                    if (emp.getPositionId() == ovtOtherPosId){
                                        if (dateFormat.parse(dateFormat.format(overtimeDetail.getDateFrom())).before(dateFormat.parse(limitStartOther))){
                                            int iErrCode = 7 ;
                                            msgString =   " Jam Lembur tidak boleh kurang dari Pukul "+limitStart;
                                            return RSLT_FORM_INCOMPLETE;
                                        }
                                    } else {
                                        if (dateFormat.parse(dateFormat.format(overtimeDetail.getDateFrom())).before(dateFormat.parse(limitStart))){
                                            int iErrCode = 7 ;
                                            msgString =   " Jam Lembur tidak boleh kurang dari Pukul "+limitStart;
                                            return RSLT_FORM_INCOMPLETE;
                                        }

...
public class CtrlTrainingAttendacePlan extends Control implements I_Language {
	...
	public int action(int command, long oidTrainType){
                ...
                case Command.DELETE :
                        if (oidTrainType != 0) {
                            try {
                                ...
                                long oidTrainingActivityActual = 0;
                                Vector listTrainingCompetencyMappingDetail = PstTrainingCompetencyMappingDetail.listByTrainingAttId(oidTrainType);
        ...
        public int updateAttendance(int command, long oidEmployee, int hoursPlanned, long oidTrainingPlan,long oidTrainingActualId){
         return updateAttendance(command, oidEmployee, hoursPlanned, 0, oidTrainingPlan, null,oidTrainingActualId);
        }
        
        public int updateAttendance(int command, long oidEmployee, int hoursPlanned, int point, long oidTrainingPlan,  TrainingAttendancePlan trainAttendanceReturn, long oidTrainingActualId){
             msgString = "";
             int excCode = I_DBExceptionInfo.NO_EXCEPTION;
             int rsCode = RSLT_OK;
             ...
             switch(command){			
                    case Command.SAVE :
                        ...
                        try {
                            Vector listAttendance = PstTrainingAttendancePlan.list(0, 1, whereClause, "");
                            ...
                            trainAttendance.setDuration(hoursPlanned); trainAttendance.setPoint(point);
                        }
                        catch(Exception e) {
                        ...
                        try{
                            if(trainAttendance.getOID() == 0) {     /* insert new */
                                ...
                                //add by Eri for insert comptency when training attandance added after add mapping competency
                                 TrainingCompetencyMappingDetail.generateMapCompetencyDetail(oidTrainingActualId, oid,0);
                                 //end
                                 
                                trainAttendanceReturn.setOID(oid);
                                trainAttendanceReturn.setTrainPlanid(oidTrainingPlan);
                                trainAttendanceReturn.setEmployeeId(oidEmployee);
                                trainAttendanceReturn.setDuration(hoursPlanned);
                                trainAttendanceReturn.setPoint(point);
                            }
        ...
        //add by Eri Yudi 2002-07-17
        public int updateAttendanceAndMark(int command, long oidEmployee, int hoursPlanned, int point,String mark, long oidTrainingPlan,  TrainingAttendancePlan trainAttendanceReturn,long oidTrainingActualId){
             msgString = "";
             int excCode = I_DBExceptionInfo.NO_EXCEPTION;
             int rsCode = RSLT_OK;
             ...
             switch(command){			
                    case Command.SAVE :
                        ...
                        try {
                            ...
                            trainAttendance.setPoint(point);
                            trainAttendance.setRemark(mark);
                        }
                        catch(Exception e) {
                            System.err.println("Error on setting attendance");
                        }

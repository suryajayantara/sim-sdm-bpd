...
public class CtrlTrainingActivityActual extends Control implements I_Language 
{
	...
	public int action(int cmd , long oidTrainingActivityActual, long oidDepartment, HttpServletRequest request){
	 ...
	 return action(cmd, trainHours, null, oidTraining, oidSchedule, oidTrainingPlan, oidTrainingActivityActual, oidAttendances, request);
        }	
        
        public synchronized int action(int cmd, String[] trainHours, String[] trainPoints, long oidTraining, long oidSchedule, long oidTrainingPlan, long oidTrainingActivityActual, String[] oidAttendances, HttpServletRequest request){
		msgString = "";
		int excCode = I_DBExceptionInfo.NO_EXCEPTION;
		int rsCode = RSLT_OK;
		...
		switch(cmd){
			...
			case Command.SAVE :
				...
				if(trainingActivityActual.getOID()==0){
					try{
                                            long oid = pstTrainingActivityActual.insertExc(this.trainingActivityActual);                              
                                            PstTrainingHistory.insertTrainingHistory(oidAttendances, trainHours, trainPoints, oidTraining, oidTrainingPlan, oidSchedule, oid);
					}
                                        catch(DBException dbexc){
                                            excCode = dbexc.getErrorCode();
                                            msgString = getSystemMessage(excCode);
					...
					try {
                                            ...
                                            PstTrainingHistory.insertTrainingHistory(oidAttendances, trainHours, trainPoints, oidTraining, oidTrainingPlan, oidSchedule, trainingActivityActual.getOID());                                           
					}
                                        catch (DBException dbexc){
                                            excCode = dbexc.getErrorCode();
                                            msgString = getSystemMessage(excCode);
		...
		return rsCode;
            ...
            public synchronized int action(int cmd, String[] trainHours,String[] trainRemark , String[] trainPoints, long oidTraining, long oidSchedule, long oidTrainingPlan, long oidTrainingActivityActual, String[] oidAttendances, HttpServletRequest request,String[] preTest,String[] postTest){
		msgString = "";
		int excCode = I_DBExceptionInfo.NO_EXCEPTION;
		int rsCode = RSLT_OK;
		...
		switch(cmd){
			...
			case Command.SAVE :
				...
				if(trainingActivityActual.getOID()==0){
					try{
                                            long oid = pstTrainingActivityActual.insertExc(this.trainingActivityActual); 
                                            PstTrainingHistory.insertTrainingHistory(oidAttendances, trainHours,trainRemark, trainPoints, oidTraining, oidTrainingPlan, oidSchedule, oid, preTest, postTest);
                                           // PstTrainingHistory.insertTrainingHistory(oidAttendances, trainHours, trainPoints, oidTraining, oidTrainingPlan, oidSchedule, oid);
					}
                                        catch(DBException dbexc){
                                            excCode = dbexc.getErrorCode();
					...
					try {
                                            ...
                                            PstTrainingHistory.insertTrainingHistory(oidAttendances, trainHours,trainRemark, trainPoints, oidTraining, oidTrainingPlan, oidSchedule, trainingActivityActual.getOID(), preTest, postTest);
                                            //PstTrainingHistory.insertTrainingHistory(oidAttendances, trainHours, trainPoints, oidTraining, oidTrainingPlan, oidSchedule, trainingActivityActual.getOID());                                           
					}
                                        catch (DBException dbexc){
                                            excCode = dbexc.getErrorCode();
			...
			case Command.DELETE :
				if (oidTrainingActivityActual != 0){
					try{
						long oid = PstTrainingActivityActual.deleteExc(oidTrainingActivityActual);
						  // add by Eri Yudi for otomatis update training compotency mapping detail 
                                                        oidTrainingActivityActual = 0;
                                                        Vector listTrainingCompetencyMappingDetail = PstTrainingCompetencyMappingDetail.listByTrainingActivityActualId(oidTrainingActivityActual);
                                                        for(int xx = 0 ; xx < listTrainingCompetencyMappingDetail.size();xx++){
                                                            ...
                                                            oidTrainingActivityActual = objTrainingCompetencyMappingDetail.getTrainingActivityActualId();

                                                        }
                                                        if(oidTrainingActivityActual != 0){
                                                            TrainingCompetencyMappingDetail.generateEmployeeCompetency(oidTrainingActivityActual);
                                                            Vector listTrainingCompetencyMap = PstTrainingCompetencyMapping.listTrainingCompetencyMapByOidActual(oidTrainingActivityActual);
                                                            for(int xx = 0 ; xx < listTrainingCompetencyMap.size();xx++){
                                                                TrainingCompetencyMapping objTrainingCompetencyMapping = (TrainingCompetencyMapping) listTrainingCompetencyMap.get(xx);
                                                                long oidDel = PstTrainingCompetencyMapping.deleteExc(objTrainingCompetencyMapping.getOID());
                                                            }
                                                        }
                                                //end
                                                // also delete training history record

...
public class PayPrintPdf extends HttpServlet {
    ...
    private static int startRowSlipComp = 8; // start row slip component
    ...
    public static int TOTAL_BENEFIT=0;
    public static int TOTAL_DEDUCTION=1;
    public static int TOTAL_TAKE_HOME_PAY=2;
    public static Rectangle PAGE_SIZE=PageSize.A4;
    public static String[][] textLang = {
        {"TOTAL PENGHASILAN", "TOTAL POTONGAN", "PENGHASILAN BERSIH"},
        {"TOTAL BENEFIT", "TOTAL DEDUCTION", "TOTAL TAKE HOME PAY"}
    };
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       // PrintWriter out = response.getWriter(); //response.getOutputStream();
        ...
        String searchNrFrom = FRMQueryString.requestString(request, "employee_num");
        String searchNrTo = FRMQueryString.requestString(request, "employee_num");
...
//        String s_level_code = null;
        ...
        HeaderFooterPrinPdf event = new HeaderFooterPrinPdf();
        Document document = new Document(PAGE_SIZE, 30, 30, 100, 80);
        ...
        for (int ip = 0; ip < listEmpPaySlip.size(); ip++) {
           ...
           /* if ((empSlip != null) && empSlip.size() > 0) {
            ...
            try {
                ...
                bankAcc = emp.getNoRekening();
                gradeId = emp.getGradeLevelId();
                ...
                case PaySlip.MODEL_UP_DOWN:
                    ...
                    try {
                        if (toEmail) {  // if email then for each payslip is different file pdf
                            document = new Document(PAGE_SIZE, 30, 30, 100, 80);
...
//  {"Seksi  "                 ,":", ""+section                          ,"Periode"                  ,":",""+periodName     },  
  ...
  {"PTKP"       ,":", ""+TaxCalculator.STATUS_EMPLOYEE_CODE[mStatus.getMaritalStatusTax()]                    ,""                          ,"",""                 }, 
  {"Nomor Karyawan"          ,":", ""+employeeNum                      ,""                          ,"",""                 }, 
  {""+divDepTitle            ,":", ""+divDepStr                        ,""                          ,"",""                 }, 
  {"Grade Karyawan"          ,":", ""+gradeCode                      ,""                          ,"",""                 }, 
...
//                            /* 2*/ {0, 0, 0, 0, 0, 0},
                        ...
                        drawListBenefit(payrollTable, startRowSlipComp, startColSlipComp, listPaySalary, employeeId, periodId, paySlipId, paidBySalary, codeUpahLembur, currencyType, tableContent,SESS_LANGUAGE);
                        drawListDeduction(payrollTable, startRowSlipComp + (listPaySalary != null ? listPaySalary.size() : 0), startColSlipComp, listPayDeduction, employeeId, periodId, paySlipId, codeTax_count, currencyType, tableContent,SESS_LANGUAGE);
                        ...
                        try {
                            Cell tCell = new Cell(new Chunk(textLang[SESS_LANGUAGE][TOTAL_TAKE_HOME_PAY], tableContent));
                        ...
                        if (SignatureOnOff > 0) {
                            ...
                            drawListSignature(payGen, payrollTable, getStartRowSlipComp() + enter + 4 + (listPaySalary != null ? listPaySalary.size() : 0) + (listPayDeduction != null ? listPayDeduction.size() : 0), 1, 2, objEmployee, tableContent, paySlip);
                        }
...
//                        document.setFooter(footer);
             ...
             /* image logo */
                    ...
                    //logo.setWidthPercentage(40);
                        ...
                        String telFax = "Telp. " + payGen.getTel() + " Fax. " + payGen.getFax();
                        String periodStr = periodName + (detailSlipYN == 1 ? " / " + Formater.formatTimeLocale(endDatePeriod, "dd-MM-yyyy") : "");
                        String divDepTitle = (detailSlipYN == 1 ? "Div. / Department" : "Satuan Kerja");
                        String divDepStr = (detailSlipYN == 1 ? ("" + division + " / " + department) : "" + division);
                        String gradeCode = grade;
                        String whereCareer = PstCareerPath.fieldNames[PstCareerPath.FLD_EMPLOYEE_ID]+"="+employeeId+" AND '"+payPeriod.getEndDate()+"' "
                                + "BETWEEN "+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_FROM]+" AND "+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_TO]
                                + " AND "+PstCareerPath.fieldNames[PstCareerPath.FLD_HISTORY_TYPE]+" = "+PstCareerPath.CAREER_TYPE;
                        ...
                        if (listCareer.size()>0){
                            for (int x=0; x < listCareer.size(); x++){
                                ...
                                try {
                                    GradeLevel gr = PstGradeLevel.fetchExc(careerPath.getGradeLevelId());
                                    gradeCode = gr.getCodeLevel();
                                } catch (Exception exc){}
                            }
                        } else {
                            try {
                                GradeLevel gr = PstGradeLevel.fetchExc(gradeId);
                                gradeCode = gr.getCodeLevel();
                            } catch (Exception exc){}
                        }
                        Calendar calComm = Calendar.getInstance();
                        calComm.setTime(commDate);
                        String commDateStr = calComm.get(Calendar.DAY_OF_MONTH)+" "+month[calComm.get(Calendar.MONTH)]+" "+calComm.get(Calendar.YEAR);//Formater.formatTimeLocale(commDate, "dd MMMM yyyy");
                        Date endDt = objPeriod.getEndDate() == null ? new Date() : objPeriod.getEndDate();
                        // end date
                        Calendar gre = Calendar.getInstance();
                        gre.setTime(endDt);
...
//  {"Periode"                 ,":", ""+periodStr                        ,""                          ,"",""                 }, 
  {" "                       ,"",  ""                                   ,""                          ,"",""                 }, 
  {"Nama Karyawan"           ,":", ""+employeeName                     ,""                          ,"",""                 }, 
  {"PTKP"       ,":", ""+TaxCalculator.STATUS_EMPLOYEE_LANG[SESS_LANGUAGE][mStatus.getMaritalStatusTax()]                    ,""                          ,"",""                 }, 
  {"Nomor Karyawan"          ,":", ""+employeeNum                      ,""                          ,"",""                 }, 
  {""+divDepTitle            ,":", ""+divDepStr                        ,""                          ,"",""                 }, 
  {"Grade Karyawan"          ,":", ""+gradeCode                      ,""                          ,"",""                 }, 
  {"Tgl. mulai kerja"        ,":", ""+commDateStr                      ,""                          ,"",""                 }, 
...
//                            /*11*/ {0, 0, 0, 0, 0, 0},
                        ...
                        Cell tCell = new Cell(new Chunk(payGen.getCompanyName().toUpperCase(), fontHeaderBig));
                        tCell.setHorizontalAlignment(Element.ALIGN_CENTER);
                        tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                        tCell.setBorderColor(new Color(255, 255, 255));
                        tCell.setColspan(6);
                        payrollTable.addCell(tCell, 0, 0);
                        
                        tCell = new Cell(new Chunk(payGen.getCompAddress()+", "+payGen.getCity(), fontHeader));
                        tCell.setHorizontalAlignment(Element.ALIGN_CENTER);
                        tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                        tCell.setBorderColor(new Color(255, 255, 255));
                        tCell.setColspan(6);
                        payrollTable.addCell(tCell, 1, 0);
                        
//                        tCell = new Cell(new Chunk(payGen.getCity(), fontHeader));
//                        tCell.setHorizontalAlignment(Element.ALIGN_CENTER);
//                        tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
//                        tCell.setBorderColor(new Color(255, 255, 255));
//                        tCell.setColspan(6);
//                        payrollTable.addCell(tCell, 2, 0);
                        
                        tCell = new Cell(new Chunk(telFax, fontHeader));
                        tCell.setHorizontalAlignment(Element.ALIGN_CENTER);
                        tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                        tCell.setBorderColor(new Color(255, 255, 255));
                        tCell.setColspan(6);
                        ...
                        tCell = new Cell(new Chunk(periodStr, fontHeader));
                        tCell.setHorizontalAlignment(Element.ALIGN_CENTER);
                        tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                        tCell.setBorderColor(new Color(255, 255, 255));
                        tCell.setColspan(6);
                        payrollTable.addCell(tCell, 3, 0);
                        
                        for (int rowIdx = 5; rowIdx < payData.length; rowIdx++) {
                            String[] rowData = payData[rowIdx];
                            for (int colIdx = 0; colIdx < rowData.length; colIdx++) {
                                String strCell = rowData[colIdx];
                                if (strCell != null && strCell.length() > 0) {
                                    tCell = new Cell(new Chunk(strCell, tableContent));
                                    tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
                                    tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                                    tCell.setBorderColor(new Color(255, 255, 255));
                                    if (colSpans[rowIdx][colIdx] > 0) {
                        ...
                        try {
                            tCell = new Cell(new Chunk(textLang[SESS_LANGUAGE][TOTAL_TAKE_HOME_PAY], tableContent));
                            tCell.setBackgroundColor(Color.LIGHT_GRAY);
                            tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
                            tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                            tCell.setBorderColor(new Color(255, 255, 255));
                        ...
                        try {
                            tCell = new Cell(new Chunk(": " + mataUang, tableContent));
                            tCell.setBackgroundColor(Color.LIGHT_GRAY);
                            tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
                            tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                            tCell.setBorderColor(new Color(255, 255, 255));
                        ...
                        try {
                            tCell = new Cell(new Chunk(Formater.formatNumberVer1(totTakHomePay, frmCurrency), tableContent));
                            tCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                            tCell.setBackgroundColor(Color.LIGHT_GRAY);
                            tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                            tCell.setBorderColor(new Color(255, 255, 255));
    ...
    public Table drawListBenefit(Table payrollTable, int startRow, int startCol, Vector objectClass, long employeeId, long periodId, long paySlipId, String paidBySalary, String codeUpahLembur, CurrencyType currencyType, Font tableContent, int SESS_LANGUAGE) {
        ...
        try {
                    Cell tCell = new Cell(new Chunk(textLang[SESS_LANGUAGE][TOTAL_BENEFIT], tableContent));
                    tCell.setBackgroundColor(Color.LIGHT_GRAY);
                    tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
                    tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                    tCell.setBorderColor(new Color(255, 255, 255));
    ...
    public String drawListDeduction(Table payrollTable, int startRow, int startCol, Vector objectClass, long employeeId, long periodId, long paySlipId, String codeTax_count, CurrencyType currencyType, Font tableContent, int SESS_LANGUAGE) {
        String output = "";
        int totalBenefit = 0;
        String garis = "";
        ...
        try {
            rows = rows + 1;
            Cell tCell = new Cell(new Chunk(textLang[SESS_LANGUAGE][TOTAL_DEDUCTION], tableContent));
            tCell.setBackgroundColor(Color.LIGHT_GRAY);
            tCell.setBorderColor(new Color(255, 255, 255));
            tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
            tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
    ...
    public String drawListSignature(PayGeneral company, Table payrollTable, int startRow, int startCol, int colSpan, Employee objEmployee, Font tableContent, PaySlip paySlip) {
        String output = "";

        int rows = startRow + 9;
        Date thisDate = new Date();
        Calendar calSlipDate = Calendar.getInstance();
        calSlipDate.setTime(paySlip.getPaySlipDate());
        SimpleDateFormat formatDate = new SimpleDateFormat("dd MMMM yyyy");
        String DateNew = calSlipDate.get(Calendar.DAY_OF_MONTH)+" "+month[calSlipDate.get(Calendar.MONTH)]+" "+calSlipDate.get(Calendar.YEAR);//formatDate.format(thisDate);
        String cityAddress="";
        ...
        */
        ...
        try {
            ...
            tCell = new Cell(new Chunk(company.getCompanyName().toUpperCase()/* +", "+level.getLevel()*/ , tableContent));
            tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
            tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            tCell.setBorderColor(new Color(255, 255, 255));
            if (colSpan > 0) {
            ...
            long divisionId = PstDivision.getDivisionIdByName(paySlip.getDivision());
            try {
            ...
            Employee emp = new Employee();
            try {
                emp = PstEmployee.fetchExc(division.getEmployeeId());
            } catch(Exception e){
                System.out.println(e.toString());
            }
			
			String oidDocGroupSignature = String.valueOf(com.dimata.system.entity.PstSystemProperty.getValueByName("OID_DOC_GROUP_SIGNATURE"));
			String whereDoc = PstEmpRelevantDoc.fieldNames[PstEmpRelevantDoc.FLD_EMP_RELVT_DOC_GRP_ID]+"="+oidDocGroupSignature
					+" AND "+PstEmpRelevantDoc.fieldNames[PstEmpRelevantDoc.FLD_EMPLOYEE_ID]+"="+division.getEmployeeId();
			Vector listDoc = PstEmpRelevantDoc.list(0, 0, whereDoc, "");
			String imageName = "";
			String imagePath = String.valueOf(com.dimata.system.entity.PstSystemProperty.getValueByName("IMGDOC"));
			if (listDoc.size()>0){
				EmpRelevantDoc empRelevantDoc = (EmpRelevantDoc) listDoc.get(0);
				String wherePages = PstEmpRelvtDocPage.fieldNames[PstEmpRelvtDocPage.FLD_DOC_RELEVANT_ID]+"="+empRelevantDoc.getOID();
				Vector listPages = PstEmpRelvtDocPage.list(0, 0, wherePages, "");
				if (listPages.size()>0){
					EmpRelvtDocPage empRelvtDocPage = (EmpRelvtDocPage) listPages.get(0);
					imageName = empRelvtDocPage.getFileName();
				}
			}
			String fullPath = "";
			if (imageName.length()>0){
                                ...
                                payrollTable.addCell(tCell, startRow++, startCol);
				String strPath = com.dimata.system.entity.PstSystemProperty.getValueByName("HARISMA_URL");
				fullPath = strPath+"/imgdoc/"+imageName;
            ...
            tCell = new Cell(new Chunk(""+emp.getFullName(), tableContent));
            tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
            tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            ...
            tCell = new Cell(new Chunk(emp.getEmployeeNum(), tableContent));
            tCell.setHorizontalAlignment(Element.ALIGN_LEFT);
            tCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            tCell.setBorderColor(new Color(255, 255, 255));
            if (colSpan > 0) {

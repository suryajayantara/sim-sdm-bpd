...
//                     
	...
	public static Vector listLLClosingLLByParameterTransferAL(SrcLeaveAppAlClosing srcLeaveAppAlClosing) {
        I_Leave leaveConfig = null;
        Vector result = new Vector();
        ...
        try {
            ...
            String sql = "SELECT LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STOCK_ID] + ",LLM."
                    ...
                    + " WHERE "
                     ...
                     //kondisi bisa diclose (mendapatkan entitle I)
                    + "( LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STATUS] + " = 0 AND"
                    + " YEAR(DATE_ADD(LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_ENTITLED_DATE] + ", INTERVAL 1 YEAR))+'-01-01' <= CURRENT_DATE "
					+ whereClause + ")"
	...
	public static void ProcessClosingLLtoAL(Vector ll_closing_selected) {
		for (int i = 0; i < ll_closing_selected.size(); i++) {
			LlClosingSelected llClosingSelected = (LlClosingSelected) ll_closing_selected.get(i);
			...
			try {
				employee = PstEmployee.fetchExc(llClosingSelected.getEmployeeId());
			} catch (Exception e) {
				System.out.println("Exception " + e.toString());
			}
			...
			String whereAL = PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_EMPLOYEE_ID]+"="+llClosingSelected.getEmployeeId()
					+ " AND "+PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_AL_STATUS]+" = "+PstAlStockManagement.AL_STS_AKTIF;
			Vector listAl = PstAlStockManagement.list(0, 0, whereAL, "");
			...
			AlStockManagement insertStock = new AlStockManagement();
			insertStock.setLeavePeriodeId(0);
			insertStock.setEmployeeId(llClosingSelected.getEmployeeId());
			insertStock.setOpening(llClosingSelected.getExtended_value());
			insertStock.setPrevBalance(llClosingSelected.getExtended_value());
			insertStock.setAlQty(entitle + llClosingSelected.getExtended_value());//update by satrya 2013-10-28 insertStock.setAlQty(entitle);
			insertStock.setQtyUsed(0);
			insertStock.setAlStatus(PstAlStockManagement.AL_STS_AKTIF);
			insertStock.setStNote("");
			insertStock.setEntitled(entitle);
			insertStock.setRecordDate(new Date());
			try {
				insertStock.setEntitleDate(new SimpleDateFormat("yyyy-MM-dd").parse(year+"-01-01"));
				insertStock.setDtOwningDate(new SimpleDateFormat("yyyy-MM-dd").parse(year+"-01-01"));
			} catch (Exception exc){
				insertStock.setEntitleDate(new Date());
				insertStock.setDtOwningDate(new Date());
			}
			...
			try {
				...
				for (int x = 0; x < listAl.size(); x++){
					AlStockManagement alStockManagement = (AlStockManagement) listAl.get(x);
					alStockManagement.setAlStatus(PstAlStockManagement.AL_STS_NOT_AKTIF);
					PstAlStockManagement.updateExc(alStockManagement);
				}
				
				LLStockManagement lLStockManagement = PstLLStockManagement.fetchExc(llClosingSelected.getLlStockId());
				lLStockManagement.setLLStatus(PstLLStockManagement.LL_STS_NOT_AKTIF);
    ...
    public static void ProcessClosingLLClose(long ll_stock_id, long employee_id, Date entitle_date) {
        ...
        llStockManagement.setQtyResidue(var_ll_entitle_I);
        ...
        //    System.out.println("EXCEPTION :::: " + e.toString());
        //}
//UPDATE `hr_al_stock_management` SET `AL_STATUS` = 1 WHERE `EMPLOYEE_ID` = 101306 AND `AL_STATUS` = 0
        try {
            ...
            System.out.println("----- UPDATE Status AL SUKSES ------------");
...
//                        + " = " + "\"" + empNumId.trim() + "\"";//penambahan trima
    ...
    public static Vector getEmployeeActiveByPeriod2(SrcLeaveAppAlClosing srcLeaveAppAlClosing) {
        ...
        long USE_CONTRACT_DATE_FOR_LEAVE = 0;
        try {
            USE_CONTRACT_DATE_FOR_LEAVE = Long.parseLong(PstSystemProperty.getValueByName("USE_CONTRACT_DATE_FOR_LEAVE"));
        } catch (Exception e) {}
        DBResultSet dbrs;
        Vector result = new Vector();
        ...
        try {
            ...
            String sql = "SELECT ALM." + PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_AL_STOCK_ID] + ",ALM."
                    ...
                    + PstEmployee.fieldNames[PstEmployee.FLD_DEPARTMENT_ID] + ",EMP."
                    + PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE] + ",EMP."
                    + PstEmployee.fieldNames[PstEmployee.FLD_FIRST_CONTRACT_DATE] + ",EMP."
                    + PstEmployee.fieldNames[PstEmployee.FLD_FULL_NAME] + ",DEPT."
                    + PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT]
                    + " FROM " + PstAlStockManagement.TBL_AL_STOCK_MANAGEMENT + " ALM "
                    + " INNER JOIN " + PstEmployee.TBL_HR_EMPLOYEE + " EMP ON ALM."
...
//                        + " = " + "\"" + empNumId.trim() + "\"";//penambahan trima
            ...
            while (rs.next()) {
                ...
                leaveAlClosingList.setQtyUsed(rs.getFloat(PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_QTY_USED]));
                leaveAlClosingList.setOpening(rs.getFloat(PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_OPENING]));
				if (rs.getFloat(PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_QTY_RESIDUE]) > 6){
					leaveAlClosingList.setQtyResidue(6);
				} else {
					leaveAlClosingList.setQtyResidue(rs.getFloat(PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_QTY_RESIDUE]));
				}
                leaveAlClosingList.setAlQty(rs.getFloat(PstAlStockManagement.fieldNames[PstAlStockManagement.FLD_AL_QTY]));
                ...
                leaveAlClosingList.setCommancingDate(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE]));
                if (USE_CONTRACT_DATE_FOR_LEAVE ==1){      
                leaveAlClosingList.setCommancingDate(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_FIRST_CONTRACT_DATE]));
                }
                leaveAlClosingList.setDepartmentId(rs.getLong(PstEmployee.fieldNames[PstEmployee.FLD_DEPARTMENT_ID]));
                leaveAlClosingList.setDepartment(rs.getString(PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT]));
                result.add(leaveAlClosingList);
...
//                        + " = " + "\"" + empNumId.trim() + "\"";//penambahan trima
            ...
            while (rs.next()) {
                ...
                if (entitled > 0) {
                    ...
                    if (USE_CONTRACT_DATE_FOR_LEAVE == 1){
                    leaveAlClosingNoStockList.setCommancingDate(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_FIRST_CONTRACT_DATE]));    
                    }
...
//update by satrya 2012-11-08
    ...
    public static void ProcessClosingCommencingPeriod2(Vector VleaveAlClosing, I_Leave leaveConfig){
        ...
        long UseContractDateForLeave = 0;
        try {
            UseContractDateForLeave = Long.parseLong(PstSystemProperty.getValueByName("USE_CONTRACT_DATE_FOR_LEAVE"));
        } catch (Exception e) {}
        ...
        if (VleaveAlClosing != null && VleaveAlClosing.size() > 0) {
            ...
            for (int i = 0; i < VleaveAlClosing.size(); i++){
                ...
                try {
                    employee = PstEmployee.fetchExc(leaveAlClosing.getEmployeeId());
                    if (UseContractDateForLeave == 1){
                    employee.setCommencingDate(employee.getFirstContractDate());    
                    }  
                } catch (Exception e) {
                    System.out.println("Exception : " + e.getMessage());
                }
                ...
                if (countData == 0){ // jika stock aktif dan tidak aktif belum ada
                  ...
                  float tmpentitle = leaveConfig.getALEntitleAnualLeaveAlaCommecingPeriode(leaveAlClosing);
                   // float tmpentitle = leaveConfig.getALEntitleAnualLeave(level.getLevel(), empCategory.getEmpCategory(), LoS, employee.getCommencingDate(),new Date());
                    ...
                    float llresidue   = PstLLStockManagement.getLlStockAmountPerEmp(employee.getOID());
                    lLStockManagement = PstLLStockManagement.getLlStockActive(employee.getOID());
                    ...
                    Date dateEntile = new Date(tmpDtClosing.getTime()); 
...
//                    }
                    ...
                    if(tmpDtClosing != null){
                        ...
                        alStockManagement.setEntitled(tmpentitle+llresidue);//entitle);
    ...
    //agus priska 20160112
     public static  Date getTmpDateEntitleCommecingPeriode(LeaveAlClosing leaveAlClosing) {
        ...
        try {
            ...
            DateiEntitleAL = new Date(DateiEntitleAL.getTime());

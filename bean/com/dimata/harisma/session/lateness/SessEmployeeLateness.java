...
public class SessEmployeeLateness {
    ...
    public static Vector listLatenessDaily(long departmentId,long companyId,long divisionId, Date selectedDate,long sectionId,String empNumb, String fullName, String lateStat, String earlyStat) 
    {
        Vector result = new Vector(1, 1);
        DBResultSet dbrs = null;
        int idxFieldName = PstEmpSchedule.getIdxNameOfTableBySelectedDate(selectedDate);
        ...
        try 
        {
            String sql = "SELECT EMP." + PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_NUM] +
                  ...
                  //update by satrya 2012-11-07
                     if((lateStat.equals("") && earlyStat.equals("")) || (lateStat.equals("1") && earlyStat.equals("1"))){
                    sql = sql +" AND ((SCH." + PstEmpSchedule.fieldNames[firstFieldIndex] +
                    " = " + PstEmpSchedule.STATUS_PRESENCE_LATE + ") OR " +
                    " (SCH." + PstEmpSchedule.fieldNames[secondFieldIndex] +
                    " = " + PstEmpSchedule.STATUS_PRESENCE_LATE + ") OR"
                                + " (SCH."+PstEmpSchedule.fieldNames[firstFieldIndex] + " = " + PstEmpSchedule.STATUS_PRESENCE_EARLY_HOME+") OR (SCH."
                                +  PstEmpSchedule.fieldNames[secondFieldIndex] + " = " +PstEmpSchedule.STATUS_PRESENCE_EARLY_HOME+"))" ;
                     } else if(lateStat.equals("1")){
                         sql = sql +" AND ((SCH." + PstEmpSchedule.fieldNames[firstFieldIndex] +
                                " = " + PstEmpSchedule.STATUS_PRESENCE_LATE + ") OR " +
                                " (SCH." + PstEmpSchedule.fieldNames[secondFieldIndex] +
                                " = " + PstEmpSchedule.STATUS_PRESENCE_LATE + "))" ;
                     } else if(earlyStat.equals("5")){
                         sql = sql +" AND ((SCH."+PstEmpSchedule.fieldNames[firstFieldIndex] + " = " + PstEmpSchedule.STATUS_PRESENCE_EARLY_HOME+") OR (SCH."
                                +  PstEmpSchedule.fieldNames[secondFieldIndex] + " = " +PstEmpSchedule.STATUS_PRESENCE_EARLY_HOME+"))" ;
                     }
...
//                        + " LIKE " + "\"%" + fullName.trim() + "%\"";//penambahan trim
    ...
    public static Vector listLatenessDaily(long departmentId,long companyId,long divisionId, Date fromDate,Date toDate,long sectionId,String empNumb, String fullName, String lateStat, String earlyStat){
        ...
        if (fromDate != null && toDate != null) {
            ...
            for (int i = 0; i <= itDate; i++) {
                ...
                Vector dLateneess = listLatenessDaily(departmentId,companyId,divisionId,selectedDate, sectionId, empNumb, fullName, lateStat, earlyStat);

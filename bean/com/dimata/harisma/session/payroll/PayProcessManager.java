...
public class PayProcessManager {
    ...
    public static PayProcessManager getInstance(long periodIdX, String levelCodeX,long oidDivisionX, long oidDepartmentX, Vector levelSelX , long payGroupIdX, long payrollGroupId, String empNumX){
        periodId=periodIdX;
        ...
        if(getProcess()==null){
            process = new PayProcess(periodId, getLevelCode(), oidDivision, oidDepartment, levelSel, payGroupId, payrollGroupId, empNum );
            getProcess().setRunning(false);
        }else {
            if(periodId!=0){            
                getProcess().setPayProcess(periodId, getLevelCode(), oidDivision, oidDepartment, levelSel, payGroupId,payrollGroupId, empNum );
            }
        }            

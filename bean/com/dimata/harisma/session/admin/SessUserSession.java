...
public class SessUserSession {
    ...
    public final static int DO_LOGIN_INVALID_CAPTCHA = 6;
    ...
    public int doLogin(String loginID, String password, HttpSession session, String kaptchaReceived) {
        String kaptchaExpected = String.valueOf(session.getValue("KAPTCHA_SESSION_KEY"));
        int useCaptcha = 0;
        try {
            useCaptcha = Integer.valueOf(PstSystemProperty.getValueByName("USE_CAPTCHA"));
        } catch (Exception exc){
            System.out.println("Exception USE_CAPTCHA : " + exc.toString());
        }
        AppUser user = null;
        if ((kaptchaReceived != null && kaptchaExpected != null && kaptchaExpected.equals(kaptchaReceived)) || useCaptcha == 0){
            user = PstAppUser.getByLoginIDAndPassword(loginID, password);
        } else {
            return DO_LOGIN_INVALID_CAPTCHA;
        }

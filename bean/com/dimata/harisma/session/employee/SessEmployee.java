...
//                vect.add(empEducation);
    ...
    return  listEmployeePayroll(idDepartment, idDivision, idSection, name, payrollNum, statusPayroll, dataStatus, null)  ; 
    }
        
    public static Vector listEmployeePayroll(long idDepartment, long idDivision, long idSection, String name, String payrollNum, int statusPayroll, int dataStatus,PayPeriod payPeriod) {
        DBResultSet dbrs = null;
        Vector result = new Vector(1, 1);
    ...
    return listEmployeePayrollBlank(idDepartment, idDivision, idSection, name, payrollNum, statusPayroll, null);
    }
    

    public static Vector listEmployeePayrollBlank(long idDepartment, long idDivision, long idSection, String name, String payrollNum, int statusPayroll, PayPeriod payPeriod) {
        DBResultSet dbrs = null;
        Vector result = new Vector(1, 1);
...
//                System.out.println("sql : " + sql);
    ...
    public static Vector listEmpPaySlipByPayslipId(Vector payslipId, long periodId, int statusData, boolean inclResign) {
        DBResultSet dbrs = null;
        Vector result = new Vector(1, 1);

        if (payslipId == null && payslipId.size() < 0 && periodId == 0) {
            return new Vector(1, 1);
        }
        try {
            /*        String sql = " SELECT distinct EMP." + PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]
      ...
      /*              + " INNER JOIN " + PstPaySlip.TBL_PAY_SLIP + " AS SLIP"                    
            ...
            //update by satrya 2013-01-14
            if (payslipId != null && payslipId.size() > 0) {
                whereClause = whereClause + " ( ";
                if ((Long) payslipId.get(0) != 0) {
                    for (int i = 0; i < payslipId.size(); i++) {
                        long lstr = (Long) payslipId.get(i);
                        whereClause = whereClause + " SLIP." + PstPaySlip.fieldNames[PstPaySlip.FLD_PAY_SLIP_ID]
                                + " = " + lstr + (i < (payslipId.size() - 1) ? " OR " : "");
                    }
                    whereClause = whereClause + " ) AND ";
                }
            }
            ...
            sql = sql + " GROUP BY SLIP." + PstPaySlip.fieldNames[PstPaySlip.FLD_PAY_SLIP_ID];
...
/**
    ...
    public static Vector employeeGapReport(String whereClause){
        Vector listData = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT EMP.*, POS.POSITION, DV.DIVISION, DV.LEVEL AS DIV_LEVEL, "
                    + "DEP.DEPARTMENT, DEP.LEVEL AS DEP_LEVEL, SEC.SECTION, SEC.LEVEL AS SEC_LEVEL, HIS.WORK_FROM "
                + " FROM hr_employee EMP "
                ...
                + " INNER JOIN hr_view_work_history_now HIS ON EMP.EMPLOYEE_ID = HIS.EMPLOYEE_ID AND WORK_HISTORY_NOW_ID = 0"
                + " WHERE EMP.RESIGNED = 0 AND "+whereClause;
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                ...
                emp.setEmployeeNum(rs.getString("EMPLOYEE_NUM"));
                emp.setPositionId(rs.getLong("POSITION_ID"));
                emp.setFullName(rs.getString("FULL_NAME"));
                emp.setLevelId(rs.getLong("LEVEL_ID"));
                list.add(emp);
                
                Division dv = new Division();
                dv.setDivision(rs.getString("DIVISION"));
                dv.setLevelDivision(rs.getInt("DIV_LEVEL"));
                list.add(dv);
                
                Department dep = new Department();
                dep.setDepartment(rs.getString("DEPARTMENT"));
                dep.setLevelDepartment(rs.getInt("DEP_LEVEL"));
                list.add(dep);
                ...
                sec.setLevelSection(rs.getInt("SEC_LEVEL"));
                list.add(sec);
                
                CareerPath his = new CareerPath();
                his.setWorkFrom(rs.getDate("WORK_FROM"));
                list.add(his);
                listData.add(list);
            }
            rs.close();

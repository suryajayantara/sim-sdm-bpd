...
//          }
            ...
            // parameter : COMPETENCY = 24
            //String[] competencyId = (String[]) vparam.get(9);
             String[] competencyId = searchSpecialQuery.getArrCompetencyId(0);
//          if(languageId!=null){
//            if (!(languageId[0].equals("0"))) {
//                sql +=   ", LANG."+PstCompetency.fieldNames[PstCompetency.FLD_COMPETENCY_NAME];
//            }
//          }
...
if(competencyId!=null){
 if (!(competencyId[0].equals("0"))) {
     sql += " LEFT JOIN "+PstEmployeeCompetency.TBL_EMPLOYEE_COMPETENCY + " AS EMPCMP ON EMP."+PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]+"=EMPCMP."+PstEmployeeCompetency.fieldNames[PstEmployeeCompetency.FLD_EMPLOYEE_ID] ;//  ", "+PstCompetency.TBL_HR_COMPETENCTY + " CMP ";
     //sql +=   ", "+PstEmpLanguage.TBL_HR_EMP_LANGUAGE + " EMPLANG ";
 }
}          
                 sql +=   " WHERE ";
...
//            }
            ...
            Vector vectName = logicParser(searchSpecialQuery.getEmployeeId());
            if (vectName != null && vectName.size() > 0) {
                whereClause = whereClause + "AND (";
                for (int i = 0; i < vectName.size(); i++) {
                    String str = (String) vectName.get(i);
          ...
          if(searchSpecialQuery.getArrBlood(0)!=null){
            ...
            // parameter : COMPETENCY = 24
            if (competencyId!=null && !(competencyId[0].equals("0"))) {
                whereClause += " AND (";
                for (int i=0; i<competencyId.length; i++) {
                    whereClause = whereClause + " EMPCMP."+PstCompetency.fieldNames[PstCompetency.FLD_COMPETENCY_ID]+
                                " = "+ competencyId[i] + " OR ";
                    if (i==(competencyId.length-1)) {
                        whereClause = whereClause.substring(0, whereClause.length()-4);
                    }
                }
                whereClause += " )";
...
if(competencyId!=null){
 if (!(competencyId[0].equals("0"))) {
     sql += " LEFT JOIN "+PstEmployeeCompetency.TBL_EMPLOYEE_COMPETENCY + " AS EMPCMP ON EMP."+PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]+"=EMPCMP."+PstEmployeeCompetency.fieldNames[PstEmployeeCompetency.FLD_EMPLOYEE_ID] ;//  ", "+PstLanguage.TBL_HR_LANGUAGE + " LANG ";
     //sql +=   ", "+PstEmpLanguage.TBL_HR_EMP_LANGUAGE + " EMPLANG ";
 }
}
                 sql +=   " WHERE ";

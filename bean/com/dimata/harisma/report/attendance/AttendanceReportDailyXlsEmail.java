...
import com.dimata.harisma.session.attendance.SessPresenceReportDaily;
 ...
 */
public class AttendanceReportDailyXlsEmail extends HttpServlet{
    ...
    */
    ...
    if ( (selectedDateFrom == null) && (selectedDateTo == null) && (empNum.equals("") || empNum == null ) && (fullName.equals("") || fullName ==null ) && (reason_sts == 0) && (oidDepartment == 0) && (oidCompany == 0) && (oidDivision == 0) && (oidSection == 0 ) && (date == null) && (sStatusResign == "" ) ){
        try{
            SessPresenceReportDaily sessPresenceReportDaily = new SessPresenceReportDaily();
            sessPresenceReportDaily = (SessPresenceReportDaily)session.getValue("sessPresenceReportDaily");
            selectedDateFrom = sessPresenceReportDaily.getSelectedDateFrom();
            selectedDateTo = sessPresenceReportDaily.getSelectedDateTo();
            empNum = sessPresenceReportDaily.getEmpNum();
            fullName = sessPresenceReportDaily.getEmpNum();
            reason_sts = sessPresenceReportDaily.getReason_sts();       
            oidDepartment = sessPresenceReportDaily.getOidDepartment();        
            oidSection = sessPresenceReportDaily.getOidSection();
            oidCompany = sessPresenceReportDaily.getOidCompany();
            oidDivision = sessPresenceReportDaily.getOidDivision();
            date = sessPresenceReportDaily.getDate();
            sStatusResign = sessPresenceReportDaily.getsStatusResign(); 
        }catch(Exception e) {}
    }

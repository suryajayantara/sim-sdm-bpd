...
public class PstEmployee extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    /* Update by Hendra Put | 2016-06-08 */
    public static final int FLD_SK_NOMOR = 95;
    public static final int FLD_SK_TANGGAL = 96;
    public static final int FLD_NKK = 97;
    /* Update by Hendra Putu | 2016-06-15 */
    public static final int FLD_SK_NOMOR_GRADE = 98;
    public static final int FLD_SK_TANGGAL_GRADE = 99;
    public static final int FLD_NAME_ON_CARD = 100;
    public static final int FLD_NATIONALITY_ID = 101;
    public static final int FLD_FIRST_CONTRACT_DATE = 102;
    public static final int FLD_BPJS_TK_PENSIUN = 103;
    public static final int FLD_BPJS_TK_PENSIUN_DATE = 104;
    ...
    public static final String[] fieldNames = {
        ...
        "HISTORY_TYPE",
        "EMP_DOC_ID_GRADE",
        /* Update by Hendra Putu | 2016-06-08 */
        "SK_NOMOR",
        "SK_TANGGAL",
        "NKK",
        "SK_NOMOR_GRADE",
    ...
    public static Employee fetchExc(long oid) throws DBException {
        try {
            ...
            /* Update By Hendra Putu | 2016-06-08 */
            employee.setSkNomor(pstEmployee.getString(FLD_SK_NOMOR));
            employee.setSkTanggal(pstEmployee.getDate(FLD_SK_TANGGAL));
            employee.setNkk(pstEmployee.getString(FLD_NKK));
            employee.setSkNomorGrade(pstEmployee.getString(FLD_SK_NOMOR_GRADE));
            employee.setSkTanggalGrade(pstEmployee.getDate(FLD_SK_TANGGAL_GRADE));
            employee.setNameCard(pstEmployee.getString(FLD_NAME_ON_CARD));
            employee.setNationalityId(pstEmployee.getLong(FLD_NATIONALITY_ID));
            employee.setFirstContractDate(pstEmployee.getDate(FLD_FIRST_CONTRACT_DATE));
            employee.setBpjsTkPensiun(pstEmployee.getLong(FLD_BPJS_TK_PENSIUN));
            employee.setBpjsTkPensiunDate(pstEmployee.getDate(FLD_BPJS_TK_PENSIUN_DATE));
    ...
    public static long insertExc(Employee employee) throws DBException {
        try {
            ...
            /* Update By Hendra Putu | 2016-06-08 */
            pstEmployee.setString(FLD_SK_NOMOR, employee.getSkNomor());
            pstEmployee.setDate(FLD_SK_TANGGAL, employee.getSkTanggal()); 
            pstEmployee.setString(FLD_NKK, employee.getNkk());
            pstEmployee.setString(FLD_SK_NOMOR_GRADE, employee.getSkNomorGrade());
            pstEmployee.setDate(FLD_SK_TANGGAL_GRADE, employee.getSkTanggalGrade());
            pstEmployee.setString(FLD_NAME_ON_CARD, employee.getNameCard());
            pstEmployee.setLong(FLD_NATIONALITY_ID, employee.getNationalityId());
            pstEmployee.setDate(FLD_FIRST_CONTRACT_DATE, employee.getFirstContractDate());
            pstEmployee.setLong(FLD_BPJS_TK_PENSIUN, employee.getBpjsTkPensiun());
            pstEmployee.setDate(FLD_BPJS_TK_PENSIUN_DATE, employee.getBpjsTkPensiunDate());
    ...
    public static long insertExcPending(Employee employee) throws DBException {
        ...
        try {
            ...
            query = pstEmployee.getInsertQuery();
            employee.setOID(pstEmployee.getlong(FLD_EMPLOYEE_ID));
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
    ...
    public static long updateExcPending(Employee employee) throws DBException {
        try {
            if (employee.getOID() != 0) {
                PstEmployee pstEmployee = new PstEmployee(employee.getOID());
               ...
               if (autoPinOk ==1){
                ...
                pstEmployee.setLong(FLD_BPJS_TK_PENSIUN, employee.getBpjsTkPensiun());
                pstEmployee.setDate(FLD_BPJS_TK_PENSIUN_DATE, employee.getBpjsTkPensiunDate());
                query = pstEmployee.getUpdateSQL();
                return employee.getOID();
    ...
    public static long deleteExcPending(long oid) throws DBException {
        try {
            PstEmployee pstEmployee = new PstEmployee(oid);
            query = pstEmployee.getDeleteSQL();
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
            throw new DBException(new PstEmployee(0), DBException.UNKNOWN);
	...
	public static Vector listJoinSlip(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT emp.* FROM " + TBL_HR_EMPLOYEE+" emp "
					+ " INNER JOIN "+PstPaySlip.TBL_PAY_SLIP+" slip "
					+ " ON emp."+PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]
					+ " = slip."+PstPaySlip.fieldNames[PstPaySlip.FLD_EMPLOYEE_ID]
					+ " INNER JOIN "+PstDivision.TBL_HR_DIVISION+" dv "
					+ " ON slip."+PstPaySlip.fieldNames[PstPaySlip.FLD_DIVISION]
    ...
    public static Vector listEmpInPosition(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = " SELECT * FROM " + TBL_HR_EMPLOYEE + " AS he "+
                         " INNER JOIN "+PstPosition.TBL_HR_POSITION+" AS hp ON hp."+PstPosition.fieldNames[PstPosition.FLD_POSITION_ID]+" = he."+PstEmployee.fieldNames[PstEmployee.FLD_POSITION_ID]+" ";
     ...
     * @return 
     */
     public static Hashtable hashListEmployeeFull(int limitStart, int recordToGet, String whereClause) {
        DBResultSet dbrs = null;
        Hashtable hashListEmployee= new Hashtable();
        try {
            String sql = "SELECT * FROM " + TBL_HR_EMPLOYEE;
            if (whereClause != null && whereClause.length() > 0) {
                ...
                sql = sql + " ORDER BY " + fieldNames[FLD_DIVISION_ID]+","+fieldNames[FLD_EMPLOYEE_NUM]+" ASC ";
            if (limitStart == 0 && recordToGet == 0) {
                sql = sql + "";
            } else {
                sql = sql + " LIMIT " + limitStart + "," + recordToGet;
            ...
            while (rs.next()) {
               ...
               Employee employee = new Employee();
                resultToObject(rs, employee);
                hashListEmployee.put(employee.getOID(), employee);
            }
            rs.close();
        } catch (Exception e) {
            System.out.println("Error pst EMployee add employee"+e);
    ...
    public static void resultToObject(ResultSet rs, Employee employee) {
        try {
            ...
            /* Update By Hendra Putu | 2016-06-08 */
            employee.setSkNomor(rs.getString(fieldNames[FLD_SK_NOMOR]));
            employee.setSkTanggal(rs.getDate(fieldNames[FLD_SK_TANGGAL]));
            employee.setNkk(rs.getString(fieldNames[FLD_NKK]));
            employee.setSkNomorGrade(rs.getString(fieldNames[FLD_SK_NOMOR_GRADE]));
            employee.setSkTanggalGrade(rs.getDate(fieldNames[FLD_SK_TANGGAL_GRADE]));
            employee.setNameCard(rs.getString(fieldNames[FLD_NAME_ON_CARD]));
            employee.setNationalityId(rs.getLong(fieldNames[FLD_NATIONALITY_ID]));
            employee.setFirstContractDate(rs.getDate(fieldNames[FLD_FIRST_CONTRACT_DATE]));
            employee.setBpjsTkPensiun(rs.getLong(fieldNames[FLD_BPJS_TK_PENSIUN]));
            employee.setFirstContractDate(rs.getDate(fieldNames[FLD_BPJS_TK_PENSIUN_DATE]));
        } catch (Exception e) {
        }
    }
    ...
    public static Vector listBenefitDeduction(int limitStart, int recordToGet, String whereClauseEmp, String order, long periodid, long comtype, Vector PayCOmp, ListBenefitDeduction listBenefitDeduction,String employee_id) {
        ...
        for (int j = 0; j < PayCOmp.size(); j++){
                 ...
                 sql= sql + " SELECT DISTINCT p." + PstPayPeriod.fieldNames[PstPayPeriod.FLD_PERIOD] +
                                          ...
                                          " , e." + PstEmployee.fieldNames[PstEmployee.FLD_RELIGION_ID] +
...
//            }
    ...
    public static String getDivisionName(long divisionId){
        String str = "";
        try {
            Division div = PstDivision.fetchExc(divisionId);
            str = div.getDivision();
        } catch(Exception e){
            System.out.println(e.toString());
        }
    ...
    public static String getLevelName(long levelId){
        String str = "";
        try {
    ...
    public static String getGradeName(long gradeLevelId){
        String str = "";
        try {
            GradeLevel grade = PstGradeLevel.fetchExc(gradeLevelId);
    ...
    public static int updateTax(long maritalId, long oid){
        int status = 0;
        try {
            String query = "";
            query  = " UPDATE "+TBL_HR_EMPLOYEE;
            query += " SET "+fieldNames[FLD_TAX_MARITAL_ID]+"="+maritalId;
            query += " WHERE "+fieldNames[FLD_EMPLOYEE_ID]+"="+oid;
            status = DBHandler.updateParsial(query);
        } catch(Exception e){
            System.out.println("update taxMarital di=>"+e.toString());
        }
        return status;
    }
    
    public static Vector listEmployeeALEligible(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = " SELECT EMP.* FROM " + TBL_HR_EMPLOYEE + " as EMP"
                    + " INNER JOIN " + PstAlStockManagement.TBL_AL_STOCK_MANAGEMENT + " as AL"
                    + " on EMP." + PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID] + "="
            ...
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                Employee entEmployee = new Employee();
                resultToObject(rs, entEmployee);
                lists.add(entEmployee);
            }
            rs.close();
            return lists;
        } catch (Exception e) {
    ...
    public static Vector listEmployeeLLEligible(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = " SELECT EMP.* FROM " + TBL_HR_EMPLOYEE + " as EMP"
                    + " INNER JOIN " + PstLLStockManagement.TBL_LL_STOCK_MANAGEMENT + " as LL"
                    + " on EMP." + PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID] + "="
    ...
    public static int getEmployeeAge(long employeeId){
        int age = 0;
        ...
        try {
            ...
            birthDate.setTime(emp.getBirthDate());
            if (birthDate.after(dtNow)) {
                return 0;
            }
            
            int nowYear = dtNow.get(Calendar.YEAR);
            int birthDateYear = birthDate.get(Calendar.YEAR);
            int nowDayOfYear = dtNow.get(Calendar.DAY_OF_YEAR);
            int birthDateDayOfYear = birthDate.get(Calendar.DAY_OF_YEAR);
            int nowMonth = dtNow.get(Calendar.MONTH);
            int birthDateMonth = birthDate.get(Calendar.MONTH);
            int nowDayOfMonth = dtNow.get(Calendar.DAY_OF_MONTH);
            int birthDateDayOfMonth = birthDate.get(Calendar.DAY_OF_MONTH);
            
            age = nowYear - birthDateYear;
            // If birth date is greater than todays date (after 2 days adjustment of leap year) then decrement age one year
            if ((birthDateDayOfYear - nowDayOfYear > 3) || (birthDateMonth > nowMonth)){
                age--;

            // If birth date and todays date are of same month and birth day of month is greater than todays day of month then decrement age
            } else if ((birthDateMonth == nowMonth) && (birthDateDayOfMonth > nowDayOfMonth)){
                age--;
            }
    ...
    public static int getYearDiffPerDate(long employeeId, Date dateCheck, Date dateFrom) {
        int age = 0;
        ...
        try {
            ...
            Calendar birthDate = Calendar.getInstance();
            birthDate.setTime(dateFrom);
            if (birthDate.after(check)) {
                return 0;
            }

            int nowYear = check.get(Calendar.YEAR);
            int birthDateYear = birthDate.get(Calendar.YEAR);
            int nowDayOfYear = check.get(Calendar.DAY_OF_YEAR);
            int birthDateDayOfYear = birthDate.get(Calendar.DAY_OF_YEAR);
            int nowMonth = check.get(Calendar.MONTH);
            int birthDateMonth = birthDate.get(Calendar.MONTH);
            int nowDayOfMonth = check.get(Calendar.DAY_OF_MONTH);
            int birthDateDayOfMonth = birthDate.get(Calendar.DAY_OF_MONTH);
	...
	public static Vector getEmployeeLLEligibleByYear(int year, String where){
		Vector lists = new Vector();
		...
		int[] intervalLL = leaveConfig.getIntervalLLinMonths();
		
		DBResultSet dbrs = null;
        try {
            String sql1 = "SELECT emp.EMPLOYEE_NUM,emp.FULL_NAME,dv.DIVISION,dep.DEPARTMENT,pos.POSITION,YEAR(emp.COMMENCING_DATE) AS COMMENCING_DATE,"
				+ "'-' AS LAST_GET,((TIMESTAMPDIFF(YEAR,emp.COMMENCING_DATE, '"+year+"-12-31'))) AS MASA_KERJA FROM hr_employee AS emp "
				+ " INNER JOIN hr_division AS dv ON emp.DIVISION_ID = dv.DIVISION_ID "
				+ " INNER JOIN hr_department AS dep ON emp.DEPARTMENT_ID = dep.DEPARTMENT_ID "
				+ " INNER JOIN hr_position AS pos ON emp.POSITION_ID = pos.POSITION_ID "
				+ " WHERE YEAR(COMMENCING_DATE) = YEAR(DATE_ADD('"+year+"-12-31', INTERVAL - "+intervalLL[0]+" MONTH))";
				if (where.length()>0){
					sql1 += " AND "+where;
				}
		
			String sql2 = "SELECT emp.EMPLOYEE_NUM,emp.FULL_NAME,dv.DIVISION,dep.DEPARTMENT,pos.POSITION,YEAR(emp.COMMENCING_DATE) AS COMMENCING_DATE,"
					+ "YEAR(DATE_ADD('"+year+"-12-31', INTERVAL - "+intervalLL[1]+" MONTH)) AS LAST_GET,((TIMESTAMPDIFF(YEAR,emp.COMMENCING_DATE, '"+year+"-12-31'))) AS MASA_KERJA FROM hr_employee AS emp "
					+ " INNER JOIN hr_division AS dv ON emp.DIVISION_ID = dv.DIVISION_ID "
					+ " INNER JOIN hr_department AS dep ON emp.DEPARTMENT_ID = dep.DEPARTMENT_ID "
					+ " INNER JOIN hr_position AS pos ON emp.POSITION_ID = pos.POSITION_ID "
					+ " WHERE ("; 

			int interval = intervalLL[0] + intervalLL[1];
			for (int i=0; i < 10; i++){
				if (i == 9){
					sql2 += " YEAR(COMMENCING_DATE) = YEAR(DATE_ADD('"+year+"-12-31', INTERVAL - "+interval+" MONTH)) ";
				} else {
					sql2 += " YEAR(COMMENCING_DATE) = YEAR(DATE_ADD('"+year+"-12-31', INTERVAL - "+interval+" MONTH)) OR ";
				}
				interval = interval + intervalLL[1];
			}
			...
			String sql = sql1 + " UNION " + sql2 +" ORDER BY COMMENCING_DATE, DIVISION";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {

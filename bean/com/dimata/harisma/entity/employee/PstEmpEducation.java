...
public class PstEmpEducation extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static long insertExcPending(EmpEducation empeducation) throws DBException {
        try {
            PstEmpEducation pstEmpEducation = new PstEmpEducation(0);
            ...
            query = pstEmpEducation.getInsertQuery();
            empeducation.setOID(pstEmpEducation.getlong(FLD_EMP_EDUCATION_ID));
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
    ...
    public static long updateExcPending(EmpEducation empeducation) throws DBException {
        try {
            if (empeducation.getOID() != 0) {
                PstEmpEducation pstEmpEducation = new PstEmpEducation(empeducation.getOID());
                ...
                query = pstEmpEducation.getUpdateSQL();
                return empeducation.getOID();
    ...
    public static long deleteExcPending(long oid) throws DBException {
        try {
            PstEmpEducation pstEmpEducation = new PstEmpEducation(oid);
            query = pstEmpEducation.getDeleteSQL();
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
            throw new DBException(new PstEmpEducation(0), DBException.UNKNOWN);
    ...
    public static Vector listDistinct(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "+fieldNames[FLD_EMPLOYEE_ID]+" FROM " + TBL_HR_EMP_EDUCATION;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
            ...
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                long employeeId = rs.getLong(PstEmpEducation.fieldNames[PstEmpEducation.FLD_EMPLOYEE_ID]);
    ...
    public static Hashtable<String, EmpEducation> listEmpEducation(int year){
        Hashtable<String, EmpEducation> lists = new Hashtable();
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT * FROM " +
                         "(SELECT EMPLOYEE_ID, MAX(END_DATE) AS END_DATE FROM hr_emp_education WHERE END_DATE <= "+year+" GROUP BY employee_id)" +
                         " AS X JOIN hr_emp_education USING (EMPLOYEE_ID, END_DATE) ";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            
            while (rs.next()) {
                EmpEducation empEdu = new EmpEducation();
                resultToObject(rs, empEdu);
                 lists.put(""+empEdu.getEmployeeId(), empEdu);
            }
            rs.close();
            return lists;
        } catch (Exception e) {
    ...
    public static EmpEducation getLastEmpEducation(long employeeId, int year){
        EmpEducation empEducation = new EmpEducation();
        
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT * FROM hr_emp_education WHERE employee_id ="+employeeId+
                        " AND end_date <="+year+" ORDER BY END_DATE DESC LIMIT 1";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();

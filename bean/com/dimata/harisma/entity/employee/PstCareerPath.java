...
public class PstCareerPath extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
	...
	public static final String TBL_HR_VIEW_WORK_HISTORY_NOW = "hr_view_work_history_now";//"HR_WORK_HISTORY_NOW";
	...
	public static int OJT_TYPE = 4;
    public static String[] historyType = {"Career", "Pejabat Sementara", "Pelaksana Tugas", "Detasir", "OJT"};
    ...
    public static long insertExcPending(CareerPath careerpath) throws DBException {
        try {
            PstCareerPath pstCareerPath = new PstCareerPath(0);
            ...
            query = pstCareerPath.getInsertQuery();
            careerpath.setOID(pstCareerPath.getlong(FLD_WORK_HISTORY_NOW_ID));
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
    ...
    public static long updateExcPending(CareerPath careerpath) throws DBException {
        try {
            if (careerpath.getOID() != 0) {
                PstCareerPath pstCareerPath = new PstCareerPath(careerpath.getOID());
                ...
                query = pstCareerPath.getUpdateSQL();
                return careerpath.getOID();
    ...
    public static long deleteExcPending(long oid) throws DBException {
        try {
            PstCareerPath pstCareerPath = new PstCareerPath(oid);
            query = pstCareerPath.getDeleteSQL();
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
            throw new DBException(new PstCareerPath(0), DBException.UNKNOWN);
	...
	public static Vector listView(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_HR_VIEW_WORK_HISTORY_NOW;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
    ...
    public static Vector listEmployeePECareerPath(String periodStart, String periodEnd) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql1 = " SELECT CP.*, POS.* FROM " + PstCareerPath.TBL_HR_WORK_HISTORY_NOW + " CP"
                    ...
                    + " POS." + PstPosition.fieldNames[PstPosition.FLD_JENIS_JABATAN] + "= "+PstPosition.JENIS_PEJABAT_EKSEKUTIF
                    + " AND ((CP." + PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_FROM] + " BETWEEN '"
                    + periodStart +  "' AND '"  + periodEnd + "'"
                    + " AND CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_HISTORY_GROUP]+" = "+PstCareerPath.RIWAYAT_JABATAN
                    + " AND CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_HISTORY_TYPE]+" IN("+PstCareerPath.CAREER_TYPE+","+PstCareerPath.PEJABAT_SEMENTARA_TYPE+"))"
                    + " OR (((CP." + PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_FROM] + " BETWEEN '"
                    + periodStart +  "' AND '"  + periodEnd + "') OR (CP." + PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_TO] + " + INTERVAL 1 DAY BETWEEN '"
                    + periodStart + "' AND '" + periodEnd + "' )) AND "
                    + "CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_HISTORY_TYPE]+" = "+PstCareerPath.DETASIR_TYPE+" AND "
                    + " (DATEDIFF(CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_TO]+", CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_FROM]+")/30 "
                    + " > 5)))";
            
            
            String dtWorkFrom = "IF((((SELECT MAX(`hr_work_history_now`.`WORK_TO`) FROM `hr_work_history_now` "
                            +" WHERE ((`hr_work_history_now`.`EMPLOYEE_ID` = `emp`.`EMPLOYEE_ID`)"
                            +" AND (`hr_work_history_now`.`HISTORY_GROUP` <> 1) "
                            +" AND (`hr_work_history_now`.`HISTORY_TYPE` IN (0, 1)))) + INTERVAL 1 DAY) IS NOT NULL),"
                            +" ((SELECT MAX(`hr_work_history_now`.`WORK_TO`) FROM `hr_work_history_now` "
                            +" WHERE ((`hr_work_history_now`.`EMPLOYEE_ID` = `emp`.`EMPLOYEE_ID`) "
                            +" AND (`hr_work_history_now`.`HISTORY_GROUP` <> 1) "
                            +" AND (`hr_work_history_now`.`HISTORY_TYPE` IN (0, 1)))) + INTERVAL 1 DAY),"
                            +"`emp`.`COMMENCING_DATE`)";
            
            String sqlJenisJabatan = "SELECT `POS`.`JENIS_JABATAN` FROM `hr_work_history_now` AS CP "
                                + " INNER JOIN hr_position AS POS ON CP.POSITION_ID = POS.POSITION_ID "
                                +" WHERE ((`CP`.`EMPLOYEE_ID` = emp.`EMPLOYEE_ID`) "
                                +" AND (`CP`.`HISTORY_GROUP` <> 1) AND (`CP`.`HISTORY_TYPE` IN (0, 1)) "
                                +" AND CP.WORK_TO < '"+periodEnd+"') ORDER BY CP.WORK_FROM DESC LIMIT 1";
            ...
            String sql2= "SELECT "
                    ...
                    + "'' AS SECTION, "
                    + dtWorkFrom + "AS WORK_FROM, "
                    + "CURDATE() AS `WORK_TO`, "
                    + "'0.00' AS SALARY, "
                    + "`emp`.`SK_NOMOR` AS `DESCRIPTION`, "
                    ...
                    + "NULL AS CONTRACT_FROM, "
                    + "NULL AS CONTRACT_TO, "
                    + "POS.* "
                    + "FROM `hr_employee` `emp` INNER JOIN hr_position AS POS ON emp.position_id = POS.position_id "
                    + "WHERE (("+dtWorkFrom+" BETWEEN '"+periodStart + "' AND '" + periodEnd + "' AND "
                    + "("+sqlJenisJabatan+") = "+PstPosition.JENIS_PEJABAT_EKSEKUTIF+"))";
            ...
            while (rs.next()) {
               CareerPath careerpath = new CareerPath();
                resultToObject(rs, careerpath);
                int workTo = getConvertDateToInt(""+careerpath.getWorkTo());
                int intPeriodStart = getConvertDateToInt(periodStart);
                int intPeriodEnd = getConvertDateToInt(periodEnd);
                if (workTo >= intPeriodStart && workTo <= intPeriodEnd ){
                    /* Cek Career Selanjutnya */
                }
                lists.add(careerpath);
            }
            rs.close();
    ...
    public static Vector listKadivCareer(long employeeId, String workTo) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = " SELECT CP.*, POS." + PstPosition.fieldNames[PstPosition.FLD_JENIS_JABATAN] + " FROM " + PstCareerPath.TBL_HR_WORK_HISTORY_NOW + " CP"
                    + " INNER JOIN " + PstPosition.TBL_HR_POSITION + " POS"
                    + " ON CP." + PstCareerPath.fieldNames[PstCareerPath.FLD_POSITION_ID] + " ="
                    + " POS." + PstPosition.fieldNames[PstPosition.FLD_POSITION_ID] + " WHERE"
                    + " POS." + PstPosition.fieldNames[PstPosition.FLD_JENIS_JABATAN] + " = "+PstPosition.JENIS_PEJABAT_EKSEKUTIF
                    ...
                    + employeeId
                    + " AND CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_TO] + "<= '"+workTo+"'"
                    + " AND CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_HISTORY_GROUP] + " = "+PstCareerPath.RIWAYAT_JABATAN;
    ...
    public static Vector lastCareerPE(long employeeId, String workTo) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = " SELECT CP.*, POS." + PstPosition.fieldNames[PstPosition.FLD_JENIS_JABATAN] + " FROM " + PstCareerPath.TBL_HR_WORK_HISTORY_NOW + " CP"
                    ...
                    + employeeId
                    + " AND CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_TO] + "<= '"+workTo+"'"
                    + " AND CP."+ PstCareerPath.fieldNames[PstCareerPath.FLD_HISTORY_GROUP]+" IN ("+PstCareerPath.RIWAYAT_JABATAN+","+PstCareerPath.RIWAYAT_CAREER_N_GRADE+","
					+ PstCareerPath.OJT_TYPE+") "
                    + " AND CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_HISTORY_TYPE]+" IN("+PstCareerPath.CAREER_TYPE+","+PstCareerPath.PEJABAT_SEMENTARA_TYPE+")"
                    + " ORDER BY CP."+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_FROM]+" DESC LIMIT 1";
	...
	private static void resultToObjectView(ResultSet rs, CareerPath careerpath) {
        try {
            careerpath.setOID(rs.getLong(PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_HISTORY_NOW_ID]));
            careerpath.setEmployeeId(rs.getLong(PstCareerPath.fieldNames[PstCareerPath.FLD_EMPLOYEE_ID]));
            careerpath.setCompanyId(rs.getLong(PstCareerPath.fieldNames[PstCareerPath.FLD_COMPANY_ID]));
    ...
    public static int getCountView(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(" + PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_HISTORY_NOW_ID] + ") FROM " + TBL_HR_VIEW_WORK_HISTORY_NOW;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
    ...
    public static int getCountPJS(int year, String inDivision) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM(CNT) FROM (SELECT COUNT(DISTINCT t.EMPLOYEE_ID) AS CNT FROM "
                    + "((SELECT vw.* FROM `hr_view_work_history_now` vw "
                    + "WHERE ((`HISTORY_GROUP` = 0 AND `HISTORY_TYPE` IN (0, 1)) OR `WORK_HISTORY_NOW_ID` = 0) AND POSITION_ID IN (SELECT POSITION_ID FROM `hr_employee` WHERE EMPLOYEE_ID = vw.`EMPLOYEE_ID`) AND DATEDIFF(WORK_FROM,(SELECT v2.WORK_TO FROM `hr_view_work_history_now` v2 WHERE v2.EMPLOYEE_ID = vw.EMPLOYEE_ID AND v2.WORK_TO < vw.WORK_TO AND ((`HISTORY_GROUP` = 0 AND `HISTORY_TYPE` IN (0, 1)) OR `WORK_HISTORY_NOW_ID` = 0) ORDER BY v2.WORK_TO DESC LIMIT 1) ) = 1) "
                    + "UNION (SELECT * FROM `hr_view_work_history_now` WHERE ((`HISTORY_GROUP` = 0 AND `HISTORY_TYPE` IN (0, 1)) OR `WORK_HISTORY_NOW_ID` = 0) AND POSITION_ID IN (SELECT POSITION_ID FROM `hr_employee` WHERE EMPLOYEE_ID = hr_view_work_history_now.`EMPLOYEE_ID`)"
                    + " ORDER BY work_from LIMIT 1)) AS t ";
            if (inDivision.length()>0){
                sql+=" INNER JOIN hr_employee AS emp ON t.EMPLOYEE_ID = emp.`EMPLOYEE_ID` WHERE emp.`DIVISION_ID` IN ("+inDivision+")";
            }
            sql += "GROUP BY t.EMPLOYEE_ID HAVING TIMESTAMPDIFF(YEAR,MIN(t.WORK_FROM),MAX(t.WORK_TO)) >= "+year+") AS DATA";
    ...
    public static int getCountPJSV2(int year, String inDivision) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(dt.EMPLOYEE_ID) FROM (SELECT t.*, SUM(DTDIFF) / 365 AS YEAR "
                    + " FROM (SELECT vw.*, (DATEDIFF(vw.`WORK_TO`, vw.`WORK_FROM`)+1) AS DTDIFF FROM `hr_view_work_history_now` vw "
                    + " INNER JOIN hr_employee emp ON vw.`EMPLOYEE_ID` = emp.`EMPLOYEE_ID` and vw.`DIVISION_ID` = emp.`DIVISION_ID` "
                    + " and vw.`DEPARTMENT_ID` = emp.`DEPARTMENT_ID` AND vw.`SECTION_ID` = emp.`SECTION_ID` AND vw.`POSITION_ID` = emp.`POSITION_ID` AND emp.resigned = 0 "
                    + " WHERE (DATEDIFF(WORK_FROM,(SELECT v2.WORK_TO FROM `hr_view_work_history_now` v2 WHERE v2.EMPLOYEE_ID = vw.`EMPLOYEE_ID` "
                    + " AND v2.WORK_TO < vw.`WORK_FROM` ORDER BY v2.WORK_TO DESC LIMIT 1) ) = 1 OR "
                    + " DATEDIFF(`WORK_TO`,(SELECT v2.WORK_FROM FROM `hr_view_work_history_now` v2 WHERE v2.EMPLOYEE_ID = vw.`EMPLOYEE_ID` "
                    + " AND v2.WORK_FROM > vw.`WORK_TO` ORDER BY v2.WORK_TO LIMIT 1) )) order by vw.`WORK_FROM` ) as t";
            if (inDivision.length()>0){
                sql+=" INNER JOIN hr_employee AS emp ON t.EMPLOYEE_ID = emp.`EMPLOYEE_ID` WHERE emp.`DIVISION_ID` IN ("+inDivision+")";
            }
            sql += "GROUP BY t.EMPLOYEE_ID HAVING YEAR >= "+year+") AS dt";
    ...
    public static Vector getlistPJS(int year, String inDivision) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DATA.* FROM (SELECT t.* FROM "
                    + "((SELECT vw.* FROM `hr_view_work_history_now` vw "
                    + "WHERE ((`HISTORY_GROUP` = 0 AND `HISTORY_TYPE` IN (0, 1)) OR `WORK_HISTORY_NOW_ID` = 0) AND POSITION_ID IN (SELECT POSITION_ID FROM `hr_employee` WHERE EMPLOYEE_ID = vw.`EMPLOYEE_ID`) AND DATEDIFF(WORK_FROM,(SELECT v2.WORK_TO FROM `hr_view_work_history_now` v2 WHERE v2.EMPLOYEE_ID = vw.EMPLOYEE_ID AND v2.WORK_TO < vw.WORK_TO AND ((`HISTORY_GROUP` = 0 AND `HISTORY_TYPE` IN (0, 1)) OR `WORK_HISTORY_NOW_ID` = 0) ORDER BY v2.WORK_TO DESC LIMIT 1) ) = 1) "
                    + "UNION (SELECT * FROM `hr_view_work_history_now` WHERE ((`HISTORY_GROUP` = 0 AND `HISTORY_TYPE` IN (0, 1)) OR `WORK_HISTORY_NOW_ID` = 0) AND POSITION_ID IN (SELECT POSITION_ID FROM `hr_employee` WHERE EMPLOYEE_ID = hr_view_work_history_now.`EMPLOYEE_ID`)"
                    + " ORDER BY work_from LIMIT 1)) AS t ";
            ...
            while (rs.next()) {
                CareerPath careerpath = new CareerPath();
                resultToObjectView(rs, careerpath);
                lists.add(careerpath);
            }
    ...
    public static Vector getlistPJSV2(int year, String inDivision) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT t.*, SUM(DTDIFF) / 365 AS YEAR "
                    + " FROM (SELECT vw.*, (DATEDIFF(vw.`WORK_TO`, vw.`WORK_FROM`)+1) AS DTDIFF FROM `hr_view_work_history_now` vw "
                    + " INNER JOIN hr_employee emp ON vw.`EMPLOYEE_ID` = emp.`EMPLOYEE_ID` and vw.`DIVISION_ID` = emp.`DIVISION_ID` "
                    + " and vw.`DEPARTMENT_ID` = emp.`DEPARTMENT_ID` AND vw.`SECTION_ID` = emp.`SECTION_ID` AND vw.`POSITION_ID` = emp.`POSITION_ID` AND emp.resigned = 0 "
                    + " WHERE (DATEDIFF(WORK_FROM,(SELECT v2.WORK_TO FROM `hr_view_work_history_now` v2 WHERE v2.EMPLOYEE_ID = vw.`EMPLOYEE_ID` "
            ...
            sql += "GROUP BY t.EMPLOYEE_ID HAVING YEAR >= "+year+" order by YEAR DESC";
            ...
            while (rs.next()) {
                CareerPath careerpath = new CareerPath();
                resultToObjectView(rs, careerpath);
                careerpath.setSalary(rs.getDouble("YEAR"));
                lists.add(careerpath);
            }
    ...
    public static int countDateDiff(Date dateBefore, Date dateAfter){
        int count = 0;
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT DATEDIFF('"+Formater.formatDate(dateBefore,"yyyy-MM-dd")+"', '"+Formater.formatDate(dateAfter,"yyyy-MM-dd")+"') AS DIFF";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
        ...
        return count;
        //return minus(-) klo dateAfternya lebih besar
        //return positif(+) klo dateAfternya lebihKeci
    }
    
    public static int getCountlistEmpPerMasaJabatan(int year, String inDivision) {
        int count = 0;
        Vector listJabatanSekarang = new Vector();
        DBResultSet dbrs = null;
        try {
            //untuk ambil jabatan yang sekarang dari view
            String sql = "SELECT * FROM hr_view_work_history_now WHERE `WORK_HISTORY_NOW_ID` = 0  ";
            if (inDivision.length()>0){
                sql+=" AND division_id IN ("+inDivision+")";
            }
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            ...
            while (rs.next()) {
                CareerPath careerpath = new CareerPath();
                resultToObjectView(rs, careerpath);
                listJabatanSekarang.add(careerpath);
            }
            rs.close();
            
            
            //untuk ambil jabatan yang sekarang dari view
            String sql2 = "SELECT carer_old.* FROM hr_view_work_history_now carer_old WHERE carer_old.WORK_HISTORY_NOW_ID <> 0  ";
            if (inDivision.length()>0){
                sql2+=" AND division_id IN ("+inDivision+")";
            }
            dbrs = DBHandler.execQueryResult(sql2);
             rs = dbrs.getResultSet();
            
            Hashtable<Long,Vector> listsCarerOld = new Hashtable(); 
            
            while (rs.next()) {
                CareerPath careerpath = new CareerPath();
                resultToObjectView(rs, careerpath);
                    //proses untuk set hastable list vector carer path lama by employee_id
                    if(listsCarerOld.get(careerpath.getEmployeeId()) == null){
                        Vector listCarer = new Vector();
                        listCarer.add(careerpath);
                        listsCarerOld.put(careerpath.getEmployeeId(), listCarer);
                    }else{
                        Vector listCarer = listsCarerOld.get(careerpath.getEmployeeId());
                        listCarer.add(careerpath);
                        listsCarerOld.put(careerpath.getEmployeeId(), listCarer);
                    }
            }
            ...
            //proses untuk cek apakah jabatan sebelumnya sama dengan jabatan sekarang
            //jika sama dan berurutan dengan jabatan sekarang
            //maka mulai menjabatanya dihitung dari riwayat jabatan sebelumnya.
            if(listJabatanSekarang.size() > 0){
                for(int xy = 0 ; xy <listJabatanSekarang.size();xy++){
                    CareerPath careerPathNow = (CareerPath) listJabatanSekarang.get(xy);
                    
                    Vector listCarerOldCompare = (Vector)listsCarerOld.get(careerPathNow.getEmployeeId());
                    if(listCarerOldCompare != null){
                        //ini proses array sengaja dibalik supaya yang dicek dari carer path terakhir
                        int xyz = listCarerOldCompare.size()-1;
                        while(xyz >=0){
                           CareerPath careerPathOld = (CareerPath) listCarerOldCompare.get(xyz);
                           boolean masaJabatanBerturut = false;
                               
                               //cek apakah companynya sama
                               if(careerPathOld.getCompanyId()== careerPathNow.getCompanyId()){
                                    //cek apakah satuan kerja sama
                                   if(careerPathOld.getDivisionId() == careerPathNow.getDivisionId()){
                                        //cek apakah unitnya sama
                                       if(careerPathOld.getDepartmentId() == careerPathNow.getDepartmentId()){
                                           //cek apakah sub unit sama
                                           if(careerPathOld.getSectionId() == careerPathNow.getSectionId()){
                                               //cek apakah position sama
                                                if(careerPathOld.getPositionId() == careerPathNow.getPositionId()){
                                                    masaJabatanBerturut = true; 
                                                }
                                           }
                                       }
                                   }
                           ...
                           if(masaJabatanBerturut){
                                int dateDiff = countDateDiff(careerPathNow.getWorkFrom(),careerPathOld.getWorkTo());
                               if(dateDiff==1){
                                   careerPathNow.setWorkFrom(careerPathOld.getWorkFrom());
                               }
                           }
                           xyz--;
                        }
                    ...
                    //proses pengecekan masa jabatan
                    //lebih dari parameter tahun
                    int yearResult = countDateDiff(careerPathNow.getWorkTo(),careerPathNow.getWorkFrom()) / 365;
          ...
          System.out.print("Eror :"+exc);
        }
        return count;
    }
            
     public static Vector getlistEmpPerMasaJabatan(int year, String inDivision) {
        Vector lists = new Vector();
        Vector listJabatanSekarang = new Vector();
        DBResultSet dbrs = null;
        try {
            ...
            if(listJabatanSekarang.size() > 0){
                for(int xy = 0 ; xy <listJabatanSekarang.size();xy++){
                    ...
                    if(yearResult >= year){
                        careerPathNow.setSalary(yearResult);
                    lists.add(careerPathNow);
                    }
        ...
        if (listCareer.size() > 0){
            ...
            for(int j=0; j<listCareer.size(); j++){
                ...
                if (careerPath.getHistoryType()==PstCareerPath.CAREER_TYPE || careerPath.getHistoryType()==PstCareerPath.PEJABAT_SEMENTARA_TYPE
						|| careerPath.getHistoryType()==PstCareerPath.OJT_TYPE){
    ...
    public static String drawGradeNow(Employee employee, Vector listCareer, String clientName) {
        ...
        output += "<td id=\"current-list\">"+gradeLevel+"</td>";
        if (employee.getHistoryType() == PEJABAT_SEMENTARA_TYPE || employee.getHistoryType() == OJT_TYPE){
            if (riwayatJabatanData.getWorkFrom() != null){
            ...
            for (int i=0; i<listCareer.size(); i++){
                ...
                if ((careerPath.getHistoryGroup() == RIWAYAT_CAREER_N_GRADE)||(careerPath.getHistoryGroup() == RIWAYAT_JABATAN)){
                    if (careerPath.getHistoryType() == CAREER_TYPE || careerPath.getHistoryType() == PEJABAT_SEMENTARA_TYPE 
							|| careerPath.getHistoryType() == OJT_TYPE ){
                        output += "<tr>";
                        if (privUpdate == true){
    ...
    public static Date getLastDateCareer(long employeeId, String historyGroupCondition){
        ...
        where += fieldNames[FLD_HISTORY_TYPE]+"="+PEJABAT_SEMENTARA_TYPE +" OR ";
		where += fieldNames[FLD_HISTORY_TYPE]+"="+OJT_TYPE +") ";
    ...
    public static Vector getCareerListByPeriod(String periodFrom, String periodTo, long divisionId){
        ...
        whereClause += " AND "+fieldNames[FLD_HISTORY_TYPE]+"="+CAREER_TYPE;
    ...
    public static Vector getEmployeeByPeriod(String periodFrom, String periodTo, long divisionId, String positionIds){
        ...
        if (careerList != null && careerList.size()>0){
            for (int i=0; i<careerList.size(); i++){
                ...
                empNPos.setDepartmentId(career.getDepartmentId());
                empNPos.setSectionId(career.getSectionId());
        ...
        if (empListTemp != null && empListTemp.size()>0){
            for (int i=0; i<empListTemp.size(); i++){
                ...
                if (ketemu){
                    ...
                    empNPos.setDepartmentId(emp.getDepartmentId());
                    empNPos.setSectionId(emp.getSectionId());
                    employeeList.add(empNPos); /* this result */
                }
            }
        }
    ...
    public static boolean isCurrentCareer(long employeeId, long positionId, String periodFrom, String periodTo){
        ...
        if (listCareer != null && listCareer.size()>0){
            ...
            int intWorkFrom = getConvertDateToInt(""+emp.getCommencingDate());
            int intWorkTo = getConvertDateToInt(strDateNow);
            ketemu = checkDataByPeriod(intWorkFrom, intWorkTo, intPeriodFrom, intPeriodTo);
        }
        }
        return ketemu;
    ...
    public static long getEmployeeIdCaseBOD(String periodFrom, String periodTo, long positionId){
        ...
        if (employeeId == 0){
            ...
            if (empListTemp != null && empListTemp.size()>0){
                ...
                if (!(ketemu)){
                    Employee emp = (Employee)empListTemp.get(0);
                    employeeId = emp.getOID();
                }
            }
    ...
    public static Vector getLastCareerPath(long oidEmployee){
        Vector lists = new Vector();

        DBResultSet dbrs = null;
        try{
            String sql = "SELECT * FROM `hr_work_history_now` " +
                        "WHERE EMPLOYEE_ID  = '"+oidEmployee+"' ORDER BY WORK_TO DESC LIMIT 1";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static Vector getLastGradeHistory(long oidEmployee, int year){
        Vector lists = new Vector();
        int prevYear = year-1;
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT * FROM " + TBL_HR_WORK_HISTORY_NOW +
                        " WHERE " + fieldNames[FLD_EMPLOYEE_ID] + "  = '"+oidEmployee+"'"
                      + " AND " + fieldNames[FLD_HISTORY_GROUP] +" IN (1,2) AND ("+ fieldNames[FLD_WORK_TO] 
                      + " = '"+prevYear+"-12-31' OR  "+ fieldNames[FLD_WORK_TO]+" LIKE '%"+year+"%' ) "
                      + "ORDER BY WORK_TO";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static String getLastWorkFrom(long employeeId){
        String date="";
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT DATE_ADD("+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_TO]+", INTERVAL 1 DAY) AS SINCE"
                      + " FROM " + TBL_HR_WORK_HISTORY_NOW
                      + " WHERE " + fieldNames[FLD_EMPLOYEE_ID] + "  = '"+employeeId+"'"
                      + " AND " + fieldNames[FLD_HISTORY_GROUP] +" = 0 AND "
                      + fieldNames[FLD_HISTORY_TYPE]+" IN (0,1) " 
                      + "ORDER BY WORK_TO DESC LIMIT 1";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();

            while (rs.next()) {
                date = rs.getString("SINCE");
            }
            rs.close();
            return date;
        } catch (Exception e) {
	...
	public static Vector getLevelSama(long employeeId, long levelId){
        Vector lists= new Vector();
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT *"
                      ...
                      + " WHERE " + fieldNames[FLD_EMPLOYEE_ID] + "  = '"+employeeId+"'"
                      + " AND " + fieldNames[FLD_HISTORY_GROUP] +" = 0 AND "
                      + fieldNames[FLD_HISTORY_TYPE]+" IN (0,1) AND "
					  + fieldNames[FLD_LEVEL_ID]+" = "+levelId
                      + " ORDER BY WORK_TO DESC";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
	...
	public static Vector getDivisiSama(long employeeId, long divisionId){
        Vector lists= new Vector();
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT *"
                      ...
                      + " AND " + fieldNames[FLD_HISTORY_GROUP] +" = 0 AND "
                      + fieldNames[FLD_HISTORY_TYPE]+" IN (0,1) AND "
					  + fieldNames[FLD_DIVISION_ID]+" = "+divisionId
                      + " ORDER BY WORK_TO DESC";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
	...
	public static Vector getJabatanSama(long employeeId, long positionId){
        Vector lists= new Vector();
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT *"
                      ...
                      + " AND " + fieldNames[FLD_HISTORY_GROUP] +" = 0 AND "
                      + fieldNames[FLD_HISTORY_TYPE]+" IN (0,1) AND "
					  + fieldNames[FLD_POSITION_ID]+" = "+positionId
                      + " ORDER BY WORK_TO DESC";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
	...
	public static Vector getCareerHistory(long employeeId){
        Vector lists= new Vector();
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT *"
	...
	public static double getDuration(String where){
        double duration = 0;
        DBResultSet dbrs = null;
        try{
            String sql = "SELECT SUM(DATEDIFF("+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_TO]+", "+PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_FROM]+") / 30)"
                      + " FROM hr_view_work_history_now"
                      + " WHERE (" + fieldNames[FLD_HISTORY_GROUP] + "  = '0'"
                      + " OR " + fieldNames[FLD_WORK_HISTORY_NOW_ID] +" = 0 ) ";
			if (where.length()>0){
				sql+= " AND "+where;
			}
            dbrs = DBHandler.execQueryResult(sql);
    ...
    public static Vector getPositionHistory(String whereCareerPath, String whereEmployee){
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        ...
        try{
            String sql = "(SELECT "
                    ...
                    + "FROM `hr_work_history_now` `whn` "
                    + "WHERE (`whn`.`HISTORY_GROUP` <> 1 AND "+whereCareerPath+")) UNION "
                    + "(SELECT "
                        + "0 AS `WORK_HISTORY_NOW_ID`, "
                        + "`emp`.`EMPLOYEE_ID` AS `EMPLOYEE_ID`, "
                        + "`emp`.`COMPANY_ID` AS `COMPANY_ID`, "
                    ...
                    + "`emp`.`HISTORY_TYPE` AS `HISTORY_TYPE`, "
                    + "`emp`.`GRADE_LEVEL_ID` AS `GRADE_LEVEL_ID` "
                    + "FROM `hr_employee` `emp` WHERE "+whereEmployee+") ORDER BY DIVISION_ID, WORK_FROM";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static ArrayList listEmptyPositionInDivision(int showOnlyEmptyPosition, String multiDivisionOid, String dateFrom, String dateTo) {
        ...
        try {
            String sql = "SELECT "
                    + "  d.DIVISION, "
                    + "  p.POSITION, "
                    + "  COALESCE(map_jabatan.EMPLOYEE_NUM, \"-\") AS EMPLOYEE_NUM, "
                    + "  COALESCE(map_jabatan.FULL_NAME, \"-\") AS FULL_NAME, "
                    + "  COALESCE(map_jabatan.WORK_FROM, \"-\") AS WORK_FROM, "
                    + "  COALESCE(map_jabatan.WORK_TO, \"-\") AS WORK_TO "
                    + " FROM "
                    + "  hr_position_division pd "
                    + "  INNER JOIN hr_division d "
                    + "    ON d.DIVISION_ID = pd.DIVISION_ID "
                    + "  INNER JOIN hr_position p "
                    + "    ON p.POSITION_ID = pd.POSITION_ID "
                    + "  INNER JOIN hr_level l "
                    ...
                    + "      vwh.WORK_TO "
                    + "    FROM "
                    + "      hr_view_work_history_now vwh "
                    + "      INNER JOIN hr_employee e "
                    + "        ON e.EMPLOYEE_ID = vwh.EMPLOYEE_ID "
                    + "    WHERE ( "
                    + "        '" + dateFrom + "' BETWEEN vwh.WORK_FROM "
                    + "        AND vwh.WORK_TO "
                    + "        OR '" + dateTo + "' BETWEEN vwh.WORK_FROM "
                    + "        AND vwh.WORK_TO "
                    + "      ) ";
            
            if (!multiDivisionOid.isEmpty()) {
                sql += "     AND vwh.DIVISION_ID IN (" + multiDivisionOid + ")";
            }
            sql += ""
                    + "    ) AS map_jabatan "
                    + "    ON map_jabatan.POSITION_ID = p.POSITION_ID ";
            
            //WHERE CLAUSE
            String where = " d.VALID_STATUS = " + PstDivision.VALID_ACTIVE;
            if (showOnlyEmptyPosition == 1) {
                where += where.isEmpty() ? "" : " AND ";
                where += " map_jabatan.EMPLOYEE_NUM IS NULL ";
            }
            if (!multiDivisionOid.isEmpty()) {
                where += where.isEmpty() ? "" : " AND ";
                where += " d.DIVISION_ID IN (" + multiDivisionOid + ")";
            }
            ...
            try (ResultSet rs = dbrs.getResultSet()) {
                while (rs.next()) {
                    ...
                    data.put("DIVISION_NAME", rs.getString("DIVISION"));
                    data.put("POSITION_NAME", rs.getString("POSITION"));
                    data.put("EMPLOYEE_NUM", rs.getString("EMPLOYEE_NUM"));
                    data.put("EMPLOYEE_NAME", rs.getString("FULL_NAME"));
                    data.put("WORK_FROM", rs.getString("WORK_FROM"));
                    data.put("WORK_TO", rs.getString("WORK_TO"));
                    lists.add(data);
                }
            }
    ...
    public static ArrayList listEmptyPositionInDivisionByPosition(int showOnlyEmptyPosition, String multiDivisionOid,String multiPositionOid, String dateFrom, String dateTo) {
        ArrayList lists = new ArrayList();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT "
            ...
            //add by eri 2020-07-17
            if(!multiPositionOid.isEmpty()){
                where += where.isEmpty() ? "" : " AND ";
                where += " p.POSITION_ID IN (" + multiPositionOid + ")";
            }
            //end add by eri
            sql += where.isEmpty() ? "" : " WHERE " + where;
    ...
    public static ArrayList listEmptyPositionInDepartment(int showOnlyEmptyPosition, String multiDepartmentOid, String dateFrom, String dateTo) {
        ArrayList lists = new ArrayList();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT "
            ...
            if (!multiDepartmentOid.isEmpty()) {
                sql += "     AND vwh.DEPARTMENT_ID IN (" + multiDepartmentOid + ")";
            }
            sql += ""
                    + "    ) AS map_jabatan "
                    + "    ON map_jabatan.POSITION_ID = p.POSITION_ID ";
            ...
            //WHERE CLAUSE
            String where = " d.VALID_STATUS = " + PstDepartment.VALID_ACTIVE;
            if (showOnlyEmptyPosition == 1) {
                where += where.isEmpty() ? "" : " AND ";
                where += " map_jabatan.EMPLOYEE_NUM IS NULL ";
            }
            if (!multiDepartmentOid.isEmpty()) {
                where += where.isEmpty() ? "" : " AND ";
                where += " d.DEPARTMENT_ID IN (" + multiDepartmentOid + ")";
            }
            sql += where.isEmpty() ? "" : " WHERE " + where;
            ...
            try (ResultSet rs = dbrs.getResultSet()) {
                while (rs.next()) {
                    HashMap<String, Object> data = new HashMap<>();
                    data.put("DEPARTMENT_NAME", rs.getString("DEPARTMENT"));
                    data.put("POSITION_NAME", rs.getString("POSITION"));
                    data.put("EMPLOYEE_NUM", rs.getString("EMPLOYEE_NUM"));
                    data.put("EMPLOYEE_NAME", rs.getString("FULL_NAME"));
                    data.put("WORK_FROM", rs.getString("WORK_FROM"));
    ...
    public static ArrayList listEmptyPositionInDepartmentByPosition(int showOnlyEmptyPosition, String multiDepartmentOid,String multiPositionOid, String dateFrom, String dateTo) {
        ArrayList lists = new ArrayList();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT "
    ...
    public static ArrayList listEmptyPositionInSection(int showOnlyEmptyPosition, String multiDivisionOid, String dateFrom, String dateTo) {
        ArrayList lists = new ArrayList();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT "
                    ...
                    + "  COALESCE(map_jabatan.WORK_TO, \"-\") AS WORK_TO "
                    + " FROM "
                    + "  hr_position_section ps "
                    + "  INNER JOIN hr_section s "
            ...
            if (!multiDivisionOid.isEmpty()) {
                sql += "     AND vwh.SECTION_ID IN (" + multiDivisionOid + ")";
            }
            sql += ""
                    + "    ) AS map_jabatan "
                    + "    ON map_jabatan.POSITION_ID = p.POSITION_ID ";
            ...
            //WHERE CLAUSE
            String where = " s.VALID_STATUS = " + PstSection.VALID_ACTIVE;
            if (showOnlyEmptyPosition == 1) {
                where += where.isEmpty() ? "" : " AND ";
                where += " map_jabatan.EMPLOYEE_NUM IS NULL ";
            }
            if (!multiDivisionOid.isEmpty()) {
                where += where.isEmpty() ? "" : " AND ";
                where += " s.SECTION_ID IN (" + multiDivisionOid + ")";
            }
            sql += where.isEmpty() ? "" : " WHERE " + where;
            ...
            try (ResultSet rs = dbrs.getResultSet()) {
                while (rs.next()) {
                    HashMap<String, Object> data = new HashMap<>();
                    data.put("SECTION_NAME", rs.getString("SECTION"));
                    data.put("POSITION_NAME", rs.getString("POSITION"));
                    data.put("EMPLOYEE_NUM", rs.getString("EMPLOYEE_NUM"));
                    data.put("EMPLOYEE_NAME", rs.getString("FULL_NAME"));
                    data.put("WORK_FROM", rs.getString("WORK_FROM"));
        ...
        public static ArrayList listEmptyPositionInSectionByPosition(int showOnlyEmptyPosition, String multiDivisionOid,String multiPositionOid, String dateFrom, String dateTo) {
        ArrayList lists = new ArrayList();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT "

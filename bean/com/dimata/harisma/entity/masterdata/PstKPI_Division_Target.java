    ...
    public static Hashtable HlistNew(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            ...
            while (rs.next()) {
                KPI_Division_Target kPI_Division_Target = new KPI_Division_Target();
                resultToObject(rs, kPI_Division_Target);
                Date startDate = rs.getDate(PstKPI_Division_Target.fieldNames[PstKPI_Division_Target.FLD_STARTDATE]); 
                int month = startDate.getMonth()+1;
                hashListDiv.put(kPI_Division_Target.getDivisionId()+"_"+month, kPI_Division_Target);  
            }
            rs.close();
            return hashListDiv;

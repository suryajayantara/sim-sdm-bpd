...
public class PstStructureTemplate extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector listWithAccess(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT temp.* FROM " + TBL_STRUCTURE_TEMPLATE +" temp "
                        + " INNER JOIN "+PstStructurePositionAccess.TBL_STRUCTURE_POSITION_ACCESS+" acc "
                        + " ON temp."+fieldNames[FLD_TEMPLATE_ID]+" = acc."+PstStructurePositionAccess.fieldNames[PstStructurePositionAccess.FLD_STRUCTURE_TEMPLATE_ID];
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {

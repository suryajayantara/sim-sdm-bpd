...
public class PstKPI_List_Group extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public PstKPI_List_Group() {
	...
	public static String[] getKpiGroup(long oidKpiList){
		String whereClause = PstKPI_List_Group.fieldNames[PstKPI_List_Group.FLD_KPI_LIST_ID]
				+" = "+oidKpiList;
		Vector listGroup = PstKPI_List_Group.list(0, 0, whereClause, "");
		String[] kpi = new String[listGroup.size()];
		for (int i=0; i < listGroup.size(); i++){
			KPI_List_Group kpiList = (KPI_List_Group) listGroup.get(i);
			kpi[i] = ""+kpiList.getKpiGroupId();
		}

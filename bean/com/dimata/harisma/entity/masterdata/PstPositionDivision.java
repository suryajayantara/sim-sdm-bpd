...
public class PstPositionDivision extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector listDistinct(String whereClause) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "+fieldNames[FLD_DIVISION_ID]+" FROM " + TBL_POSITION_DIVISION;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                long divisionId = rs.getLong(PstPositionDivision.fieldNames[PstPositionDivision.FLD_DIVISION_ID]);
                lists.add(divisionId);
            }
            rs.close();
            return lists;
        } catch (Exception e) {

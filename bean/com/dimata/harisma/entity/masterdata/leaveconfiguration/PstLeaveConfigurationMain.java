...
//  }
    ...
    public static Vector listJoinEmpLeaveConfigDepart(Vector listDetail) {
        return listJoinEmpLeaveConfigDepart(listDetail, 0);
    }
    
    public static Vector listJoinEmpLeaveConfigDepart(Vector listDetail, long empId) {
        
        String whereClause = "";
        if (listDetail.size()>0){
            for (int x=0; x<listDetail.size(); x++){
                OvertimeDetail overtimeDetail = (OvertimeDetail) listDetail.get(x);
                
                whereClause += overtimeDetail.getEmpDeptId()+",";
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT md.*, cm.`EMPLOYEE_ID`,  emp.`FULL_NAME`,emp.`FULL_NAME`, dpt.DEPARTMENT, sec.SECTION FROM hr_leave_configuration_main_departement AS md  INNER JOIN hr_department AS dpt  ON md.DEPARTEMENT_ID = dpt.DEPARTMENT_ID "
                    + "  INNER JOIN hr_leave_configuration_main AS cm ON cm.`LEAVE_CONFIGURATION_MAIN_ID` = md.`LEAVE_CONFIGURATION_MAIN_ID`"
                    + "  INNER JOIN hr_employee AS emp  ON cm.EMPLOYEE_ID = emp.EMPLOYEE_ID "
                    + "  LEFT JOIN hr_section AS sec   ON sec.SECTION_ID = md.SECTION_ID WHERE md.`DEPARTEMENT_ID` IN ("+whereClause+") ";
                    if(empId>0){
                    sql += " AND cm.`EMPLOYEE_ID` NOT IN ("+empId+")"   ; //kecuali requester
                    }
                    sql += " GROUP BY cm.`EMPLOYEE_ID`"   ;
            ...
            while (rs.next()) {
                ...
                employee.setOID(rs.getLong("cm."+PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]));
                employee.setFullName(rs.getString("emp."+PstEmployee.fieldNames[PstEmployee.FLD_FULL_NAME]));
                lists.add(employee);
            }
            rs.close();
            return lists;

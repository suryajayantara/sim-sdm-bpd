...
public class PstPositionDepartment extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector listDistinct(String whereClause) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "+fieldNames[FLD_DEPARTMENT_ID]+" FROM " + TBL_POSITION_DEPARTMENT;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                long deptId = rs.getLong(PstPositionDepartment.fieldNames[PstPositionDepartment.FLD_DEPARTMENT_ID]);
                lists.add(deptId);
            }
            rs.close();
            return lists;

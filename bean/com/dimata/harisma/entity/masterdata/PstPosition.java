...
public class PstPosition extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language{
    ...
    //ARYS 20160818
    public static final int FLD_TENAGA_KERJA=20;
    public static final int FLD_JENIS_JABATAN = 21;
    public static final int FLD_ALIAS = 22;
    public static final int FLD_POSITION_TYPE_ID = 23;
...
//        "--None--"
    ...
    //Jenis Jabatan
    public static final int JENIS_KOMISARIS_UTAMA = 0;
    public static final int JENIS_KOMISARIS = 1;
    public static final int JENIS_DIREKTUR_UTAMA = 2;
    public static final int JENIS_DIREKTUR = 3;
    public static final int JENIS_DEWAN_PENGAWAS_SYARIAH = 4;
    public static final int JENIS_PEJABAT_EKSEKUTIF = 5;
    public static final int JENIS_STAF = 6;
    public static final int JENIS_LAIN_LAIN = 7;
    
    public static final String[] strJenisJabatan = {
        "Komisaris Utama",
        "Komisaris",
        "Direktur Utama",
        "Direktur",
    ...
    public static final int[] strJenisJabatanInt = {
      01,
      02,
      03,
      04,
    ...
    //ARYS 20160818
    public static final int ASING_KEPEMIMPINAN = 0;
    public static final int ASING_TENAGA_AHLI = 1;
    public static final int ASING_USAHA_JASA = 2;
    public static final int ASING_USAHA_PENJUALAN = 3;
    public static final int ASING_TATA_USAHA = 4;
    public static final int LOKAL_KEPEMIMPINAN = 5;
    public static final int LOKAL_TENAGA_AHLI = 6;
    public static final int LOKAL_USAHA_JASA = 7;
    public static final int LOKAL_USAHA_PENJUALAN = 8;
    public static final int LOKAL_TATA_USAHA = 9;
    
    
    
    public static final String[] strTenagaKerja = {
        "Asing - Kepemimpinan",
        "Asing - Tenaga Ahli",
        "Asing - Usaha Jasa",
        "Asing - Usaha Penjualan",
        "Asing - Tata Usaha",
        
        "Lokal - Kepemimpinan",
        "Lokal - Tenaga Ahli",
        "Lokal - Usaha Jasa",
        "Lokal - Usaha Penjualan",
        "Lokal - Tata Usaha",
        

    };
    public static final int[] strTenagaKerjaint = {
        1,
        2,
        3,
        4,
    ...
    public static Position fetchExc(long oid) throws DBException {
        try {
            ...
            //ARYS 20160818
            position.setTenagaKerja(pstPosition.getInt(FLD_TENAGA_KERJA));
            position.setJenisJabatan(pstPosition.getInt(FLD_JENIS_JABATAN));
            position.setAlias(pstPosition.getString(FLD_ALIAS));
            position.setPosTypeId(pstPosition.getlong(FLD_POSITION_TYPE_ID));
    ...
    public static long insertExc(Position position) throws DBException {
        try {
            ...
            //arys 20160818
            pstPosition.setInt(FLD_TENAGA_KERJA, position.getTenagaKerja());
            pstPosition.setInt(FLD_JENIS_JABATAN, position.getJenisJabatan());
            pstPosition.setString(FLD_ALIAS, position.getAlias());
            pstPosition.setLong(FLD_POSITION_TYPE_ID, position.getPosTypeId());
     ...
     */
     ...
     public static Hashtable listHast0Value(int limitStart, int recordToGet, String whereClause, String order) {
        Hashtable listt = new Hashtable();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_HR_POSITION;
    ...
    public static void resultToObject(ResultSet rs, Position position){
        ...
        try {
            ...
            //arys 20160818
            position.setTenagaKerja(rs.getInt(PstPosition.fieldNames[PstPosition.FLD_TENAGA_KERJA]));
            position.setJenisJabatan(rs.getInt(PstPosition.fieldNames[PstPosition.FLD_JENIS_JABATAN]));
            position.setAlias(rs.getString(PstPosition.fieldNames[PstPosition.FLD_ALIAS]));
            position.setPosTypeId(rs.getLong(PstPosition.fieldNames[PstPosition.FLD_POSITION_TYPE_ID]));
    ...
    public static Vector listWithJoinKpiSettingPosition(long oidKpiSetting) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM hr_kpi_setting \n" +
                            "INNER JOIN `hr_kpi_setting_position`\n" +
                            "ON `hr_kpi_setting`.`KPI_SETTING_ID` = `hr_kpi_setting_position`.`KPI_SETTING_ID`\n" +
                            "LEFT JOIN hr_position\n" +
                            "ON `hr_kpi_setting_position`.`POSITION_ID` = `hr_position`.`POSITION_ID`\n" +
                            "WHERE `hr_kpi_setting_position`.`KPI_SETTING_ID` = "+ oidKpiSetting;
                         
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                Position entPosition = new Position();
                resultToObject(rs, entPosition);
                lists.add(entPosition);
            }
            rs.close();
            return lists;
        } catch (Exception e) {
    ...
    public static Vector getPositionSortByLevel(String whereClause){
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "";
            sql  = "SELECT hr_position.POSITION_ID FROM hr_position ";
            sql += "INNER JOIN hr_level ON hr_position.LEVEL_ID=hr_level.LEVEL_ID ";
            sql += "WHERE hr_position.POSITION_ID IN ("+whereClause+") ";
            sql += "ORDER BY hr_level.LEVEL_RANK DESC";

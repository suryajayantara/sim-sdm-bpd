...
public class PstLevel extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {    
    ...
    public static final int FLD_MAX_NUMBER_APPROVAL = 12;
	public static final int FLD_MIN_GRADE = 13;
	public static final int FLD_MAX_GRADE = 14;
    ...
    public static Level fetchExc(long oid) throws DBException {        
        try {            
            ...
            level.setMaxNumberApproval(pstLevel.getInt(FLD_MAX_NUMBER_APPROVAL));
			level.setGradeMin(pstLevel.getInt(FLD_MIN_GRADE));
			level.setGradeMax(pstLevel.getInt(FLD_MAX_GRADE));
    ...
    public static long insertExc(Level level) throws DBException {        
        try {            
            ...
            pstLevel.setInt(FLD_MAX_NUMBER_APPROVAL, level.getMaxNumberApproval());
			pstLevel.setInt(FLD_MIN_GRADE, level.getGradeMin());
			pstLevel.setInt(FLD_MAX_GRADE, level.getGradeMax());
            pstLevel.insert();            
	...
	public static Level getByGrade(int min, int max) {
		Level level = new Level();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_HR_LEVEL + " WHERE " + fieldNames[FLD_MIN_GRADE]
					+ "  BETWEEN "+min+" AND "+max+" AND "+fieldNames[FLD_MAX_GRADE]
					+" BETWEEN "+min+" AND "+max+ " LIMIT 1";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                resultToObject(rs, level);
    ...
    public static void resultToObject(ResultSet rs, Level level) {
        try {
            ...
            level.setMaxNumberApproval(rs.getInt(PstLevel.fieldNames[PstLevel.FLD_MAX_NUMBER_APPROVAL]));
			level.setGradeMin(rs.getInt(PstLevel.fieldNames[PstLevel.FLD_MIN_GRADE]));
			level.setGradeMax(rs.getInt(PstLevel.fieldNames[PstLevel.FLD_MAX_GRADE]));
        } catch (Exception e) {
        }
    }

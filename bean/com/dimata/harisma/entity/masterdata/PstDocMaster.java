   ...
   public PstDocMaster() {
    ...
    public static DocMaster fetchExc(long oid) throws DBException {
        try {
            ...
            docMaster.setDivisionId(pstDocMaster.getLong(FLD_DIVISION_ID));
    ...
    public static long insertExc(DocMaster docMaster) throws DBException {
        try {
            ...
            pstDocMaster.setLong(FLD_DIVISION_ID, docMaster.getDivisionId());
    ...
    public static DocMaster getDocMasterByEmpDocId(long empDocId) {
        DocMaster docMaster = new DocMaster();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT "+TBL_HR_DOC_MASTER+".* FROM "+TBL_HR_DOC_MASTER+" \n" +
                            "INNER JOIN hr_emp_doc \n" +
                            "ON hr_emp_doc.`DOC_MASTER_ID` = hr_doc_master.`DOC_MASTER_ID`\n" +
                            "INNER JOIN hr_emp_doc_field\n" +
                            "ON `hr_emp_doc_field`.`EMP_DOC_ID` = hr_emp_doc.`EMP_DOC_ID`\n" +
                            "WHERE hr_emp_doc.`EMP_DOC_ID` = "+empDocId+" \n" +
                            "LIMIT 0 , 1"  ;
        ...
        return new DocMaster();
    }
	
	public static Vector listJoinMapUser(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT M.* FROM " + TBL_HR_DOC_MASTER+" M INNER JOIN "
					+ PstDocMasterUser.TBL_HR_DOC_MASTER_USER+" U "
					+ "ON M."+fieldNames[FLD_DOC_MASTER_ID]+" = U."+PstDocMasterUser.fieldNames[PstDocMasterUser.FLD_DOC_MASTER_ID];
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
      ...
      public static void resultToObject(ResultSet rs, DocMaster docMaster) {
        try {
            ...
            docMaster.setDivisionId(rs.getLong(PstDocMaster.fieldNames[PstDocMaster.FLD_DIVISION_ID]));
        } catch (Exception e) {
        }
    }

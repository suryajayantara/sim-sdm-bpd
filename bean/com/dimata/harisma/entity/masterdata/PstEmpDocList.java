...
public class PstEmpDocList extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public PstEmpDocList() {
    ...
    public static String listEmployeeId(long empDocId, String objectName) {
        String lists = " ";
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_HR_EMP_DOC_LIST;
            
                   sql = sql + " WHERE   " + PstEmpDocList.fieldNames[PstEmpDocList.FLD_EMP_DOC_ID] + " = " + empDocId + " AND "+ PstEmpDocList.fieldNames[PstEmpDocList.FLD_OBJECT_NAME] + " = \"" + objectName+" \" ";
            
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                lists += rs.getLong(PstEmpDocList.fieldNames[PstEmpDocList.FLD_EMPLOYEE_ID]) + ",";
            }
            rs.close();
...
public static long deletewhere(long employeeId, long empDocId, String objectName) {
    ...
    public static int findLimitStart(long oid, int recordToGet, String whereClause) {
     ...
     public static long getEmployeeIdByObjectnameEmpDocId(String objectName, long empDocId) {
        DBResultSet dbrs = null;
        long result = 0;
        try {
            String sql = "SELECT " + PstEmpDocList.fieldNames[PstEmpDocList.FLD_EMPLOYEE_ID] + " FROM " + TBL_HR_EMP_DOC_LIST + " WHERE "
                    + PstEmpDocList.fieldNames[PstEmpDocList.FLD_OBJECT_NAME] + " = \"" + objectName + "\" AND " + PstEmpDocList.fieldNames[PstEmpDocList.FLD_EMP_DOC_ID] + " = " + empDocId;
    ...
    public static Vector listSPJ(long oidPeriod) {
        Vector lists = new Vector();
        String whereStatus = PstPeriod.fieldNames[PstPeriod.FLD_PERIOD_ID] + " = " + oidPeriod;
        Vector periodList = PstPeriod.list(0, 0, whereStatus, "");
        String startDate = "";
        String endDate = "";
        if (periodList != null & periodList.size() > 0){
            for (int i = 0; i < periodList.size(); i++){
                Period period = (Period) periodList.get(i);
                startDate = String.valueOf(period.getStartDate());
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT LST.* FROM " + TBL_HR_EMP_DOC_LIST + " AS LST INNER JOIN " + PstEmpDocField.TBL_HR_EMP_DOC_FIELD + 
                    " AS FLD ON LST." + PstEmpDocField.fieldNames[PstEmpDocField.FLD_EMP_DOC_ID] + " = FLD." + PstEmpDocList.fieldNames[PstEmpDocList.FLD_EMP_DOC_ID] +
                    " WHERE FLD." + PstEmpDocField.fieldNames[PstEmpDocField.FLD_VALUE] + " BETWEEN '" +startDate+ "' AND '" + endDate + "' AND LST."
                    + PstEmpDocList.fieldNames[PstEmpDocList.FLD_OBJECT_NAME] + " = 'TRAINNER2'";
    ...
    public static long getOIDSign(long oidEmpDoc) {
        long result = 0;
        String whereStatus = PstEmpDocList.fieldNames[PstEmpDocList.FLD_EMP_DOC_ID] + " = " + oidEmpDoc +
                            " AND " + PstEmpDocList.fieldNames[PstEmpDocList.FLD_OBJECT_NAME] + " = 'TRAINNER3'";
        Vector list = PstEmpDocList.list(0, 0, whereStatus, "");
        if (list != null & list.size() > 0){
            for (int i = 0; i < list.size(); i++){
                EmpDocList empDocList = (EmpDocList) list.get(i);
                result = empDocList.getEmployee_id();
            }
        }

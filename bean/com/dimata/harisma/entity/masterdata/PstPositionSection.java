...
public class PstPositionSection extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector listDistinct(String whereClause) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "+fieldNames[FLD_SECTION_ID]+" FROM " + TBL_POSITION_SECTION;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                long secId = rs.getLong(PstPositionSection.fieldNames[PstPositionSection.FLD_SECTION_ID]);
                lists.add(secId);
            }
            rs.close();
            return lists;

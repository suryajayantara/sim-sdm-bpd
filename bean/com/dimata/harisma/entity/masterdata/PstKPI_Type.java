...
public class PstKPI_Type extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
   ...
   public static final int FLD_INDEXING = 3;
   ...
   public PstKPI_Type() {
    ...
    public static KPI_Type fetchExc(long oid) throws DBException {
        try {
            ...
            kPI_Type.setIndexing(pstKPI_Type.getInt(FLD_INDEXING));
    ...
    public static long insertExc(KPI_Type kPI_Type) throws DBException {
        try {
            ...
            pstKPI_Type.setInt(FLD_INDEXING, kPI_Type.getIndexing());
    ...
    public static Vector listWithJoinKpiSettingTypeAndKpiSetting(long oidKpiSetting) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT \n" +
"	`hr_kpi_type`.`TYPE_NAME`,\n" +
...
"	 WHERE `hr_kpi_setting`.`KPI_SETTING_ID`="+ oidKpiSetting ;
    ...
    public static int getLatestIndexing(String whereClause) throws Exception{
        Vector list = new Vector();
        DBResultSet dbrs = null;
        
        try { 
            String sql = "SELECT MAX(NUMBER_INDEX) FROM " + TBL_HR_KPI_TYPE;
            if(whereClause != null && whereClause.length() > 0){
                sql += "WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet res = dbrs.getResultSet();
            
            int latestIndex = 0;
            while(res.next()){
                latestIndex = res.getInt(1);
                latestIndex++;
            }
 ...
 public static String getKpiTypeName(long oidKpiType) {
        String result = "";
        try {
            PstKPI_Type pstKpiType = new PstKPI_Type();
            KPI_Type kpiType = pstKpiType.fetchExc(oidKpiType);
            return kpiType.getType_name();
        } catch (Exception e) {

...
public class PstOvertimeDetail extends DBHandler implements I_DBType, I_Language, I_DBInterface, I_PersintentExc, I_DocStatus {
    ...
    public static final int FLD_NORMAL_OVERTIME = 23;
    public static final int FLD_LATE_APPROVAL = 24;
     public static final int FLD_UPLOAD_FILE = 25;
    ...
    public static final int[] fieldTypes = {
        ...
        TYPE_INT,TYPE_LONG,TYPE_INT,TYPE_INT,
        TYPE_STRING
    };
...
private static Vector logicParser(String text) {
    ...
    public static OvertimeDetail fetchExc(long oid) throws DBException {
        try {
            ...
            overtimeDetail.setNormalOvertime(pstOvertimeDetail.getInt(FLD_NORMAL_OVERTIME));
            overtimeDetail.setLateApproval(pstOvertimeDetail.getInt(FLD_LATE_APPROVAL));
            overtimeDetail.setUploadFile(pstOvertimeDetail.getString(FLD_UPLOAD_FILE));
    ...
    public static long insertExc(OvertimeDetail overtimeDetail) throws DBException {
        try {            
            ...
            pstOvertimeDetail.setInt(FLD_NORMAL_OVERTIME, overtimeDetail.getNormalOvertime());
            pstOvertimeDetail.setInt(FLD_LATE_APPROVAL, overtimeDetail.getLateApproval());
            pstOvertimeDetail.setString(FLD_UPLOAD_FILE, overtimeDetail.getUploadFile());
    ...
    public synchronized  static long updateExc(OvertimeDetail overtimeDetail) throws DBException {
        try {
            if (overtimeDetail.getOID() != 0) {
                ...
                //overtimeDetail.setDuration(overtimeDetail.getNetDuration());
                pstOvertimeDetail.setDouble(FLD_OVT_DURATION, overtimeDetail.getTotDuration());
                pstOvertimeDetail.setString(FLD_OVT_DOC_NR, overtimeDetail.getOvt_doc_nr());
                pstOvertimeDetail.setLong(FLD_PAY_SLIP_ID, overtimeDetail.getPay_slip_id());
                pstOvertimeDetail.setString(FLD_STD_WORK_SCHDL, overtimeDetail.getWork_schedule());
                pstOvertimeDetail.setString(FLD_OVT_CODE, overtimeDetail.getOvt_code());
...
//                                 + " LIKE '%"+empNum.trim()+"%'";
    ...
    public static Vector listOvertimeDetailIn(int limitStart, int recordToGet,String oidDepartment, String fullName, Date selectedDateFrom, Date selectedDateTo, String oidSection, String empNum, String order,Vector stStatus,String oidCompany,String oidDivision) {
        Vector lists = new Vector();
      ...
      if(selectedDateFrom!=null && selectedDateTo!=null){
         ...
         if(!oidDepartment.equals("")){
            whereClauseEmpTime = whereClauseEmpTime + " AND "+ "emp."+PstEmployee.fieldNames[PstEmployee.FLD_DEPARTMENT_ID]+" IN (" + oidDepartment+")";
        }
         if(!oidSection.equals("")){
            whereClauseEmpTime = whereClauseEmpTime + " AND "+ "emp."+PstEmployee.fieldNames[PstEmployee.FLD_SECTION_ID]+" IN (" + oidSection+")";
        }
        if(fullName !=null && fullName.length() > 0){
//            whereClauseEmpTime = whereClauseEmpTime + " AND "+ " emp."+PstEmployee.fieldNames[PstEmployee.FLD_FULL_NAME]
//                                 + " LIKE '%"+fullName.trim()+"%'";
...
//                                 + " LIKE '%"+empNum.trim()+"%'";
        ...
        if( stStatus!=null && stStatus.size()>0){
                 //whereClauseEmpTime  = whereClauseEmpTime + PstPresence.fieldNames[PstPresence.FLD_STATUS] + " IN ("+ getEmployeePresence +")" ;
            String getEmployeePresence = PstPresence.getEmployeeIn(0 , 0, "", oidDepartment, fullName.trim(),  
                        selectedDateFrom, selectedDateTo, oidSection, empNum.trim(),stStatus); 
    ...
    public static void resultToObjectWithEmployee(ResultSet rs, OvertimeDetail overtimeDetail) {
        try {
            ...
            overtimeDetail.setNormalOvertime(rs.getInt(fieldNames[FLD_NORMAL_OVERTIME]));
            overtimeDetail.setLateApproval(rs.getInt(fieldNames[FLD_LATE_APPROVAL]));
            overtimeDetail.setUploadFile(rs.getString(fieldNames[FLD_UPLOAD_FILE]));
        } catch (Exception e) {
    ...
    public static void resultToObject(ResultSet rs, OvertimeDetail overtimeDetail) {
        try {
            ...
            overtimeDetail.setTotDuration(rs.getDouble(fieldNames[FLD_OVT_DURATION]));
            overtimeDetail.setEmployee_num(rs.getString(fieldNames[FLD_EMPLOYEE_NUM]));
            overtimeDetail.setOvt_doc_nr(rs.getString(fieldNames[FLD_OVT_DOC_NR]));
            overtimeDetail.setPay_slip_id(rs.getLong(fieldNames[FLD_PAY_SLIP_ID]));
            overtimeDetail.setPeriodId(rs.getLong(fieldNames[FLD_PERIOD_ID]));
 ...
 */
  public static boolean checkOIDDetailByOvertrimeId(long overtimeID) {
    ...
    //dedy - 20160812
    public static Vector list4(int limitStart, int recordToGet, String whereClauseEmpTime, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        String test="";
        try {
...
//                                 + " LIKE '%"+empNum.trim()+"%'";
     ...
     public static Vector listOvertimeIn(int limitStart, int recordToGet, String oidDepartment, String fullName, Date selectedDateFrom, Date selectedDateTo, String oidSection, String empNum, String order,Vector stStatus,String oidCompany,String oidDivision) {
         String whereClauseEmpTime = "";
         //update by satrya 2012-10-15
      if(selectedDateFrom!=null && selectedDateTo!=null){
         if (selectedDateFrom.getTime() > selectedDateTo.getTime()) {
         ...
         if(!oidDepartment.equals("")){
            whereClauseEmpTime = whereClauseEmpTime + " AND "+ "dep."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT_ID]+" IN (" + oidDepartment+")";
        }
         if(!oidSection.equals("")){
            whereClauseEmpTime = whereClauseEmpTime + " AND "+ "sec."+PstEmployee.fieldNames[PstEmployee.FLD_SECTION_ID]+" IN (" + oidSection+")";
        }
        if(fullName !=null && fullName.length() > 0){
//            whereClauseEmpTime = whereClauseEmpTime + " AND "+ " emp."+PstEmployee.fieldNames[PstEmployee.FLD_FULL_NAME]
//                                 + " LIKE '%"+fullName.trim()+"%'";
...
public static int getTotalDatesOverTm(Vector dates, long periodId, long empId, double minOvtmHours){
    ...
    public static boolean checkOvertime (long employeeId, String startTime, String endTime){
        ...
        try{
	        String sql = "SELECT *"+
			     " FROM " +PstOvertimeDetail.TBL_OVERTIME_DETAIL + 
                             " WHERE " +PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_EMPLOYEE_ID]+
                             " = "+employeeId+
                             " AND (("+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_FROM]+" = '"+startTime+"'"+
                             " OR "+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_TO]+" = '"+endTime+"')"+
                             " OR ('"+startTime+"' BETWEEN "+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_FROM]+" AND "
                             + PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_TO]+" ) OR " +
                             "('"+endTime+"' BETWEEN "+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_FROM]+" AND "
                             + PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_TO]+" ))"
                             + " AND "+PstOvertime.fieldNames[PstOvertime.FLD_STATUS_DOC] + "!="+I_DocStatus.DOCUMENT_STATUS_CANCELLED;
    ...
    public static boolean checkLateApproval (long employeeId, Date presenceDate){
        boolean status = false;
        Calendar cal = Calendar.getInstance();
        cal.setTime(presenceDate);
        ...
        try{
	        String sql = "SELECT *"+
			     " FROM " +PstOvertimeDetail.TBL_OVERTIME_DETAIL + 
                             " WHERE DATE_FORMAT("+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_FROM]+",'%Y-%m-%d') = "
                             + "'" +Formater.formatDate(cal.getTime(), "yyyy-MM-dd")+"'"
                             + " AND "+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_STATUS]+" IN (2,4) "
                             + " AND "+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_EMPLOYEE_ID]+" = "+employeeId
                             + " AND "+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_LATE_APPROVAL]+" = 1";
    ...
    public static long getOvertimeId (long employeeId, String dateFrom, int typeApproval, String listPos){
        long oid = 0;
        ...
        DBResultSet dbrs = null;
        try{
	        String sql = "SELECT od.OVERTIME_ID"+
			     " FROM " +PstOvertimeDetail.TBL_OVERTIME_DETAIL + " AS od" +
                             " INNER JOIN hr_overtime AS ot ON od.`OVERTIME_ID` = ot.`OVERTIME_ID`"+
                             " INNER JOIN hr_employee AS emp ON od.`EMPLOYEE_ID` = emp.`EMPLOYEE_ID`"+
                             " WHERE DATE_FORMAT(od."+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_FROM]+",'%Y-%m-%d') = "
                             + "'" +dateFrom+"' AND ot.`DIVISION_ID` = '"+emp.getDivisionId()+"' AND ot.`OVERTIME_TYPE` =" + typeApproval
                             + " AND ot.`OBJECTIVE` = 'upload from excel' and ot.`STATUS_DOC` != 2";
                    
                    if (typeApproval == 0){
                        sql +=  " AND emp.`POSITION_ID` IN("+ listPos+")";
                    }         
    ...
    public static double getOvertimeTotal(long employeeId, String startDate, String endDate){
        double total=0.0;
        DBResultSet dbrs = null;
        try{
	        String sql = "SELECT SUM("+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_TOT_IDX]+")"+
			     " FROM " +PstOvertimeDetail.TBL_OVERTIME_DETAIL +
                             " WHERE " +PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_EMPLOYEE_ID]+" = "+ employeeId +
                             " AND " +PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_DATE_FROM] + " BETWEEN '"
                             + startDate+ " 00:00:00' AND '"+endDate+" 23:59:00' AND "+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_STATUS]+"=4" ;
	...
	public static String listEmpReport(int limitStart, int recordToGet, String whereClauseEmpTime, String order) {
		return listEmpReport(limitStart, recordToGet, whereClauseEmpTime, order, 0);
	}
    
    public static String listEmpReport(int limitStart, int recordToGet, String whereClauseEmpTime, String order, long payPeriodId) {
        String listEmployee = "";
        DBResultSet dbrs = null;
        String test="";
        try {
            String sql = "SELECT DISTINCT (odt."+PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_EMPLOYEE_ID]
                    +") FROM " + PstOvertimeDetail.TBL_OVERTIME_DETAIL
                    + " as odt INNER JOIN " + PstEmployee.TBL_HR_EMPLOYEE + " as emp ON odt." + PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_EMPLOYEE_ID] + "= emp."
                    + PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID] + " INNER JOIN " + PstPaySlip.TBL_PAY_SLIP + " as ps ON emp." 
					+ PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID] + "= ps."
                    + PstPaySlip.fieldNames[PstPaySlip.FLD_EMPLOYEE_ID] + " AND ps."+ PstPaySlip.fieldNames[PstPaySlip.FLD_PERIOD_ID] + " = "+payPeriodId;
            if (whereClauseEmpTime != null && whereClauseEmpTime.length() > 0) {
                sql = sql + " WHERE " + whereClauseEmpTime;
            }
            sql = sql + " GROUP BY " + PstOvertimeDetail.fieldNames[PstOvertimeDetail.FLD_OVERTIME_DETAIL_ID];
           ...
           // System.out.println("sql overtime detail " + sql);
            ...
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                String oidEmployee = rs.getString("EMPLOYEE_ID");
                listEmployee = listEmployee + "," + oidEmployee;
            }
            rs.close();
            listEmployee = listEmployee.substring(1);
            return listEmployee;
    ...
    public static synchronized long updateUploadFileNameByOid(long actualId, String fileName) throws DBException {
           OvertimeDetail entOvertimeDetail = new OvertimeDetail();
        try {
             entOvertimeDetail = (OvertimeDetail) PstOvertimeDetail.fetchExc(actualId);
                entOvertimeDetail.setUploadFile(fileName);
                updateExc(entOvertimeDetail);
        ...
        return entOvertimeDetail.getOID();
    }

...
public class PstSpecialUnpaidLeaveTaken extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language{
   ...
   public static Vector listJoinSymbol(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SP.* FROM " + TBL_SPECIAL_UNPAID_LEAVE_TAKEN+" SP "
					+ "INNER JOIN "+PstScheduleSymbol.TBL_HR_SCHEDULE_SYMBOL+" SYM "
					+ "ON SP."+fieldNames[FLD_SCHEDULED_ID]+" = SYM."+PstScheduleSymbol.fieldNames[PstScheduleSymbol.FLD_SCHEDULE];
            if (whereClause != null && whereClause.length() > 0)
                sql = sql + " WHERE " + whereClause;
            if (order != null && order.length() > 0)
                sql = sql + " ORDER BY " + order;
    ...
    public static boolean UnpaidLeaveToday(long empId, Date scheduleDate) {
		...
		try {
			String sql = "SELECT hla."+ PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_LEAVE_APPLICATION_ID] + ", has."+PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_SCHEDULED_ID] +" FROM " + PstLeaveApplication.TBL_LEAVE_APPLICATION + " hla ";
                                ...
                                sql = sql + " WHERE (`hla`.`DOC_STATUS` >=2 AND `hla`.`DOC_STATUS` NOT IN (4,6)) AND hla.`EMPLOYEE_ID` = " + empId + " AND (\"" +  Formater.formatDate(scheduleDate, "yyyy-MM-dd") + "\" BETWEEN DATE_FORMAT(has.`TAKEN_DATE`,'%Y-%m-%d') AND DATE_FORMAT(has.`TAKEN_FINNISH_DATE`,'%Y-%m-%d') )";
...
public static float getSumQtySpcUnpLeaveTkn(String whereClause){
    ...
    public static boolean getHaveTakenExecuteForm(Date dateFrom, long empId) {
        ...
        try {
            String sql = "SELECT hsult.* FROM `hr_special_unpaid_leave_taken` hsult INNER JOIN `hr_leave_application` hla ON hla.`LEAVE_APPLICATION_ID` = `hsult`.`LEAVE_APPLICATION_ID`"; 
                   sql += "WHERE `hsult`.`EMPLOYEE_ID` = "+empId+" AND ( \""+ Formater.formatDate(dateFrom, "yyyy-MM-dd HH:mm:ss") +"\" BETWEEN `hsult`.`TAKEN_DATE` AND `hsult`.`TAKEN_FINNISH_DATE`  )  AND `hla`.`DOC_STATUS` = 3 ";
    ...
    public static boolean getHaveTakenExecuteForm2(Date dateFrom, Date dateTo, long empId, String oidSchedule) {
        ...
        try {
             sql  = "SELECT hsult.* FROM `hr_special_unpaid_leave_taken` hsult INNER JOIN `hr_leave_application` hla ON hla.`LEAVE_APPLICATION_ID` = `hsult`.`LEAVE_APPLICATION_ID`"; 
                   sql += "WHERE `hsult`.`EMPLOYEE_ID` = "+empId+" AND ( \""+ Formater.formatDate(dateFrom, "yyyy-MM-dd") +"\"  BETWEEN LEFT(`hsult`.`TAKEN_DATE`,10)  AND LEFT(`hsult`.`TAKEN_FINNISH_DATE`,10) )  AND `hla`.`DOC_STATUS` "+ docStatus+" ";
    ...
    public static boolean getHaveTakenExecuteFormExclude(Date dateFrom, Date dateTo, long empId, String oidSchedule) {
        DBResultSet dbrs = null;
        boolean ada = false;
        if ((oidSchedule.equals("Not initialized")) || oidSchedule.length() == 0 ){
            return ada;
        ...
        try {
             sql  = "SELECT hsult.* FROM `hr_special_unpaid_leave_taken` hsult INNER JOIN `hr_leave_application` hla ON hla.`LEAVE_APPLICATION_ID` = `hsult`.`LEAVE_APPLICATION_ID`"; 
                   sql += "WHERE `hsult`.`EMPLOYEE_ID` = "+empId+" AND ( \""+ Formater.formatDate(dateFrom, "yyyy-MM-dd") +"\"  BETWEEN LEFT(`hsult`.`TAKEN_DATE`,10)  AND LEFT(`hsult`.`TAKEN_FINNISH_DATE`,10) )  AND `hla`.`DOC_STATUS` "+ docStatus+" ";
            if (oidSchedule.length() > 0 && !(oidSchedule.equals("Not initialized")) ){
                   sql += "AND hsult.`SCHEDULED_ID` NOT IN ("+oidSchedule+")";
            }
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
...
/**
    ...
    //add by Eri Yudi 20200912
     public static boolean getWFHLeaveFormApproved(Date dateForm, long empId) {
        DBResultSet dbrs = null;
        boolean haveWFHFormApproved = false;
        try {
            String WFHSymbol = PstSystemProperty.getValueByName("SCHEDULE_WFH_SYMBOL");
            String sql = "SELECT lvd.* FROM "+TBL_SPECIAL_UNPAID_LEAVE_TAKEN+" lvd"
                    + " INNER JOIN `"+PstScheduleSymbol.TBL_HR_SCHEDULE_SYMBOL+"` sch"
                    + " ON sch.`"+PstScheduleSymbol.fieldNames[PstScheduleSymbol.FLD_SCHEDULE_ID]+"` = lvd.`"+PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_SCHEDULED_ID]+"` "
                    + " INNER JOIN `"+PstLeaveApplication.TBL_LEAVE_APPLICATION+"` lv " 
                    + " ON lv.`"+PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_LEAVE_APPLICATION_ID]+"` = lvd.`"+PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_LEAVE_APLICATION_ID]+"` "; 
                 
            
             String whereClause = " WHERE lvd.`"+PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_EMPLOYEE_ID]+"` = '"+empId+"' " +
                                    "  AND sch.`"+PstScheduleSymbol.fieldNames[PstScheduleSymbol.FLD_SYMBOL]+"` IN ("+WFHSymbol+") " +
                                    "  AND lv.`"+PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_DOC_STATUS]+"` IN ( '"+PstLeaveApplication.FLD_STATUS_APPlICATION_APPROVED+"','"+PstLeaveApplication.FLD_STATUS_APPlICATION_EXECUTED+"') " +
                                    "  AND '"+Formater.formatDate(dateForm,"yyyy-MM-dd HH:mm:ss")+"' BETWEEN DATE_FORMAT(lvd.`"+PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_TAKEN_DATE]+"`,'%Y-%m-%d 00:00:00') " +
                                    "  AND DATE_FORMAT(lvd.`"+PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_TAKEN_FINNISH_DATE]+"`,'%Y-%m-%d 23:59:59')  ";
             String orderClause = " ORDER BY "+PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_SPECIAL_UNPAID_LEAVE_TAKEN_ID]+"  ";
             
             sql+=whereClause;
             sql+=orderClause;
            //System.out.println("SQl Eri :"+sql);
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();

            while (rs.next()) {
                haveWFHFormApproved = true;
}

            rs.close();
            return haveWFHFormApproved;
        } catch (Exception e) {
            return haveWFHFormApproved;
        } finally {
            DBResultSet.close(dbrs);

...
//            System.out.println("SQL existLlStockTaken : " + stSQL);
     ...
     * @return
     */    
    public static boolean existLlStockTaken2(long employeeId, Date takenDate)
    {
        boolean result = false;        
        DBResultSet dbrs = null;    
        String strTakenDate = "\"" + com.dimata.util.Formater.formatDate(takenDate, "yyyy-MM-dd") + "\"";
        String stSQL = "SELECT " + PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_LL_STOCK_TAKEN_ID] +
                       ...
                       " WHERE " + PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_EMPLOYEE_ID] +
                       " = " + employeeId +
                       " AND " + strTakenDate + " BETWEEN "+ PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_TAKEN_DATE] +
                       " AND " + PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_TAKEN_FINNISH_DATE];
        try
        {
//            System.out.println("SQL existLlStockTaken : " + stSQL);
            dbrs = DBHandler.execQueryResult(stSQL);
     ...
     * @return
     */    
    public static boolean existLlStockTakenByLeaveStatus(long employeeId, Date takenDate)
    {
        boolean result = false;        
        DBResultSet dbrs = null;    
        String strTakenDate = "\"" + com.dimata.util.Formater.formatDate(takenDate, "yyyy-MM-dd") + "\"";
        String stSQL = "SELECT LL." + PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_LL_STOCK_TAKEN_ID] +
                       " FROM " + PstLlStockTaken.TBL_HR_LL_STOCK_TAKEN + " AS LL" +
                       " INNER JOIN "+ PstLeaveApplication.TBL_LEAVE_APPLICATION + " AS LV " +
                       " ON LL."+PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_LEAVE_APPLICATION_ID]+
                       " = LV."+PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_LEAVE_APPLICATION_ID] +
                       " WHERE LL." + PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_EMPLOYEE_ID] +
                       " = " + employeeId +
                       " AND " + strTakenDate + " BETWEEN "+ PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_TAKEN_DATE] +
                       " AND " + PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_TAKEN_FINNISH_DATE]+
...
//            System.out.println("sql getSum  "+sql);
   ...
   /**
    ...
    public static Vector getLlTakenQty(long llStockId){
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_HR_LL_STOCK_TAKEN;
            sql += " WHERE LL_STOCK_ID="+llStockId+" AND taken_qty >=15;";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while(rs.next()) {
                LlStockTaken llStockTaken = new LlStockTaken();
    ...
    public static double getPreviousLeaveQty(long alStockId, Date takenDate){
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM(TAKEN_QTY) AS TAKEN FROM hr_ll_stock_taken ll "
                        + "INNER JOIN `hr_leave_application` lv "
                        + "ON ll.`LEAVE_APPLICATION_ID` = lv.`LEAVE_APPLICATION_ID` "
                    + "WHERE ll_stock_id = '"+alStockId+"' "

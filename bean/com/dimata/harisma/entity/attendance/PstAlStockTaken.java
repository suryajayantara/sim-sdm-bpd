...
public class PstAlStockTaken extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static void resultToObject(ResultSet rs, AlStockTaken alStockTaken) {
        try {
            ...
            alStockTaken.setTakenDate(
                    ...
                    ? DBHandler.convertDate(rs.getDate(PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_TAKEN_DATE]),
            ...
            alStockTaken.setPaidDate(
                    ...
                    ? DBHandler.convertDate(rs.getDate(PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_PAID_DATE]),
            ...
            alStockTaken.setTakenFinnishDate(
                    ...
                    ? DBHandler.convertDate(rs.getDate(PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_TAKEN_FINNISH_DATE]),
    ...
    public static boolean getHaveTakenExecuteForm2(Date dateFrom, Date dateTo, long empId, String oidSchedule) {
        ...
        try {
            String sql  = "SELECT hast.* FROM `hr_al_stock_taken` hast INNER JOIN `hr_leave_application` hla ON hla.`LEAVE_APPLICATION_ID` = `hast`.`LEAVE_APPLICATION_ID`"; 
                   sql += "WHERE `hast`.`EMPLOYEE_ID` = "+empId+" AND ( \""+ Formater.formatDate(dateFrom, "yyyy-MM-dd") +"\"  BETWEEN LEFT(`hast`.`TAKEN_DATE`,11)  AND LEFT(`hast`.`TAKEN_FINNISH_DATE`,11) )  AND `hla`.`DOC_STATUS` > 2 ";
    ...
    public static boolean getHaveTakenExecuteFormWithTypeLeaveCategory(Date dateFrom, Date dateTo, long empId, int type) {
        DBResultSet dbrs = null;
        DBResultSet dbrs2 = null;
        boolean ada = false;
        String ClientName = "";
        try {
            ClientName = String.valueOf(com.dimata.system.entity.PstSystemProperty.getValueByName("CLIENT_NAME"));//menambahkan system properties
        } catch (Exception e) {
        ...
        try {
            String sql  = "SELECT hast.* FROM `hr_al_stock_taken` hast INNER JOIN `hr_leave_application` hla ON hla.`LEAVE_APPLICATION_ID` = `hast`.`LEAVE_APPLICATION_ID`"; 
                   sql += "WHERE `hast`.`EMPLOYEE_ID` = "+empId+" AND ( \""+ Formater.formatDate(dateFrom, "yyyy-MM-dd") +"\"  BETWEEN LEFT(`hast`.`TAKEN_DATE`,10)  AND LEFT(`hast`.`TAKEN_FINNISH_DATE`,10) )  AND `hla`.`DOC_STATUS` "+docStatus+" AND `hla`.`TYPE_LEAVE_CATEGORY` = "+type;
            ...
            String sql2  = "SELECT hlst.* FROM `hr_ll_stock_taken` hlst INNER JOIN `hr_leave_application` hla ON hla.`LEAVE_APPLICATION_ID` = `hlst`.`LEAVE_APPLICATION_ID`"; 
                   sql2 += "WHERE `hlst`.`EMPLOYEE_ID` = "+empId+" AND ( \""+ Formater.formatDate(dateFrom, "yyyy-MM-dd") +"\"  BETWEEN LEFT(`hlst`.`TAKEN_DATE`,10)  AND LEFT(`hlst`.`TAKEN_FINNISH_DATE`,10) )  AND `hla`.`DOC_STATUS` "+docStatus+" AND `hla`.`TYPE_LEAVE_CATEGORY` = "+type;
...
//            System.out.println("SQL existAlStockTaken : " + stSQL);
    ...
    public static boolean existAlStockTaken1(long employeeId, Date takenDate) {
        boolean result = false;
        DBResultSet dbrs = null;
        String strTakenDate = "\"" + com.dimata.util.Formater.formatDate(takenDate, "yyyy-MM-dd") + "\"";
        String stSQL = "SELECT " + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_AL_STOCK_TAKEN_ID]
                ...
                + " WHERE " + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_EMPLOYEE_ID]
                + " = " + employeeId
                + " AND " + strTakenDate + " BETWEEN " + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_TAKEN_DATE]
                + " AND " + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_TAKEN_FINNISH_DATE];
        try {
//            System.out.println("SQL existAlStockTaken : " + stSQL);
            dbrs = DBHandler.execQueryResult(stSQL);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static boolean existAlStockTakenByCutiPenting(long employeeId, Date startDate, Date endDate) {
        boolean result = false;
        DBResultSet dbrs = null;
        String strTakenDate = "\"" + com.dimata.util.Formater.formatDate(startDate, "yyyy-MM-dd") + "\"";
        String endTakenDate = "\"" + com.dimata.util.Formater.formatDate(endDate, "yyyy-MM-dd") + "\"";
        String stSQL = "SELECT AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_AL_STOCK_TAKEN_ID]
                ...
                + " INNER JOIN " + PstLeaveApplication.TBL_LEAVE_APPLICATION + " AS LA ON LA." +PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_LEAVE_APPLICATION_ID] + " = AL."+PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_LEAVE_APPLICATION_ID] 
                + " WHERE AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_EMPLOYEE_ID]
                + " = " + employeeId
                + " AND LA." + PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_TYPE_LEAVE_CATEGORY]
                + " = " + 2
                + " AND AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_TAKEN_DATE]
                + " BETWEEN " + strTakenDate + " AND " +endTakenDate;
        try {
//            System.out.println("SQL existAlStockTaken : " + stSQL);
            dbrs = DBHandler.execQueryResult(stSQL);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static boolean existAlStockTakenByLeaveStatus(long employeeId, Date takenDate) {
        boolean result = false;
        DBResultSet dbrs = null;
        String strTakenDate = "\"" + com.dimata.util.Formater.formatDate(takenDate, "yyyy-MM-dd") + "\"";
        String stSQL = "SELECT AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_AL_STOCK_TAKEN_ID] 
                + " FROM " + PstAlStockTaken.TBL_HR_AL_STOCK_TAKEN + " AS AL"
                + " INNER JOIN "+PstLeaveApplication.TBL_LEAVE_APPLICATION+" AS LV "
                + " ON AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_LEAVE_APPLICATION_ID] + " = LV." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_LEAVE_APPLICATION_ID]
                + " WHERE AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_EMPLOYEE_ID]
                + " = " + employeeId
                + " AND " + strTakenDate + " BETWEEN AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_TAKEN_DATE]
                + " AND AL." + PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_TAKEN_FINNISH_DATE]+
                " AND LV." + PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_DOC_STATUS] + " NOT IN (4,6) " ;
        try {
//            System.out.println("SQL existAlStockTaken : " + stSQL);
            dbrs = DBHandler.execQueryResult(stSQL);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static Vector getAlTakenQty(long alStockId) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_HR_AL_STOCK_TAKEN;
            sql += " WHERE AL_STOCK_ID="+alStockId+" AND taken_qty >=3";
    ...
    public static double getPreviousLeaveQty(long alStockId, Date takenDate){
        double prevQty = 0.0;
        
        
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM(TAKEN_QTY) AS TAKEN FROM hr_al_stock_taken al "
                        + "INNER JOIN `hr_leave_application` lv "
                        + "ON al.`LEAVE_APPLICATION_ID` = lv.`LEAVE_APPLICATION_ID` "
                    + "WHERE al_stock_id = '"+alStockId+"' "
                        + "AND TAKEN_DATE < '"+Formater.formatDate(takenDate, "yyyy-MM-dd")+"' "
                        + "AND lv.DOC_STATUS IN (2,3)";

            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                prevQty = rs.getDouble(1);
            }
            rs.close();

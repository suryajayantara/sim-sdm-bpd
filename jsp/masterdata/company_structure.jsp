<%-- 
    ...
    Created on : Nov 27, 2019, 2:26:22 PM
    Author     : IanRizky
--%>
...
<div class="divinput">
    <select class="chosen-select" name="<%=frmCompany%>" id="company" onchange="javascript:loadDivision(this.value, <%=strFieldNames%>)">
...
<div class="divinput">
    <select class="chosen-select" name="<%=frmDivision%>" id="division" onchange="javascript:loadDepartment('<%=companyId%>', this.value, <%=strFieldNames%>)">
        <option value="0">-select-</option>
        <%
        if(companyId != 0){
            String whereDiv = PstDivision.fieldNames[PstDivision.FLD_COMPANY_ID]+"="+companyId
					+" AND "+PstDivision.fieldNames[PstDivision.FLD_VALID_STATUS]+"=1";
...
<div class="divinput">
    <select class="chosen-select" name="<%=frmDepartment%>" id="department" onchange="javascript:loadSection('<%=companyId%>','<%=divisionId%>',this.value, <%=strFieldNames%>)">
        <option value="0">-select-</option>
        <%
        if (divisionId != 0){
...
<div class="divinput">
    <select class="chosen-select" name="<%=frmSection%>" id="section">
        <option value="0">-select-</option>
        <%
        if (departmentId != 0){

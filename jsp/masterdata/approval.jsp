...
*/
    ...
    public String getApproval(long empId, int inc, long kpiId, long userLoggin, int maxApproval, long divisionId){
        String str = "";
        ...
        int fieldApp = 0;
        try {
            kpiTarget = PstKpiTarget.fetchExc(kpiId);
            fieldApp = getFieldApproval(kpiTarget.getApproval1(), 
            kpiTarget.getApproval2(), kpiTarget.getApproval3(), 
            kpiTarget.getApproval4(), kpiTarget.getApproval5(), 
            kpiTarget.getApproval6());
        } catch(Exception e){
                ...
                if (listEmployeeDivisionTopLink.size() > 0){
                    for (int i=0;i< listEmployeeDivisionTopLink.size(); i++){
                        ...
                        if (empApprovalStatus == 1){
                            ...
                            if (listEmpReplacement.size() > 0){
                                ...
                                str = getApproval(emp.getOID(),inc, kpiId, userLoggin, maxApproval, divisionId);
                                break;
                            }
                        } 
                        int loop = inc;
                        ...
                        if (emp.getOID() != userLoggin){
                            ...
                            str = "<a class=\"btn-grey\" style=\"color:#FFF\" href=\"javascript:cmdViewApproval('"+kpiId+"')\">View</a>";
                        } else {
                            /* Jika employeeId == useLoggin */
                            if (kpiTarget.getOID() != 0){
                                if (kpiTarget.getApproval1() == userLoggin 
                                || kpiTarget.getApproval2() == userLoggin 
                                || kpiTarget.getApproval3() == userLoggin
                                || kpiTarget.getApproval4() == userLoggin
                                || kpiTarget.getApproval5() == userLoggin
                                || kpiTarget.getApproval6() == userLoggin){
                                    str = "<a class=\"btn-grey\" style=\"color:#FFF\" href=\"javascript:cmdViewApproval('"+kpiId+"')\">View</a>";
                                } else if(oidEmpReal != 0 && oidEmpReal != emp.getOID()) {
                                        int check = checkNextApproval(oidEmpReal, maxApproval, divisionId, kpiId);
                                        str = "<a class=\"btn-green\" style=\"color:#38a109\" href=\"javascript:cmdApprove('"+oidEmpReal+"','"+kpiId+"','"+fieldApp+"','"+check+"','"+userLoggin+"')\">Approve</a>";
                                        break;    
                                } else {
                                        int check = checkNextApproval(userLoggin, maxApproval, divisionId, kpiId);
                                        str = "<a class=\"btn-green\" style=\"color:#38a109\" href=\"javascript:cmdApprove('"+userLoggin+"','"+kpiId+"','"+fieldApp+"','"+check+"','0')\">Approve</a>";
                                        break;
                                    }
                                }
                            }
                    ...
                    if (listEmployeeDivisionTopLink.size() > 0){
                        for (int i=0;i< listEmployeeDivisionTopLink.size(); i++){
                            ...
                            if (emp.getOID() == userLoggin){
                                    str = "<a class=\"btn-green\" style=\"color:#38a109\" href=\"javascript:cmdApprove('"+userLoggin+"','"+kpiId+"','"+fieldApp+"','2','0')\">Approve</a>";
                                    break;
                            } else {
                                str = "<a class=\"btn-grey\" style=\"color:#FFF\" href=\"javascript:cmdViewApproval('"+kpiId+"')\">View</a>";
                            }
...
<%
    ...
    long targetId = FRMQueryString.requestLong(request, "target_id");
    ...
    if (iCommand == Command.APPROVE){
        try {
            ...
            String sql = "UPDATE hr_kpi_target SET "+oidFieldApp+"="+employeeId+", "+dateFieldApp+"='"+dateNow+"'";
            if (checkApproval != 1){
                sql += ", status_doc=2 ";
            }
            sql += " WHERE kpi_target_id="+targetId;
            DBHandler.execUpdate(sql);
            if (checkApproval != 1){
                    String where = "TARGET."+PstKpiTarget.fieldNames[PstKpiTarget.FLD_KPI_TARGET_ID]+"="+targetId;
                    Vector listKpiEmployee = PstKpiTargetDetailEmployee.listJoin(0, 0, where, "");
                    if (listKpiEmployee.size()>0){
                            for (int x = 0; x < listKpiEmployee.size(); x++){
                                    KpiTargetDetailEmployee employee = (KpiTargetDetailEmployee) listKpiEmployee.get(x);
                                    KpiTargetDetail detail = new KpiTargetDetail();
                                    try {
                                            detail = PstKpiTargetDetail.fetchExc(employee.getKpiTargetDetailId());
                                    } catch (Exception exc){}
                                    
                                    String whereEmpTarget = PstKPI_Employee_Target.fieldNames[PstKPI_Employee_Target.FLD_EMPLOYEE_ID]+"="+employee.getEmployeeId()
                                            +" AND "+PstKPI_Employee_Target.fieldNames[PstKPI_Employee_Target.FLD_KPI_TARGET_DETAIL_ID]+"="+detail.getOID()
                                            +" AND "+PstKPI_Employee_Target.fieldNames[PstKPI_Employee_Target.FLD_KPI_LIST_ID]+"="+detail.getKpiId();
                                    Vector listEmpTarget = PstKPI_Employee_Target.list(0, 0, whereEmpTarget, "");
                                    if (listEmpTarget.size()>0){
                                        KPI_Employee_Target empTarget = (KPI_Employee_Target) listEmpTarget.get(0);
                                        empTarget.setStartDate(detail.getDateFrom());
                                        empTarget.setEndDate(detail.getDateTo());
                                        empTarget.setTarget(detail.getAmount());
                                        try {
                                                PstKPI_Employee_Target.updateExc(empTarget);
                                        } catch (Exception exc){}
                                    } else {
                                    
                                        KPI_Employee_Target empTarget = new KPI_Employee_Target();
                                        empTarget.setEmployeeId(employee.getEmployeeId());;
                                        empTarget.setKpiListId(detail.getKpiId());
                                        empTarget.setStartDate(detail.getDateFrom());
                                        empTarget.setEndDate(detail.getDateTo());
                                        empTarget.setTarget(detail.getAmount());
                                        empTarget.setKpiTargetDetailId(detail.getOID());
                                        try {
                                                PstKPI_Employee_Target.insertExc(empTarget);
                                        } catch (Exception exc){}
                                    }
                            }
                    }
    ...
    String whereClause = PstKpiTarget.getKPIFilter(emplx.getOID(), emplx.getDivisionId());
    if (whereClause.length()>0){
        whereClause = "hr_kpi_target.kpi_target_id IN(" + whereClause.substring(0, whereClause.length()-1) + ")";
        kpiList = PstKpiTarget.listKpi(whereClause);
    }
                ...
                <table class="tblStyle">
                    ...
                    <%
                            ...
                            for (int i=0; i<kpiList.size(); i++){
                                String[] data = (String[])kpiList.get(i);
                                String where = PstKpiTarget.fieldNames[PstKpiTarget.FLD_KPI_TARGET_ID]+"="+data[0];
                                ...
                                <tr>
									...
									<td style="background-color: #FFF;"><%= data[11] %></td>

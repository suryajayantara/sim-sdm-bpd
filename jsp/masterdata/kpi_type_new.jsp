...
<%!
...

	{
		...
		ctrlist.setHeaderStyle("listgentitle");
                ctrlist.addHeader("Type Name","20%");
                ctrlist.addHeader("Company &nbsp; &nbsp;| &nbsp; &nbsp; Division","20%");
...
//                }
		for (int i = 0; i < objectClass.size(); i++) {
			 ...
			 if(index == i && (iCommand == Command.EDIT || iCommand == Command.ASK)){
                                ...
                                Hashtable listMapDivision = PstKpiTypeDivision.listHashTable(0,0,whereClause,"");
                                ...
                                if(listDivision.size() > 0 ){
                                            for(int id = 0; id < listDivision.size(); id++){
                                             Division objDivision = (Division) listDivision.get(id);
				...
				rowx.add("<input type=\"text\" name=\""+frmObject.fieldNames[FrmKPI_Type.FRM_FIELD_TYPE_NAME] +"\" value=\""+kPI_Type.getType_name()+"\" class=\"elemenForm\"> * ");
                               rowx.add("<table><tr><td><select name=\"company\" class=\"select2\" multiple=\"multiple\" data-placeholder=\"Select...\"width: 100%;\">"+optionCompany+"</select></td>"+"<td> <select name=\"company\" class=\"select2\" multiple=\"multiple\" data-placeholder=\"Select...\"width: 100%;\">"+optionDivision+"</select></td> </tr> </table>" );
                                ...
                                if(listMappedCompany.size() > 0 ){
                                    ...
                                    for(int xu = 0; xu < listMappedCompany.size(); xu++){
                                     Company mapCompany = (Company) listMappedCompany.get(xu);
                                            ...
                                            Vector<String> listDevision = new Vector(); // diisi list 
                                            listDevision.add("Div X"); 
                                            listDevision.add("Div Y"); // di hapus kalau sudah panggil list
                                            if(listDevision!=null && listDevision.size()>0){
                                            for(int id=0;id<listDevision.size();id++){
                                                
                                                    divisionNames=divisionNames+(id==0 ? " : ":", ")+listDevision.get(id);
                                                }

                                            }
                                            companyName=companyName+"<td>"+mapCompany.getCompany()+"</td>";
                                            companyName=companyName+"<td>"+divisionNames+"<href=''> ...</href></td>";
                 ...
                 if(iCommand == Command.ADD || (iCommand == Command.SAVE && frmObject.errorSize() > 0)){ 
                        ...
                        for(int id = 0; id < listDivision.size(); id++){
                            ...
                            optionDivision+="<option value="+objDivision.getOID()+">"+objDivision.getDivision()+"</option>";
                                ...
                                rowx.add("<table><tr><td> <select name=\"company\" class=\"select2\" multiple=\"multiple\" data-placeholder=\"Select...\"width: 100%;\">"+optionCompany+"</select></td>"+"<td><select name=\"division\" class=\"select2\" multiple=\"multiple\" data-placeholder=\"Select...\"width: 100%;\">"+optionDivision+"</select></td></tr></table>" ); 
...
if(iCommand == Command.SAVE){
    ...
    if(oid_division != null){
        for(int ux = 0 ; ux < oid_division.length;ux++){
            ...
            objMapKpiTypeDiv.setKpiTypeId(oidKPI_Type);

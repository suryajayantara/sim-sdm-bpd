...
<%  
    ...
    String[] arrPosition = FRMQueryString.requestStringValues(request, "positionId");
    ...
    if (template.getStartDate()!= null){
        strStartDate = ""+template.getStartDate();
        strEndDate = ""+template.getEndDate();
    } else {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        Date now = new Date();
        strStartDate = sdf.format(now);
        strEndDate = sdf.format(now);
    }
    
    if (iCommand == Command.SAVE && arrPosition != null){
        /* hapus dulu jika sudah ada data */
        String whereAccess = PstStructurePositionAccess.fieldNames[PstStructurePositionAccess.FLD_STRUCTURE_TEMPLATE_ID]+"="+oidTemplate;
        Vector listAccess = PstStructurePositionAccess.list(0, 0, whereAccess, "");
        if (listAccess != null && listAccess.size() > 0){
            for (int i=0; i < listAccess.size(); i++){
                StructurePositionAccess access = (StructurePositionAccess) listAccess.get(i);
                try {
                    PstStructurePositionAccess.deleteExc(access.getOID());
                } catch (Exception exc){}
            }
        }
        /* save ke mapping */
        for (int i=0; i < arrPosition.length; i++){
            StructurePositionAccess access = new StructurePositionAccess();
            access.setTemplateId(oidTemplate);
            access.setPositionId(Long.valueOf(arrPosition[i]));
            try {
                PstStructurePositionAccess.insertExc(access);
            } catch (Exception exc){}
        }
    }
%>
...
$(function() {
                                                        ...
                                                        <form name="<%=FrmStructureTemplate.FRM_NAME_STRUCTURE_TEMPLATE%>" method="post" action="">
                                                            ...
                                                            <table>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td valign="middle"><input type="text" id="datepicker1" name="<%=frmTemplate.fieldNames[FrmStructureTemplate.FRM_FIELD_START_DATE]%>" size="50" value="<%= strStartDate %>" /></td>
                                                                </tr>
                                                                <tr>
                                                                    <td valign="middle">End Date</td>
                                                                    <td valign="middle"><input type="text" id="datepicker2" name="<%=frmTemplate.fieldNames[FrmStructureTemplate.FRM_FIELD_END_DATE]%>" size="50" value="<%= strEndDate %>" /></td>
                                                                </tr>
                                                                <tr>
                                                                    <td valign="middle">Hak Akses Jabatan</td>
                                                                    <td valign="middle">
                                                                        <%
                                                                            ...
                                                                            if (iCommand == Command.EDIT){
                                                                                ...
                                                                                Vector listAccess = PstStructurePositionAccess.list(0, 0, whereAccess, "");
                                                                                if (listAccess != null && listAccess.size() > 0){
                                                                                    arrPosition = new String[listAccess.size()];
                                                                                    for (int i=0; i < listAccess.size(); i++){
                                                                                        StructurePositionAccess access = (StructurePositionAccess) listAccess.get(i);
                                                                                        try {
                                                                                            arrPosition[i] = ""+access.getPositionId();
                                                                                        } catch (Exception exc){}
                                                                                    }
                                                                                }
                                                                            }
                                                                        %>
                                                                        <%= ControlCombo.drawStringArraySelected("positionId", "chosen-select", null, arrPosition, pos_key, pos_val, null, "multiple") %> 
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    <td colspan="2">
    ...
    <!-- #BeginEditable "script" --> <script type="text/javascript">

...
<%
    ...
    String oidTrainingActivityActual = request.getParameter("oidTrainingActivityActual");
...
<%!
    ...
    public String drawData(Vector objectClass, String oidTrainingActivityActual,String frm) {
        String str = "";
        long typ = 0;
        long grp = 0;
            for (int i = 0; i < objectClass.size(); i++) {
                ...
                if(frm.equals("frm_trainingplan")){
                    str += "<div id='divComp'><a href=\"javascript:cmdChoose('"+competency.getOID()+"','"+oidTrainingActivityActual+"')\">"+competency.getCompetencyName()+"</a></div>";
                }else{
...
<head>
    ...
    <script language="javascript">
        <%if(frm.equals("frm_trainingplan")){%>
        function cmdChoose(competencyId,oidTrainingActivityActual) {
            newWindow=window.open("../employee/training/training_competency_edit.jsp?comm="+<%=comm%>+"&competencyId="+competencyId+"&oidTrainingActivityActual="+oidTrainingActivityActual,"SelectEmployee", "height=500,width=600,status=yes,toolbar=no,menubar=no,location=center,scrollbars=yes");
            newWindow.focus();
        }
        <%}else{%>
...
<body>
    ...
    <div id="content">
        ...
        <%=drawData(listCompetency,oidTrainingActivityActual,frm)%>
        <%}else{%>
        <div>no record</div>
        <%}%>

...
<%!
    ...
    */
    ...
    /* drawList Table*/
    public String drawList(Vector listField, Vector listRecord, Vector listComp, Vector listComb, long oidCustom, int showValue, int tampilData0,Vector listRecordCompare) {
        ...
        String[][] arrFieldCompare = new String[listRecordCompare.size()+1][listField.size()];
        int[] listIdxCompare = new int[listRecord.size()+1];
        double[][] totalComp = null;
        double[][] totalCompCompare = null;
        double[][] totalCombineCompare = null;
        if(listRecordCompare.size()>0){
            totalComp = new double[listRecord.size()][listComp.size()*2]; /* for total salary component */
            totalCompCompare = new double[listRecordCompare.size()][listComp.size()]; /* for total salary component compare */
            totalCombineCompare = new double[listRecordCompare.size()][listComb.size()]; /* for total combine */
        }else{
            totalComp = new double[listRecord.size()][listComp.size()]; /* for total salary component */
        }
        
        double[][] totalCombine = new double[listRecord.size()][listComb.size()]; /* for total combine */
        String[][] arrCField = new String[listRecord.size()][listRptCF.size()];
        String[][] arrNRField = new String[listRecord.size()][listRptNR.size()];
...
//list record compare add by Eri 2020-09-30
        if (listRecordCompare != null && listRecordCompare.size()>0){
            for(int y=0; y<listRecordCompare.size(); y++){
                CustomRptDynamic dyc = (CustomRptDynamic)listRecordCompare.get(y);
                 /* Get dinamis field */
...
//                    countValueCompCompare += totalCompCompare[y][k];
                }

                /* get value field combine */
                for(int l=0; l<listComb.size(); l++){
                    CustomRptConfig rptComb = (CustomRptConfig)listComb.get(l);
                    totalCombineCompare[y][l] = convertDouble(dyc.getField(rptComb.getRptConfigFieldName()));
                }
               
                }
            }
        
//end list record compare
...
//list record
        if (listRecord != null && listRecord.size()>0){
            for(int y=0; y<listRecord.size(); y++){
                ...
                if (listField != null && listField.size() > 0){
                    for(int i=0;i<listField.size(); i++){
                        ...
                        if(rpt.getRptConfigFieldName().equals("EMPLOYEE_NUM")){
                            for(int xy = 0 ; xy < listRecordCompare.size(); xy++){
                ...
                for(int l=0; l<listComb.size(); l++){
                    CustomRptConfig rptComb = (CustomRptConfig)listComb.get(l);
                    totalCombine[y][l] = convertDouble(dyc.getField(rptComb.getRptConfigFieldName()));
                }
        ...
        for(int j=0; j<listComp.size(); j++){
            ...
            if(listRecordCompare.size()>0){
                table +="<td class='title_tbl'>"+comp.getRptConfigFieldHeader()+" (Compare)</td>";
            }
        }
        ...
        if (listRptNR != null && listRptNR.size()>0){
            CustomRptConfig rptNR = (CustomRptConfig)listRptNR.get(0);
            table +="<td class='title_tbl'>"+rptNR.getRptConfigFieldHeader()+"</td>";
        }
        ...
        for(int k=0; k<listComb.size(); k++){
            ...
            if(listRecordCompare.size()>0){
             table +="<td class='title_tbl'>"+comb.getRptConfigFieldHeader()+" (Compare)</td>";
            }
        }
        table +="</tr>";
        /* ============== record result ==============*/
        if (listRecord != null && listRecord.size()>0){
            for(int y=0; y<listRecord.size(); y++){
                if (showRecord[y] == true){
                         ...
                         if(listRecordCompare.size()>0){
                            if(listIdxCompare[y] != -1){
                                if(totalComp[y][k] == totalCompCompare[listIdxCompare[y]][k]){
                                     table +="<td>"+Formater.formatNumber(totalCompCompare[listIdxCompare[y]][k], "") /*convertInteger(totalComp[y][k])*/+"</td>";  
                                }else{
                                     table +="<td style=\"color: red\">"+Formater.formatNumber(totalCompCompare[listIdxCompare[y]][k], "") /*convertInteger(totalComp[y][k])*/+"</td>";  
                                }
                            }else{
                                if(totalComp[y][k] == 0){
                                    table +="<td>"+Formater.formatNumber(0, "") /*convertInteger(totalComp[y][k])*/+"</td>";
                                }else{
                                     table +="<td style=\"color: red\">"+Formater.formatNumber(0, "") /*convertInteger(totalComp[y][k])*/+"</td>";
                                }
                            }   
                         }
                         //end add by Eri
                    ...
                    for (int m=0; m<listRptNR.size(); m++){
                        table +="<td>"+arrNRField[y][m]+"</td>";
                    }
                    for(int l=0; l<listComb.size(); l++){
                        table +="<td>"+Formater.formatNumber(totalCombine[y][l], "") /*convertInteger(totalCombine[y][l])*/+"</td>";
                        if(listRecordCompare.size()>0){
                            if(listIdxCompare[y] != -1){
                                if(totalCombine[y][l] == totalCombineCompare[listIdxCompare[y]][l]){
                                     table +="<td>"+Formater.formatNumber(totalCombineCompare[listIdxCompare[y]][l], "") /*convertInteger(totalCombine[y][l])*/+"</td>";
                                }else{
                                     table +="<td style=\"color: red\" >"+Formater.formatNumber(totalCombineCompare[listIdxCompare[y]][l], "") /*convertInteger(totalCombine[y][l])*/+"</td>";
                                }
                            }else{
                                if(totalCombine[y][l] == 0){
                                     table +="<td>"+Formater.formatNumber(0, "") /*convertInteger(totalCombine[y][l])*/+"</td>";
                                }else{
                                     table +="<td style=\"color: red\" >"+Formater.formatNumber(0, "") /*convertInteger(totalCombine[y][l])*/+"</td>";
                                }
                            }
                        }
                    }
                    ...
                    if (ketemu == true){
                        ...
                        addColoum = false;
                        for(int b=0; b<listField.size()-(idxCount); b++){
                            table +="<td id='tdTotal'>&nbsp;</td>";
                        }
                        ...
                        for(int kolom1 = 0; kolom1<listComb.size(); kolom1++){
                            for(int x=yStart; x<yEnd; x++){
                               total += totalCombine[x][kolom1];
                           }
                           table +="<td id='tdTotal'>"+Formater.formatNumber(total, "") /*convertInteger(total)*/+"</td>";
                           total = 0;   
                        }
            ...
            for(int j=0; j<listComp.size(); j++){
                ...
                if(listRecordCompare.size()>0){
                    if(grandTotalComp == grandTotalCompCompare){
                        table +="<td class='td_grand_total'>"+ Formater.formatNumber(grandTotalCompCompare, "") + /*convertInteger(grandTotalComp)*/"</td>";
                    }else{
                         table +="<td class='td_grand_total' style=\"color: red\">"+ Formater.formatNumber(grandTotalCompCompare, "") + /*convertInteger(grandTotalComp)*/"</td>";
                    }
                }    
                grandTotalComp = 0;
            ...
            for (int xx=0; xx<listComb.size(); xx++){
                for(int y=0; y<listRecord.size(); y++){
                    grandTotalComb = grandTotalComb +  totalCombine[y][xx];
                    if(listRecordCompare.size()>0){
                        if(listIdxCompare[y] != -1){
                        grandTotalCombCompare = grandTotalCombCompare + totalCombineCompare[listIdxCompare[y]][xx] ;
                        }
                    }
                }  
                if (grandTotalComb != 0){
                    ...
                    if(listRecordCompare.size()>0){
                        if(grandTotalComb == grandTotalCombCompare){
                           table +="<td class='td_grand_total'>"+ Formater.formatNumber(grandTotalCombCompare, "") + /*convertInteger(grandTotalComp)*/"</td>";
                        }else{
                            table +="<td class='td_grand_total' style=\"color: red\">"+ Formater.formatNumber(grandTotalCombCompare, "") + /*convertInteger(grandTotalComp)*/"</td>";
                        }
                    }
                }      
                grandTotalComb = 0;
...
<%  
    ...
    int previous0 = FRMQueryString.requestInt(request, "previous_0");
    /*
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function cmdShowComparePeriod(val){
                document.form_generate.generate.value="0";
                document.form_generate.compare_gaji.value=val;
                document.form_generate.action="custom_rpt_generate.jsp";
                document.form_generate.submit();
            }
            function cmdPosted()
            ...
            function cmdPrintPdf(){
                document.form_generate.command.value="<%=Command.ADD%>"; 
                document.form_generate.generate.value="1";
                document.form_generate.action="<%=printroot%>.report.payroll.CustomReportPdf"; 
                document.form_generate.target = "ReportPdf";
                document.form_generate.submit();
            }
        </script>
                                                ...
                                                <tr>
                                                    <td valign="top">
                                                        <form name="form_generate" method="POST" action="">
                                                            ...
                                                            <input type="hidden" name="print_emp_oid" value="<%=appUserSess.getEmployeeId()%>" />
                                                            ...
                                                            <table>
                                                                ...
                                                                <tr>
                                                                    <td>
                                                                        ...
                                                                        <div>
                                                                            Tampilkan baris data jika ada nilai 0 : <input type="radio" name="tampil_data_0" checked="checked" value="0" />Yes | 
                                                                            <input type="radio" name="tampil_data_0" value="1" />No
                                                                        </div>
                                                                        <div>
                                                                            Bayarkan di tempat lama : <input type="radio" name="previous_0"  value="0" />Yes | 
                                                                            <input type="radio" name="previous_0" value="1" checked="checked"/>No
                                                                        </div>
                                                                        <div>
                                                                            Tampilkan Grand Total : <input type="radio" name="grand_total_0" checked="checked" value="0" />Yes | 
                                                                            <input type="radio" name="grand_total_0" value="1" />No
                                                                        </div>
                                                                        <div>
                                                                            Compare Gaji : 
                                                                            <% if(compare_gaji == 0){%>
                                                                            <input type="radio" name="compare_gaji"  value="1" onclick="cmdShowComparePeriod(this.value)" />Yes | 
                                                                            <input type="radio" name="compare_gaji" value="0" checked="checked" onclick="cmdShowComparePeriod(this.value)"/>No
                                                                            <%}else{%>
                                                                             <input type="radio" name="compare_gaji"  value="1" checked="checked" onclick="cmdShowComparePeriod(this.value)" />Yes | 
                                                                            <input type="radio" name="compare_gaji" value="0"  onclick="cmdShowComparePeriod(this.value)"/>No
                                                                            <%}%>
                                                                        </div>
                                                                        <div>
                                                                            Sign by : 
                                                                            <%
                                                                                ...
                                                                                if (appUserSess.getAdminStatus() != 1){
                                                                                    ...
                                                                                    try {
                                                                                        div = userSession.getDivision();
                                                                                    } catch (Exception exc){
                                                                                        System.out.println("Exception at fetch division : "+exc.toString());
                                                                                    }
                                                                                    whereSign = PstEmployee.fieldNames[PstEmployee.FLD_DIVISION_ID]+"="+div.getOID(); 
                                                                                } 
                                                                                Vector listEmpSign = PstEmployee.list(0, 0, whereSign, "");
                                                                                Vector emp_value = new Vector(1, 1);
                                                                                Vector emp_key = new Vector(1, 1);
                                                                                emp_key.add("Select Employee...");
                                                                                emp_value.add("0");
                                                                                if (listEmpSign != null){
                                                                                    for (int r = 0; r < listEmpSign.size(); r++) {
                                                                                        Employee employee = (Employee) listEmpSign.get(r);
                                                                                        emp_key.add(employee.getFullName());
                                                                                        emp_value.add(String.valueOf(employee.getOID()));
                                                                                    }
                                                                                }
                                                                            %>
                                                                            <%= ControlCombo.draw("employeeSign", "chosen-select", null, null, emp_value, emp_key, " ") %>
                                                                        </div>
                                                                    </td>
                                                                </tr>
                                                                <tr><td>&nbsp;</td></tr>
                                                                            ...
                                                                            if (listWhere != null && listWhere.size()>0){
                                                                                for(int i=0; i<listWhere.size(); i++){
                                                                                    ...
                                                                                    <tr>
                                                                                        ...
                                                                                        <td>
                                                                                            ...
                                                                                            if (isDate == true){
                                                                                                %>
                                                                                                <input type="text" name="where_value<%= i %>" value="" size="70" />
                                                                                                <%
                                                                                            } else {
                                                                                                %>
                                         
                                                                                                <select name="where_value<%= i %>"  class="chosen-select"  <% if (selection.getRptConfigFieldName().equals("DIVISION")){%> multiple="multiple"<%} %>>
                                                                                                    <option value="">-SELECT-</option>
                                                                                                    <%
                                                                                                    String sqlData = "";
                                                                                                    if (selection.getRptConfigFieldName().equals("DIVISION")){
                                                                                                        sqlData = "SELECT DISTINCT ps.DIVISION FROM pay_slip ps inner join hr_division dv on ps.`DIVISION` = dv.`DIVISION` where dv.`VALID_STATUS` = 1";
                                                                                                    } else if (selection.getRptConfigFieldName().equals("PERIOD")){
                                                                                                        sqlData = "SELECT DISTINCT "+selection.getRptConfigFieldName()+" FROM "+selection.getRptConfigTableName()+" ORDER BY START_DATE DESC";
                                                                                                    } else {
                                                                                                        sqlData = "SELECT DISTINCT "+selection.getRptConfigFieldName()+" FROM "+selection.getRptConfigTableName();
                                                                                                    }
                                                                               ...
                                                                               <%if(compare_gaji == 1){%>
                                                                                <tr>
                                                                                        ...
                                                                                        <td>
                                                                                            <input type="hidden" name="where_field_compare" value="pay_period.PERIOD" />
                                                                                            <select name="operator_compare">
                                                                                                <option value="=">=</option>
                                                                                                <option value="!=">!=</option>
                                                                                                <option value=">">></option>
                                                                                                <option value="<"><</option>
                                                                                            ...
                                                                                            <!--<input type="text" name="where_value" value="" size="70" />-->
                                                                                                ...
                                                                                                <select name="where_value_compare_0"  class="chosen-select">
                                                                                                    <option value="">-SELECT-</option>
                                                                                                    <%
                                                                                                        try{
                                                                                                        Vector listPayPeriod = (Vector) PstPayPeriod.list(0, 0, "", ""+PstPayPeriod.fieldNames[PstPayPeriod.FLD_START_DATE]+" ASC " );
                                                                                                        for(int xy = 0 ; xy < listPayPeriod.size();xy++){
                                                                                                            PayPeriod entPayPeriod = (PayPeriod) listPayPeriod.get(xy);
                                                                                                        
                                                                                                        
                                                                                                                    %>
                                                                                                                    <option value="<%=entPayPeriod.getPeriod()%>"><%= entPayPeriod.getPeriod() %></option>
                                                                                                                    <%
                                                                                                                }   
                                                                                                        }catch(Exception exc){
...
<!--                                                                        <button id="btn" onclick="cmdPrintPdf()">Print to Pdf</button>&nbsp;-->
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        */
                                                        //String whereClause = "";
                                                        //whereClause = PstCustomRptConfig.findWhereData(selectionChoose, whereField, vectWhereValue, whereType, operator, whereCustom);
                                                        /*
                                                        ...
                                                        */
                                                        ...
                                                        if (listData != null && listData.size() > 0){
                                                            ...
                                                            for(int i=0;i<listData.size(); i++){
                                                                ...
                                                                if (previous0 == 0){
                                                                    String period = "";
                                                                    if (rpt.getRptConfigTableName().equals("pay_slip") && rpt.getRptConfigFieldName().equals("DIVISION")){
                                                                        for (int x = 0; x < whereField.length; x++){
                                                                            if (whereField[x].equals("pay_period.PERIOD")){
                                                                                String[] dataWhereValue = (String[]) vectWhereValue.get(x);
                                                                                period = dataWhereValue[x];
                                                                            
                                                                                Vector listPeriodNow = PstPayPeriod.list(0,0,PstPayPeriod.fieldNames[PstPayPeriod.FLD_PERIOD] + " = '" +period+"'","" );
                                                                                if(listPeriodNow.size() > 0){
                                                                                    PayPeriod periodNow = (PayPeriod) listPeriodNow.get(0);
                                                                                    PayPeriod payPeriodPrevious =  PstPayPeriod.getPreviousPayPeriod(periodNow.getOID()); 
                                                                                    strSelect += "(SELECT ps.DIVISION FROM pay_slip ps INNER JOIN pay_period as pp ON pp.PERIOD_ID" +
                                                                                                 " = ps.PERIOD_ID INNER JOIN hr_employee as emp ON ps.EMPLOYEE_ID = emp.EMPLOYEE_ID " +
                                                                                                 " WHERE pp.PERIOD = '" +payPeriodPrevious.getPeriod()+ "' AND emp.EMPLOYEE_NUM = hr_employee.EMPLOYEE_NUM) as DIVISION"; 
                                                                                    ...
                                                                                    for (int n = 0; n < whereField.length; n++){
                                                                                        if (whereField[n].equals("pay_slip.DIVISION")){
                                                                                            whereField[n] = "(SELECT ps.DIVISION FROM pay_slip ps INNER JOIN pay_period as pp ON pp.PERIOD_ID" +
                                                                                                            " = ps.PERIOD_ID INNER JOIN hr_employee as emp ON ps.EMPLOYEE_ID = emp.EMPLOYEE_ID " +
                                                                                                            " WHERE pp.PERIOD = '" +payPeriodPrevious.getPeriod()+ "' AND emp.EMPLOYEE_NUM = hr_employee.EMPLOYEE_NUM)";
                                                                                        }
                                                                                    }
                                                        ...
                                                        */
                                                        ...
                                                        if(compare_gaji == 1){
                                                        whereClauseCompare =PstCustomRptConfig.findWhereData(selectionChoose, whereField, vectWhereValue, whereType, operator, whereCustom,whereFieldCompare,vectWhereValueCompare,operatorCompare);
                                                        }
                                                        ...
                                                        boolean checkMBT = PstCustomRptConfig.checkMBT(selectionChoose, whereField, vectWhereValue, whereType, operator, whereCustom);
                                                        /* ORDER BY */
                                                        ...
                                                        String whereUnion = PstCustomRptConfig.whereDate(selectionChoose, whereField, vectWhereValue, whereType, operator, whereCustom);
                                                        if(checkMBT){
                                                        ...
                                                        if (listSalaryComp != null && listSalaryComp.size()>0){
                                                            if(!checkMBT){
                                                                ...
                                                                listResult = PstCustomRptConfig.listData(query, listData);
                                                            }
                                                            %>
                                                            <div><%=drawList(listData, listResult, listSalaryComp, listComb, oidCustom, showValue, tampilData0,listResultCompare)%></div>
                                                            <%
                                                        } else {
                                                            if (!checkMBT){
                                                                listResult = PstCustomRptConfig.listDataWithoutPaySlip(strSelect+" FROM hr_employee "+strJoin+" "+whereClause + strOrderBy, listData);
                                                            } else {
                                                                listResult = PstCustomRptConfig.listDataWithoutPaySlip(query, listData);
                                                            }
                                                            %>

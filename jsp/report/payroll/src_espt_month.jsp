...
<%  
    ...
    if (iCommand == Command.LIST){
        String getInBenSalBrutto = PstSystemProperty.getValueByName("ESPT_RPT_TYPE_BENEFIT");
        String getInDedSalBrutto = PstSystemProperty.getValueByName("ESPT_RPT_TYPE_DEDUCTION");

        String inSalBenBrutto = "";
        String inSalDedBrutto = "";
        Vector listPayCompBen = new Vector();
        Vector listPayCompDed = new Vector();
        if(getInBenSalBrutto.equals("1")){
            String where = PstPayComponent.fieldNames[PstPayComponent.FLD_COMP_TYPE] + " = 1 AND " + PstPayComponent.fieldNames[PstPayComponent.FLD_TAX_ITEM] + " IN (1,2,3)";
            listPayCompBen = PstPayComponent.list(0, 0, where, "");
            if(listPayCompBen != null && listPayCompBen.size()>0){
                for (int i = 0; i<listPayCompBen.size();i++){
                PayComponent payComp = (PayComponent) listPayCompBen.get(i);
                inSalBenBrutto = inSalBenBrutto + payComp.getCompCode() + ",";
                }
            }
            inSalBenBrutto="'"+inSalBenBrutto.replaceAll(",", "','")+"'";
            
        } else {
        inSalBenBrutto =  PstSystemProperty.getValueByName("PAYROLL_ESPT_SALARY_BRUTTO_COMP_BEN") ;//'TI','ALW28', 'ALW02';         
        inSalBenBrutto="'"+inSalBenBrutto.replaceAll(",", "','")+"'";
        }
        
        if (getInDedSalBrutto.equals("1")){
            String where = PstPayComponent.fieldNames[PstPayComponent.FLD_COMP_TYPE] + " = 2 AND " + PstPayComponent.fieldNames[PstPayComponent.FLD_TAX_ITEM] + " IN (1,2)";
            listPayCompDed = PstPayComponent.list(0, 0, where, "");
            if(listPayCompDed != null && listPayCompDed.size()>0){
                for (int i = 0; i<listPayCompDed.size();i++){
                PayComponent payComp = (PayComponent) listPayCompDed.get(i);
                inSalDedBrutto = inSalDedBrutto + payComp.getCompCode() + ",";
                }
            }
            inSalDedBrutto="'"+inSalDedBrutto.replaceAll(",", "','")+"'";
        } else {
            inSalDedBrutto =  PstSystemProperty.getValueByName("PAYROLL_ESPT_SALARY_BRUTTO_COMP_DED") ;//'TI','ALW28', 'ALW02';         
            inSalDedBrutto="'"+inSalDedBrutto.replaceAll(",", "','")+"'";
        }
        String inPPHBrutto =  PstSystemProperty.getValueByName("PAYROLL_ESPT_PPH_COMP");
        inPPHBrutto="'"+inPPHBrutto.replaceAll(",", "','")+"'";
        listEspt = SessESPT.getListEsptMonth(periodId, divisionId, departmentId, sectionId, inSalBenBrutto, inSalDedBrutto,inPPHBrutto, inp_resignStatus);
    }
%>
<html>
    <head>
                                                        ...
                                                        <form name="frm_src_espt" method="POST" action="">
                                                            ...
                                                            <table>
                                                                ...
                                                                <%
                                                                    ...
                                                                    String whereDivision = "COMPANY_ID = " + companyId + " AND " + PstDivision.fieldNames[PstDivision.FLD_VALID_STATUS] + " = 1";
                                                                    div_value.add("0");
                                                                    div_key.add("-select-");
                                                                    String div_selected = "";
                                                                    // List Department
                                                                    ...
                                                                    String whereSection = "DEPARTMENT_ID = " + departmentId + " AND " + PstSection.fieldNames[PstSection.FLD_VALID_STATUS] + " = 1";
                                                                    section_value.add("0");

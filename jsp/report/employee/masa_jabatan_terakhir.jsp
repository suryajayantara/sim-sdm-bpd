<%-- 
    Document   : masa_jabatan_terakhir
    Created on : Jul 4, 2018, 9:58:48 AM
    Author     : dimata005
--%>
                    ...
                    if (listEmployee.size() > 0){
						switch(reportType){
							...
							for (int i=0; i<listEmployee.size();i++){
								...
								double month = DateCalc.monthDifferenceDouble(dtStart, dtSearch);
								Vector vectLvl = new Vector();
								vectLvl = PstCareerPath.getCareerHistory(emp.getOID());
								for (int x=0; x < vectLvl.size();x++){
									CareerPath careerPath = (CareerPath) vectLvl.get(x);
										if (careerPath.getPositionId() == emp.getPositionId()){
										month = month + DateCalc.monthDifferenceDouble(careerPath.getWorkFrom(), careerPath.getWorkTo());
								...
								%>
									<tr>
										...
										<td style="background-color: <%=bgColor%>;"><%=String.format("%.0f", month)%></td>
									</tr>
								<%
							}
							%>
							...
							for (int i=0; i<listEmployee.size();i++){
								...
								for (int x=0; x < vectLvl.size();x++){
									CareerPath careerPath = (CareerPath) vectLvl.get(x);
									if (careerPath.getLevelId() == emp.getLevelId()){
										month = month + DateCalc.monthDifferenceDouble(careerPath.getWorkFrom(), careerPath.getWorkTo());
									} else {
										break;
									}
							...
							for (int i=0; i<listEmployee.size();i++){
								...
								for (int x=0; x < vectLvl.size();x++){
									CareerPath careerPath = (CareerPath) vectLvl.get(x);
									if (careerPath.getDivisionId() == emp.getDivisionId()){
										month = month + DateCalc.monthDifferenceDouble(careerPath.getWorkFrom(), careerPath.getWorkTo());
										startMenjabat = ""+careerPath.getWorkFrom();
									} else {
										break;

   ...
   if (userGroupList != null && userGroupList.size() > 0) {
        for (int i = 0; i < userGroupList.size(); i++) {
            ...
            if (groupList != null && groupList.size() > 0) {
                ...
                if (appGroup.getGroupName().equals("Head") || appGroup.getGroupName().equals("Admin Cabang")){
...
<div class="divinput">
    <select name="<%=frmDivision%>" id="division" onchange="javascript:loadDepartment('<%=companyId%>', this.value, <%=strFieldNames%>)" <%=strDisable%>>
        ...
        if(companyId != 0){
            String whereDiv = "";
            if (appGroupStr.equals("Head") || appGroupStr.equals("Admin Cabang") || appGroupStr.equals("Direksi")){
                if (appGroupStr.equals("Head") || appGroupStr.equals("Admin Cabang")){
...
<div class="divinput">
    <select name="<%=frmSection%>" id="section">
        ...
        <%
        if (departmentId != 0){
            String whereSection = PstSection.fieldNames[PstSection.FLD_DEPARTMENT_ID]+"="+departmentId + " AND " + PstSection.fieldNames[PstSection.FLD_VALID_STATUS]+"="+PstSection.VALID_ACTIVE;

...
<%
    ...
    int usiaMBT = Integer.valueOf(String.valueOf(PstSystemProperty.getValueByName("UMUR_MBT")));
    /* get data end contrct by lenght day from master notification */
    Vector listEmpMBT = new Vector();
    try{
     int lenghtDay = PstNotification.checkLenghtNotifByUser(userid,PstNotification.NOTIF_KARYAWAN_MBT);
      
        //whereclause mencari list karyawan akan berulang tahun dengan interval lenght day dari hari ini
        String whereClauseMBT = " "+"STR_TO_DATE(CONCAT(YEAR(CURDATE()),'-',MONTH("+PstEmployee.fieldNames[PstEmployee.FLD_BIRTH_DATE]+"),'-',DAY("+PstEmployee.fieldNames[PstEmployee.FLD_BIRTH_DATE]+")),'%Y-%c-%e')"+
        ...
        listEmpMBT = (Vector) PstEmployee.list(0, 0, whereClauseMBT, "");
    }catch(Exception exc){
    ...
    <body>
        ...
        <div class="content-main">
            <form name="frm" method="post" action="">
                ...
                <% 
                    if (listEmpMBT != null && listEmpMBT.size()>0){
                %>
                <table class="tblStyle">
                    ...
                    <%
                      for (int i=0; i<listEmpMBT.size(); i++){
                          Employee employee = (Employee) listEmpMBT.get(i);
                          ...
                          int no = 0;
                           if((usia == (usiaMBT - 1) && bedaHari > 0) || (usia == usiaMBT && bedaHari == 0)){
                  
                    %>    
                     
                                <tr>
                                    <!--<td style="background-color: #FFF;"><input type="checkbox" name="chx" value="" /></td>-->
                                    <td style="background-color: #FFF;"><%=no+1%></td>

...
//add by Eri Yudi 2022-02-21 for privilege user cabang
if (appUserSess.getAdminStatus()==0){
     companyId = emplx.getCompanyId();
...
<div class="divinput">
    <select name="<%=frmCompany%>" id="company" onchange="javascript:loadDivision(this.value, <%=strFieldNames%>)" class="chosen-select">
...
<div class="divinput">
    <select name="<%=frmDivision%>" id="division" onchange="javascript:loadDepartment('<%=companyId%>', this.value, <%=strFieldNames%>)" class="chosen-select">
        ...
        if(companyId != 0){
			...
			if (appUserSess.getAdminStatus()==0){
				whereDiv = PstDivision.fieldNames[PstDivision.FLD_DIVISION_ID]+"="+divisionId;
			}
...
<div class="divinput">
    <select name="<%=frmDepartment%>" id="department" onchange="javascript:loadSection('<%=companyId%>','<%=divisionId%>',this.value, <%=strFieldNames%>)" class="chosen-select">
        <option value="0">-select-</option>
        <%
        if (divisionId != 0){
...
<div class="divinput">
    <select name="<%=frmSection%>" id="section" class="chosen-select">
        <option value="0">-select-</option>
        <%
        if (departmentId != 0){

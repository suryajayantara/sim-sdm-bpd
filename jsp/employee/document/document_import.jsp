...
<!DOCTYPE html>
<%
    Long docId = FRMQueryString.requestLong(request, "doc_id");
    /* Get Pay Component */
    long master =0;
    String title = "";
    String document = "";
    String docName ="";
    if (docId>0){
       String whereClause = PstEmpDoc.fieldNames[PstEmpDoc.FLD_EMP_DOC_ID]+"='"+docId+"'";
       Vector listDoc = PstEmpDoc.list(0, 1, whereClause, "");
       if (listDoc != null && listDoc.size()>0){
           EmpDoc docList = (EmpDoc)listDoc.get(0);
           master = docList.getDoc_master_id();
           title = docList.getDoc_title();
       }
       String where = PstDocMaster.fieldNames[PstDocMaster.FLD_DOC_MASTER_ID]+"='"+master+"'";
       Vector listMaster = PstDocMaster.list(0,1,where,"");
       if(listMaster != null && listMaster.size()>0){
           DocMaster docMaster = (DocMaster)listMaster.get(0);
           document = "<strong>" + docMaster.getDoc_title() +"(" + title +")</strong>";
           docName =  docMaster.getDoc_title();
       }
    }
    session.putValue("doc_title", docName);
    session.putValue("doc_id", docId);
%>
<html>
    <head>
        ...
        <script type="text/javascript">
           function cmdBack(docId){
                document.frm.action="EmpDocumentDetails.jsp?EmpDocument_oid="+docId+"";
                document.frm.submit();
            }
            function cmdDownload(){
                document.frm.action = "import_surat.xls";
    ...
    <body>
        ...
        <div id="menu_utama">
            <span id="menu_title">Document <strong style="color:#333;"> / </strong>Import </span>
            <a style="color:#F5F5F5" class="btn-small" href="javascript:cmdBack('<%=docId%>')">Back</a>
        </div>
        <div class="content-main">
            <div class="info">Make sure your sheet name is <%= document %></div>
            <form name="frm" method ="post" action="document_process.jsp" enctype="multipart/form-data">

...
public String drawList(Vector objectClass, int st){
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		lstData.add(rowx);
		lstLinkData.add(String.valueOf(employee.getOID()) + "','" + employee.getEmployeeNum() + "','" + employee.getFullName() + "','" + department.getDepartment()+ "','" + position.getPosition()+ "','" + employee.getPositionId());
	}
	return ctrlist.draw();
}
%>
...
function cmdEdit(oid, number, fullname, department, position, positionId) {
        ...
        self.opener.document.<%=formName%>.EMP_POSITION_ID.value = positionId;
        self.close();
    }
function cmdAdd() {

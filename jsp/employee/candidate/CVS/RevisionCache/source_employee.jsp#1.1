    ...
    <head>
        ...
        <script language="javascript">
            function cmdGetItem(oid, name) {
                self.opener.document.getElementById("browse").innerHTML=name;
                self.opener.document.frm.requested_by.value=oid;
                self.close();
            }
        </script>
    </head>
    <body>
        ...
        if (listEmployee != null && listEmployee.size()>0){
            for(int i=0; i<listEmployee.size(); i++){
                ...
                %>
                <div class="item" onclick="cmdGetItem('<%=employee.getOID()%>','<%=employee.getFullName()%>')"><%=employee.getFullName()%></div>
                <%
            }
        }
        %>

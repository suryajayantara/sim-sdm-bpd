...
<%!
 ...
 +                 " BETWEEN START_DATE AND END_DATE "
    ...
    public String getApproval(long empId, int maxApproval, boolean needHrApproval, long divisionId){
        String str = "";
                    ...
                    for (int i=0;i< listEmployeeDivisionTopLink.size(); i++){
                        ...
                        if (empApprovalStatus == 1){
                            ...
                            if (listEmpReplacement.size() > 0){
                                ...
                                str = str + " / " + getApproval(emp.getOID(),maxApproval, needHrApproval, divisionId);
                            }
                        } else {
...
<%
    ...
    String oidLevel = "0";
    try {
        oidLevel = PstSystemProperty.getValueByNameWithStringNull("OID_LEVEL_DIDNT_NEED_SDM");
    } catch (Exception exc){
        oidLevel = "0";
    }
    ...
    String [] oidLevelDontNeedHrApproval = oidLevel.split(",");
    long empId = 0;
    long approval1 = 0;
    long approval2 = 0;
    long approval3 = 0;
    ...
    try {
        leaveApp = PstLeaveApplication.fetchExc(leaveId);
        empId = leaveApp.getEmployeeId();
        approval1 = leaveApp.getApproval_1();
        approval2 = leaveApp.getApproval_2();
        approval3 = leaveApp.getApproval_3();
        approval4 = leaveApp.getApproval_4();
        approval5 = leaveApp.getApproval_5();
        approval6 = leaveApp.getApproval_6();
        hrApproval = leaveApp.getHrManApproval();
        repApproval1 = leaveApp.getRep_approval_1();
        repApproval2 = leaveApp.getRep_approval_2();
        repApproval3 = leaveApp.getRep_approval_3();
        repApproval4 = leaveApp.getRep_approval_4();
        repApproval5 = leaveApp.getRep_approval_5();
        repApproval6 = leaveApp.getRep_approval_6();
    ...
    if (empId != 0){
       ...
       //Level level = new Level();
        ...
        for (int i=0; i < oidLevelDontNeedHrApproval.length; i++){
            if(oidLevelDontNeedHrApproval[i].equals(""+level.getOID())){
...
<%
    if (approval1 != 0){
        if (lastApproval == 1 && leaveApp.getDocStatus() == 5){
            %>
        ...
        if (approval2 != 0){
            if (lastApproval == 2 && leaveApp.getDocStatus() == 5){
                %>
            ...
            if (approval3 != 0){
                if (lastApproval == 3 && leaveApp.getDocStatus() == 5){
                    %>
                ...
                if (approval4 != 0){
                    if (lastApproval == 4 && leaveApp.getDocStatus() == 5){
                        %>
                    ...
                    if (approval5 != 0){
                        if (lastApproval == 5 && leaveApp.getDocStatus() == 5){
                            %>
                        ...
                        if (approval6 != 0){
                            if (lastApproval == 6 && leaveApp.getDocStatus() == 5){
                                %>
                        ...
                        } else if (leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1) {
                            %>
                            <%= getApproval(approval5,maxLevel, needHrApproval, emp.getDivisionId())+" : Next<br>" %>
                            <%
                        } else if (needHrApproval  && hr_department != emp.getDepartmentId()  && ( leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1)){
                            if (hrApproval != 0){
                                %>
                                <%= getEmployeeName(hrApproval)+" : Approved<br>" %>
                                <%
                            } else {
                                %>
                                <%= getHrApproval(leaveId)+" : Next<br>" %>
                                <%
                            }
                        }
                    } else if (leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1) {
                        %>
                        <%= getApproval(approval4,maxLevel, needHrApproval, emp.getDivisionId())+" : Next<br>" %>
                        <%
                    } else if (needHrApproval  && hr_department != emp.getDepartmentId()  && ( leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1)){
                        if (hrApproval != 0){
                            %>
                    ...
                    %>
                    <%= getApproval(approval3,maxLevel,needHrApproval,emp.getDivisionId())+" : Next<br>" %>
                    <%
                } else if (needHrApproval  && hr_department != emp.getDepartmentId()  && ( leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1)){
                    if (hrApproval != 0){
                        %>
                ...
                %>
                <%= getApproval(approval2,maxLevel,needHrApproval,emp.getDivisionId())+" : Next<br>" %>
                <%
            } else if (needHrApproval  && hr_department != emp.getDepartmentId()  && ( leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1)){
                if (hrApproval != 0){
                    %>
            ...
            %>
            <%= getApproval(approval1,maxLevel,needHrApproval,emp.getDivisionId())+" : Next<br>" %>
            <%
        } else if (needHrApproval  && hr_department != emp.getDepartmentId() && ( leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1)){
            if (hrApproval != 0){
                %>
        ...
        %>
        <%= getApproval(empId,maxLevel,needHrApproval,emp.getDivisionId())+" : Next<br>" %>
        <%
    } else if (needHrApproval && hr_department != emp.getDepartmentId()  && ( leaveApp.getDocStatus() == 0 || leaveApp.getDocStatus() == 1)){
        if (hrApproval != 0){
            %>

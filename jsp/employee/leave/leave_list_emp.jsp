...
<%
	...
	if (iCommand == Command.DELETE) {
        if (leaveId != 0) {
            ...
            try{
                leave = PstLeaveApplication.fetchExc(leaveId);
            } catch (Exception exc){

            }
			
			String whereAL = PstAlStockTaken.fieldNames[PstAlStockTaken.FLD_LEAVE_APPLICATION_ID]+"="+leaveId;
			String whereLL = PstLlStockTaken.fieldNames[PstLlStockTaken.FLD_LEAVE_APPLICATION_ID]+"="+leaveId;
			String whereSL = PstSpecialUnpaidLeaveTaken.fieldNames[PstSpecialUnpaidLeaveTaken.FLD_LEAVE_APLICATION_ID]+"="+leaveId;
			
			Vector listAl = PstAlStockTaken.list(0, 0, whereAL, "");
			Vector listLl = PstLlStockTaken.list(0, 0, whereLL, "");
			Vector listSL = PstSpecialUnpaidLeaveTaken.list(0, 0, whereSL, "");
			...
			if (listAl.size()>0){
				for (int i = 0; i < listAl.size(); i++){
					...
					try {
						long oid = PstAlStockTaken.deleteExc(alStockTaken.getOID());
					} catch (Exception exc){}
				}
			}
			if (listLl.size()>0){
				for (int i = 0; i < listLl.size(); i++){
					LlStockTaken llStockTaken = (LlStockTaken) listLl.get(i);
					LLStockManagement lLStockManagement = PstLLStockManagement.fetchExc(llStockTaken.getLlStockId());
					if (leave.getDocStatus() == 3){
						lLStockManagement.setQtyUsed(lLStockManagement.getQtyUsed()-llStockTaken.getTakenQty());
						try {
							PstLLStockManagement.updateExc(lLStockManagement);
						} catch (Exception exc){}
					}
					try {
						long oid = PstLlStockTaken.deleteExc(llStockTaken.getOID());
					} catch (Exception exc){}
				}
			}
			if (listSL.size()>0){
				for (int i = 0; i < listSL.size(); i++){
					SpecialUnpaidLeaveTaken specialUnpaidLeaveTaken = (SpecialUnpaidLeaveTaken) listSL.get(i);
					try {
						long oid = PstSpecialUnpaidLeaveTaken.deleteExc(specialUnpaidLeaveTaken.getOID());
					} catch (Exception exc){}
				}
			}
			try {
...
<html>
    <head>
        ...
        <script type="text/javascript">
			...
			function cmdDelete(leaveId){
				var r = confirm("Delete Form ?");
				if (r) {
					document.frm.command.value="<%= Command.DELETE %>";
					document.frm.leave_id.value = leaveId;
					document.frm.action="leave_list_emp.jsp";
                            ...
                            <tr>
                                ...
                                <td style="background-color: #FFF;">
                                    ...
                                    if (leave.getDocStatus() == PstLeaveApplication.FLD_STATUS_APPlICATION_APPROVED
										...
										<a href="javascript:cmdDelete('<%= leave.getOID() %>')">Hapus</a> |
                                        <%
                                    }
                                    %>

...
static final int SEARCH_CLOSE_TO_ALL    = 3;

public String drawList(Vector result_parameter,Vector result_all,int intervals,int intervals_2,I_Leave leaveConfig, int status)
{
	...
	ctrlist.addHeader("<center>"+dictionaryD.getWord(I_Dictionary.TAKEN)+"</center>","5%");	
		...
		int maxList=0;
        if(result_parameter != null && result_parameter.size() > 0){        
          maxList= result_parameter.size(); 
        }
        int maxListEmpty=0;
        ...
        ctrlist.addHeader("<center>"+dictionaryD.getWord(I_Dictionary.CLOSING)+"</center>"+link,"5%");	
             ...
             if(leaveConfig!=null && leaveConfig.getLLShowEntile2()==false){
                ...
                if(statusApplicationNotClose == true && statusLongLeave == SessLeaveClosing.LL_CLOSE){
                    ...
                    if(countData > 1){
						...
						if(status == 3){
							rowx.add("<input type=\"text\" name=\"data_extend"+i+"\" value=\""+String.format("%.0f", extend_val)+"\" size=\"3\">"+" "+drawlist);
                            ...
                            rowx.add("-");
						} else if(statusLongLeave == SessLeaveClosing.LL_CLOSE){
                            ...
                            rowx.add("<input type=\"text\" name=\"data_extend"+i+"\" value=\""+String.format("%.0f", residu)+"\" size=\"3\">"+" "+drawlist);
...
if(iCommand==Command.ACTIVATE){
    ...
    if(result_close != null && result_close.size() > 0){
		...
		if (objSrcLeaveAppAlClosing.getStatus() == 3){
			SessLeaveClosing.ProcessClosingLLtoAL(result_close);
		} else {
...
if(Type==SEARCH_BY_PARAMETER){
    ...
    // untuk pencarian 
	if (objSrcLeaveAppAlClosing.getStatus() == 3){
		resultLL_all = SessLeaveClosing.listLLClosingLLByParameterTransferAL(objSrcLeaveAppAlClosing);
	} else {
                                          ...
                                          <td width="85%"> 
											...
											Vector listDept = new Vector();
                                                                                        if(objSrcLeaveAppAlClosing.getDivisionId() != 0){
                                                                                            listDept = PstDepartment.list(0, 0, "hr_department.DIVISION_ID="+objSrcLeaveAppAlClosing.getDivisionId() + " AND " + PstDepartment.TBL_HR_DEPARTMENT + "." + PstDepartment.fieldNames[PstDepartment.FLD_VALID_STATUS] + " = " + PstDepartment.VALID_ACTIVE, " DEPARTMENT ");                                                        
                                                                                        }
...
*/
                                          ...
                                          <td width="85%"> 
                                            <%														
                                                ...
                                                if(objSrcLeaveAppAlClosing.getDepartmentId() != 0){
                                                    listSec = PstSection.list(0, 0, "DEPARTMENT_ID="+objSrcLeaveAppAlClosing.getDepartmentId() + " AND " + PstSection.fieldNames[PstSection.FLD_VALID_STATUS] + " = " + PstSection.VALID_ACTIVE, " SECTION ");                                                          
                                                }                                                         
					    ...
					    %>
                                        ...
                                        <%if(objSrcLeaveAppAlClosing.getStatus()==0){%>
										  ...
										  <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="3" class="elemenForm"   onkeydown="javascript:fnTrapKD()">Close to AL
                                        <%}else if(objSrcLeaveAppAlClosing.getStatus()==1){%>
                                          ...
                                          <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="2" class="elemenForm"   onkeydown="javascript:fnTrapKD()">All
										  <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="3" class="elemenForm"   onkeydown="javascript:fnTrapKD()">Close to AL
                                        <%}else if(objSrcLeaveAppAlClosing.getStatus()==2){%>
                                            <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="0" class="elemenForm"   onkeydown="javascript:fnTrapKD()">Active
                                          <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="1" class="elemenForm"   onkeydown="javascript:fnTrapKD()">To Be Close
                                        ...
                                        <%} else {%>
                                          ...
                                          <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="1" class="elemenForm"   onkeydown="javascript:fnTrapKD()">To Be Close
                                          <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="2" class="elemenForm"   onkeydown="javascript:fnTrapKD()">All
										  <input type="radio" name="<%=objFrmSrcLeaveAppAlClosing.fieldNames[objFrmSrcLeaveAppAlClosing.FRM_FIELD_STATUS] %>"  value="3" class="elemenForm"  checked="checked" onkeydown="javascript:fnTrapKD()">Close to AL
										<%}%>
                                        ...
                                        <% 
                                           if(resultLL_all!=null && resultLL_all.size()>0){
                                               ...
                                               out.println(drawList(resultLL_all,resultLL_parameter,configuration,configuration_2,leaveConfig, objSrcLeaveAppAlClosing.getStatus()));

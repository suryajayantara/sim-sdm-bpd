...
<%
	...
	boolean needSaving = FRMQueryString.requestBoolean(request,"needSaving");
        boolean processFormula = FRMQueryString.requestBoolean(request,"processFormula");
                    ...
                    for(int l=0;l<vItems.size();l++){
                        ...
                        if(assItem.getPage()==prevPage){
                            ...
                            assItem.getType() == PstAssessmentFormItem.ITEM_TYPE_KPI_EMPLOYEE_POSITION
                                ...
                                float rating = 0;
                                if(processFormula){
                                    if(assItem.getFormula() != null && !(assItem.getFormula().equals(""))){
                                        Appraisal app = PstAppraisal.getDataFromFormula(oidAppraisalMain, assItem.getOID());
                                        appTemp.setRealization(app.getRealization());
                                        appTemp.setPoint(app.getPoint());
                                        appTemp.setRating(app.getRating());
                                        appTemp.setEvidence(app.getEvidence());
                                    }
                                }
								...
								long tempHrAppMainOid = oidAppraisalMain;
								String[] oidKpi = request.getParameterValues("FRM_FIELD_KPI"+assItem.getOID());    
								if (oidKpi != null){
									for (int i = 0; i < oidKpi.length; i++){
										long tempAppraisalOid = FRMQueryString.requestLong(request, FrmAppraisal.fieldNames[FrmAppraisal.FRM_FIELD_APPRAISAL_ID]+assItem.getOID()+"_"+oidKpi[i]);
										long tempAssItemOid = FRMQueryString.requestLong(request, FrmAppraisal.fieldNames[FrmAppraisal.FRM_FIELD_ASS_FORM_ITEM_ID]+assItem.getOID()+"_"+oidKpi[i]);
										float tempRealization  = FRMQueryString.requestFloat(request, "FRM_FIELD_REALIZATION"+assItem.getOID()+"_"+oidKpi[i]);
										float tempWeight  = FRMQueryString.requestFloat(request, "FRM_FIELD_WEIGHT_POINT"+assItem.getOID()+"_"+oidKpi[i]);
										float tempPoint  = FRMQueryString.requestFloat(request, "FRM_FIELD_POINT"+assItem.getOID()+"_"+oidKpi[i]);
										String tempEvidence= FRMQueryString.requestString(request, "FRM_FIELD_WEIGHT_EVIDENCE"+assItem.getOID()+"_"+oidKpi[i]);
										double rating = tempWeight * tempPoint;
										...
										appTemp.setAppMainId(tempHrAppMainOid);
										appTemp.setAssFormItemId(tempAssItemOid);
										appTemp.setEmpComment("");
										appTemp.setAssComment("");
										appTemp.setAnswer_1("");
										appTemp.setAnswer_2("");
										appTemp.setAnswer_3("");
										appTemp.setAnswer_4("");
										...
										appTemp.setEvidence(tempEvidence);                                                                
										appTemp.setPoint(tempPoint);
										appTemp.setRating(rating);
										appTemp.setKpiId(Long.valueOf(oidKpi[i]));
										appTemp.setWeight(tempWeight);
										//totalRating= totalRating + tempRating;
										long tempOid = 0;
										try{
										if(appTemp.getOID()>0){
...
function cmdSave(){
	...
	document.<%=FrmAppraisal.FRM_APPRAISAL%>.command.value="<%=String.valueOf(Command.SAVE)%>";
        document.<%=FrmAppraisal.FRM_APPRAISAL%>.needSaving.value="true";
	document.<%=FrmAppraisal.FRM_APPRAISAL%>.processFormula.value="false";
        document.<%=FrmAppraisal.FRM_APPRAISAL%>.action="appraisalForm.jsp";
	document.<%=FrmAppraisal.FRM_APPRAISAL%>.submit();
	}
...
function cmdProcess(){
	...
	document.<%=FrmAppraisal.FRM_APPRAISAL%>.command.value="<%=String.valueOf(Command.SAVE)%>";
        document.<%=FrmAppraisal.FRM_APPRAISAL%>.needSaving.value="true";
        document.<%=FrmAppraisal.FRM_APPRAISAL%>.processFormula.value="true";
	document.<%=FrmAppraisal.FRM_APPRAISAL%>.action="appraisalForm.jsp";
	document.<%=FrmAppraisal.FRM_APPRAISAL%>.submit();
	}
                          ...
                          <tr> 
                            <td valign="top"> <!-- #BeginEditable "content" --> 
                              <form name="<%=FrmAppraisal.FRM_APPRAISAL%>" method="post" action="">
                                ...
                                <input type="hidden" name="processFormula" value="<%=String.valueOf(processFormula)%>">
                                                        ...
                                                        <table width="100%" border="0" cellspacing="1" cellpadding="1" >
                                                            <tr>
                                                                ...
                                                                <td width="80%" align="right">
                                                                    <%
                                                                    if(currPage!=0){
                                                                    out.println("<span class='pagelink2'><a href='javascript:cmdProcess()'>Process</a></span> <span class='pagelink2'><a href='javascript:cmdSave()'>Save</a></span>");
                                                                    }
                                                                    %>
                                                                </td>
                                                            <tr>

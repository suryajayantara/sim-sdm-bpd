...
%>
<%!
public String drawList(Vector objectClass, int st,FrmEmpDocList frmObject, long empDocL, String ObjectName, Hashtable listEmpExist){
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		rowx.add("<input type=\"text\" name=\""+frmObject.fieldNames[FrmEmpDocList.FRM_FIELD_JOB_DESC] +i+"\" value=\""+empDocList.getJob_desc()+"\" class=\"elemenForm\"><input type=\"hidden\" name=\""+frmObject.fieldNames[FrmEmpDocList.FRM_FIELD_EMPLOYEE_ID] +i+"\" value=\""+employee.getOID()+"\" class=\"elemenForm\"> <input type=\"hidden\" name=\""+frmObject.fieldNames[FrmEmpDocList.FRM_FIELD_EMP_DOC_ID] +i+"\" value=\""+empDocL+"\" class=\"elemenForm\"><input type=\"hidden\" name=\""+frmObject.fieldNames[FrmEmpDocList.FRM_FIELD_OBJECT_NAME] +i+"\" value=\""+ObjectName+"\" class=\"elemenForm\">");
                ...
                int v = 1;
                String checked = "";
                if (listEmpExist.get(employee.getOID()) != null){
                    checked = "checked";
                }
                rowx.add("<input type=\"checkbox\" name=\"userSelect"+i+"\" id=\"userSelect"+i+"\" value=\""+v+"\" "+checked+" >");
...
//    String whereClause = "RESIGNED = 0";
    ...
    where ="";  
      iErrCode = ctrlEmpDocList.action(iCommand , oidEmpDocList,srcEmployee,start,recordToGet,where,objectName,oidEmpDoc,0);
   ...
   String whereC = " "+PstEmpDocList.fieldNames[PstEmpDocList.FLD_OBJECT_NAME] + " = \"" + objectName+"\" AND "+PstEmpDocList.fieldNames[PstEmpDocList.FLD_EMP_DOC_ID] + " = \"" + oidEmpDoc+"\"";
   Hashtable listEmpExist = PstEmpDocList.listHastExist(0, 0, whereC, ""); 
%>
      ...
      ||(iCommand==Command.LAST)||(iCommand==Command.LIST))) { %>
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                              <%if((listEmployee!=null)&&(listEmployee.size()>0)){%>
                                                <tr> 
                                                  <td height="8" width="100%"><%=drawList(listEmployee, start,frmEmpDocList,oidEmpDoc ,objectName,listEmpExist)%></td>
                                                </tr>
                                                <%}else{%>
                                                <tr> 

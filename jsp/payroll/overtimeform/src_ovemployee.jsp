...
<%
    ...
    String dateReq = FRMQueryString.requestString(request, FrmOvertime.fieldNames[FrmOvertime.FRM_FIELD_REQ_DATE]);
...
**/
            String dateFrom =  FRMQueryString.requestString(request, FrmOvertimeDetail.fieldNames[FrmOvertimeDetail.FRM_FIELD_DATE_FROM]);
            String dateTo = FRMQueryString.requestString(request, FrmOvertimeDetail.fieldNames[FrmOvertimeDetail.FRM_FIELD_DATE_TO]);
    ...
    if(iCommand==Command.SUBMIT){  
        ...
        if(empLst!=null && empLst.length > 0 && oidOvertime!=0){
            ...
            Date dtNow = new Date();
            if (!(dateFrom.equals(""))){
                dtNow = df.parse(dateFrom);
            }

            Date dtEnd = new Date();
            if (!(dateTo.equals(""))){
                dtEnd = df.parse(dateTo);
            }
             ...
             try{
                ...
                if(ovd.getDateFrom()!=null && ovd.getDateTo()!=null){
                  ...
                  if(vListOverlapSchedule!=null && vListOverlapSchedule.size()>0){
                   ...
                   try {
                        String strBackDate = PstSystemProperty.getValueByNameWithStringNull("OVERTIME_CAN_INPUT_BACKDATE");
                        boolean canInputBackdate = Boolean.valueOf(strBackDate);
                        if (!canInputBackdate){
                            Employee emp = PstEmployee.fetchExc(ovd.getEmployeeId());
                            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
                            if (dateFormat.parse(dateFormat.format(ovd.getDateFrom())).before(dateFormat.parse(dateFormat.format(new Date())))){
                                iErrCode = 8 ;
                                msgString = msgString + " Overtime detail " + "<B>"+emp.getFullName() +"</B>"+ " " +  " Tidak dapat input form lembur untuk tanggal yang sudah lewat <br>";
                   ...
                   try {
                        String limitStart = PstSystemProperty.getValueByNameWithStringNull("OVERTIME_MIN_START_TIME");
                        
                        Employee emp = PstEmployee.fetchExc(ovd.getEmployeeId());
                        boolean isCheck = true;
                        try {
                            String[] oidLevel = PstSystemProperty.getValueByName("LEVEL_OID_NOT_LIMIT_OVERTIME").split(",");
                            if (oidLevel != null){
                                for (String oid : oidLevel){
                                    long lOid = Long.valueOf(oid);
                                    if (lOid == emp.getLevelId()){
                                        isCheck = false;
                                    }
                                }
                            }
                        ...
                        if (isCheck){
                            Vector listPH = PstPublicHolidays.list(0, 0, "'"+Formater.formatDate(ovd.getDateFrom(), "yyyy-MM-dd")+"' BETWEEN  holiday_date AND holiday_date_to", "holiday_status");
                            boolean isPH = false;
                            if (listPH.size()>0){
                            ...
                            boolean ovtEndOfYear = PstOvertime.endOfYearOvertime(ovd.getDateFrom());
                            if (!isPH && !ovtEndOfYear){
                                String[] stDays = {
                                    "Sunday","Monday","Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"
                                };

                                Calendar objCal = Calendar.getInstance();
                                objCal.setTime(ovd.getDateFrom());

                                String day = stDays[objCal.get(Calendar.DAY_OF_WEEK)-1];
                                if(day != stDays[0] && day != stDays[6]){
                                    SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm") ;
                                    if (dateFormat.parse(dateFormat.format(ovd.getDateFrom())).before(dateFormat.parse(limitStart))){
                                        iErrCode = 7;
                                        cekError = false;
                                        msgString = msgString + " Overtime detail " + "<B>"+emp.getFullName() +"</B>"+ " " +  " Jam Lembur tidak boleh kurang dari Pukul "+limitStart+"<br>";
                                    }
                                }
                            }
                        }
...
//                                                        String strDtEnd = sdf.format(endDate);
                                                        String strDtNow = dateReq+" 00:00";
                                                        String strDtEnd = dateReq+" 00:00";
                                                      %>
                                                  From <input type="text" name="<%=FrmOvertimeDetail.fieldNames[FrmOvertimeDetail.FRM_FIELD_DATE_FROM]%>" id="datetimepicker" value="<%=strDtNow%>" />
                                                  <td >&nbsp;</td>
                                                  <td nowrap>To <input type="text" name="<%=FrmOvertimeDetail.fieldNames[FrmOvertimeDetail.FRM_FIELD_DATE_TO]%>" id="datetimepicker2" value="<%=strDtEnd%>" />                                                   
                                                  <td >&nbsp;</td>

...
long oidOvertime = FRMQueryString.requestLong(request, FrmOvertime.fieldNames[FrmOvertime.FRM_FIELD_OVERTIME_ID]);
//update by satrya 2014-01-13
String source = FRMQueryString.requestString(request, "source");
long oidEmployee = FRMQueryString.requestLong(request, FrmOvertime.fieldNames[FrmOvertime.FRM_FIELD_REQ_ID]);
...
if(loggedInSuccess)
{
    ...
    switch(indexApproval){
        case 1: 
            %>
                <jsp:forward page="overtime.jsp"> 
                <jsp:param name="command" value="<%=Command.EDIT%>" />	
                <jsp:param name="start" value="<%=start%>" />
                <jsp:param name="overtime_oid" value="<%=oidOvertime%>" />
...
{
  //getApprovalDate();
  document.frm_overtime.command.value="<%=String.valueOf(Command.SUBMIT)%>"; 
  document.frm_overtime.action = "overtime_approval.jsp";
...
{
  document.frm_overtime.command.value="<%=String.valueOf(Command.EDIT)%>"; 
  document.frm_overtime.<%=FrmOvertime.fieldNames[FrmOvertime.FRM_FIELD_OVERTIME_ID]%>.value=oidOvertime; 
  document.frm_overtime.<%=FrmOvertime.fieldNames[FrmOvertime.FRM_FIELD_REQ_ID]%>.value=oidEmployee; 
  document.frm_overtime.hidden_dep_head_authorize.value=depHeadAuthorize;      
  document.frm_overtime.action = "overtime.jsp";
  document.frm_overtime.submit();
}

function getApprovalDate(){    
     <%=ControlDatePopup.writeDateCaller("frm_overtime","ApprovalDates")%>
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%" bgcolor="#FFFFFF">
  ...
  <tr>
    <td valign="middle" align="left">
      <form name="frm_overtime" action="" onsubmit="window.status=''">
        <input type="hidden" name="command" value="<%=String.valueOf(iCommand)%>">  
        <input type="hidden" name="<%=FrmOvertime.fieldNames[FrmOvertime.FRM_FIELD_OVERTIME_ID]%>" value="<%=String.valueOf(oidOvertime)%>">
        <input type="hidden" name="<%=FrmOvertime.fieldNames[FrmOvertime.FRM_FIELD_REQ_ID]%>" value="<%=oidEmployee%>">
...
<script language="JavaScript">
 document.frm_overtime.pass_wd.focus();
</script>
</body>
</html>

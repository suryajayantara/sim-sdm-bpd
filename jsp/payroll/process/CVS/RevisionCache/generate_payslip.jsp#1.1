<%-- 
    Document   : generate_payslip
    Created on : Aug 16, 2016, 11:06:27 AM
    Author     : Dimata 007
--%>
...
<%
    ...
    long companyId = FRMQueryString.requestLong(request, "frm_company");
    long divisionId = FRMQueryString.requestLong(request, "frm_division");
    long departmentId = FRMQueryString.requestLong(request, "frm_department");
    long sectionId = FRMQueryString.requestLong(request, "frm_section");
    ...
    String commDate = FRMQueryString.requestString(request, "commDate");
    ...
    if (iCommand == Command.VIEW){
        ...
        if (commDate.length()>0){
            whereClause = " "+PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE]+" > '"+commDate+"'";
            whereCollect.add(whereClause);
        }
        ...
        listEmployee = PstPayEmpLevel.getDataEmpLevelActive(whereClause);
    }
%>
<html>
    <head>
...
<script type="text/javascript">
    function cmdGenerate(){
        ...
        document.frm.action="generate_payslip.jsp";
        document.frm.submit();
    }
    function cmdBack(){
...
<script>
    ...
    <body onload="pageLoad()">
        ...
        <div id="menu_utama">
            <span id="menu_title"><a class="btn-small" style="color:#FFF;" href="javascript:cmdBack()">Back</a> Generate Slip Gaji</span>
        </div>
                ...
                <table width="100%">
                    <tr>
                        <td valign="top" width="30%">
                            ...
                            <div class="caption">Level Gaji</div>
                            <div class="divinput">
                                <select name="level_code">
                                    <option value="0">-Select-</option>
                                    <%
                                    Vector listLevelGaji = PstSalaryLevel.list(0, 0, "", "");
                                    if (listLevelGaji != null && listLevelGaji.size()>0){
                                        for (int i=0; i<listLevelGaji.size(); i++){
                                            SalaryLevel salary = (SalaryLevel)listLevelGaji.get(i);
                                            %>
                                            <option value="<%=salary.getLevelCode()%>"><%= salary.getLevelCode() %></option>
                                            <%        
                                        }
                                    }
                                    %>
                            ...
                            <div class="caption">Tanggal Bergabung dari</div>
                            <div class="divinput">
                                <input type="text" name="commDate" id="date_from" value="" class="datepicker" />
                            </div>
                            <div>&nbsp;</div>
                            <a href="javascript:cmdGenerate()" class="btn" style="color:#FFF;">Generate</a>
                        </td>
                        <td valign="top" width="70%">
                            ...
                            if (iCommand == Command.VIEW){
                                ...
                                %>
                                <% PstPaySlip.generatePaySlip(periodId, listEmployee, output); %>
                                <%
                            }
                            %>
                        </td>

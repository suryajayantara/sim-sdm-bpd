...
// test 123
        if (listPayInput != null && listPayInput.size() > 0) {
            ...
            if (iCommand == Command.EDIT || iCommand == Command.SUBMIT) { // test dikecogi
...
// test kode di bawah dikecogi lo...!
                ...
                if ((iCommand == Command.EDIT || iCommand == Command.SUBMIT) && prosess == 1) { // ini dikecogi lo...!!
                    ///searchParamnya
                    //query
                    //lalu action'nya
	...
	public static void processAll(int iCommand, Vector listPayInput, HttpServletRequest request, Vector listPos, Vector listReason, ParamPayInput paramPayInput, Hashtable listPayInputTbl, Hashtable existPayInputId, Hashtable hashPrevPrivateNote, int[] nightAllow){
		...
		if (listPayInput != null && listPayInput.size() > 0) {
            ...
            for (int i = 0; i < listPayInput.size(); i++) { // test for listPayInput
                ...
                if ((iCommand == Command.EDIT || iCommand == Command.SUBMIT)) { // ini dikecogi lo...!!
                    ///searchParamnya
                    //query
                    //lalu action'nya
                    //untuk melihat data" overtime
...
<%
    ...
    if (iCommand == Command.LIST) {
        ...
        if (paramPayInput != null && paramPayInput.getsEmployeeId() != null && paramPayInput.getsEmployeeId().length() > 0 && paramPayInput.getPeriodId() != 0) {
            listPayInput = SessPayInput.listPayInput(departmentName, companyId, oidDivision, (levelCode), sectionName, searchNrFrom, searchNrTo, searchName, periodId, dataStatus, isChekedRadioButtonSearchNr, searchNr);
            nightAllowanceEmp = new int[listPayInput.size()];
    ...
    else if (iCommand == Command.EDIT || iCommand == Command.SAVE || iCommand == Command.SUBMIT) {
	...
	if (iCommand == Command.SUBMIT){
		processAll(iCommand,listPayInput,request /*,hashCheked*/,listPos,listReason,paramPayInput,listPayInputTbl,existPayInputId,hashPrevPrivateNote, nightAllowanceEmp);
	}
%>
...
function cmdGenerateAll(){
	document.frm_input_pay.command.value="<%=Command.SUBMIT%>";
	document.frm_input_pay.action="pay-input.jsp";
	document.frm_input_pay.submit();
}
          ...
          <td width="100%"> 
			...
			%> <%= ControlCombo.draw("section", "formElemen", null, "" + sectionName, sec_value, sec_key, " onkeydown=\"javascript:fnTrapKD()\"")%> 
                                        ...
                                        <tr> 
                                          ...
                                          <a href="javascript:cmdSearch()">Search  for Employee</a>
											<img src="<%=approot%>/images/spacer.gif" width="6" height="1">
                                          <a href="javascript:cmdGenerateAll()">Generate All</a></td>                                          
					...
					<tr>
                                        ...
                                        <tr> 
										 <td height="13" width="0%">&nbsp;</td>
										  ...
										  if((listPayInput!=null)&&(listPayInput.size()>0) && iCommand != Command.SUBMIT){
